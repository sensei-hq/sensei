<script lang="ts">
  ...
  let myProjects = $derived(serverProjects.filter(p => repoIds.has(p.repoId)));
  let indexedCount = $derived(myProjects.filter(p => p.indexedAt).length);
  let failedCount = $derived(myProjects.filter(p => p.lastError).length);
  let partialCount = $derived(myProjects.filter(p => p.partiallyIndexed).length);
  let runningCount = $derived(activeQueue.filter(id => repoIds.has(id)).length);
  let totalCount = $derived(solution?.repos.length ?? 0);
  let unindexedCount = $derived(totalCount - indexedCount - failedCount - partialCount);
  ...
  function getStatus(repoId: string): 'indexed' | 'indexing' | 'failed' | 'partial' | 'pending' {
    ...
    const p = myProjects.find(p => p.repoId === repoId);
  ...
  async function retryAllFailed() {
    if (!solution) return;
    const failed = solution.repos.filter(r => {
      const s = getStatus(r.repoId);
  ...
  <div class="flex-1 overflow-y-auto px-5 py-4 space-y-4">
    ...
    {#if runningCount > 0 || indexingRepos.size > 0}
      <div>
        ...
        {#each solution.repos.filter(r => getStatus(r.repoId) === 'indexing') as repo (repo.repoId)}
          ...
          <div class="rounded-lg bg-primary-z1 border border-primary-z2 px-3 py-2.5 mb-1.5 space-y-1.5">
            <div class="flex items-center gap-2">
              ...
              <span class="text-sm font-medium text-primary-z7 flex-1 truncate">{repo.label ?? repo.path.split('/').at(-1)}</span>
              {#if prog}
    ...
    <!-- All repos -->
    <div>
      <p class="text-[10px] font-semibold text-surface-z5 uppercase tracking-wide mb-2">All repos</p>
      <div class="rounded-lg border border-surface-z0/50 divide-y divide-surface-z0/30">
        {#each solution.repos as repo (repo.repoId)}
          {@const status = getStatus(repo.repoId)}
          {@const serverInfo = myProjects.find(p => p.repoId === repo.repoId)}
          {@const prog = progressMap[repo.repoId]}
          <div class="px-3 py-2 flex items-center gap-3">
            <span class="rounded px-1.5 py-0.5 text-[10px] font-medium {STATUS_CLS[status]}">{status}</span>
            <div class="flex-1 min-w-0">
              <p class="text-sm text-surface-z7 truncate">{repo.label ?? repo.path.split('/').at(-1)}</p>
