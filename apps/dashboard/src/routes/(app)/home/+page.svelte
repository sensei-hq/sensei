...
<div class="p-4 sm:p-6 lg:p-7">
  ...
  <div class="mb-6 flex flex-wrap items-start justify-between gap-3">
    <div>
      ...
      <p class="mt-0.5 text-sm text-surface-z5">{data.orgName} · {data.orgRole || 'Member'} · {data.email}</p>
    </div>
    <div class="flex items-center gap-3">
      ...
      <div class="flex h-8 w-8 items-center justify-center rounded-full bg-primary-z6 text-xs font-bold text-white">{initials}</div>
    </div>
  </div>
      ...
      <div class="flex items-center gap-4">
        <div class="relative h-20 w-20 shrink-0">
          ...
          <div class="absolute inset-0 flex flex-col items-center justify-center">
            {#if data.ftr != null}
              <span class="text-lg font-bold {ftrColor(data.ftr)}">{data.ftr}%</span>
            {:else}
              <span class="text-lg font-bold text-surface-z4">—</span>
            {/if}
        ...
        <div>
          ...
          {:else}
            <p class="mt-1 text-xs text-surface-z4">No sessions yet</p>
          {/if}
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <div class="space-y-2">
        {#if data.repos.length === 0}
          <div class="rounded-xl border border-dashed border-surface-z3 bg-surface-z1 px-4 py-6 text-center">
            <p class="text-sm text-surface-z5">No repos indexed yet.</p>
            <p class="mt-1 text-xs text-surface-z4">Run <code class="font-mono">sensei init</code> in a repo to get started.</p>
          </div>
        {:else}
          {#each data.repos as repo}
            <div class="flex cursor-pointer items-center gap-3 rounded-xl border border-surface-z3 bg-surface-z2 px-4 py-3 transition-colors hover:border-surface-z4">
              <div class="min-w-0 flex-1">
                ...
                <p class="truncate text-xs text-surface-z5">{repo.lang} · {repo.indexed}</p>
              </div>
            </div>
          {/each}
        {/if}
  ...
  <div class="rounded-xl border border-surface-z3 bg-surface-z2 overflow-hidden">
    ...
    {#if data.sessions.length === 0}
      <div class="px-5 py-8 text-center">
        ...
        <p class="mt-1 text-xs text-surface-z4">Connect via MCP and run a task to see sessions here.</p>
      </div>
    {:else}
