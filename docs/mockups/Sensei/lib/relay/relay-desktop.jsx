...
const Dot = ({ c, pulse }) => <span style={{ width: 7, height: 7, borderRadius: '50%', background: c, flexShrink: 0, animation: pulse ? 'relayPulse 2s infinite' : 'none' }}/>;
...
function RelayWindow({ active = 'relay', children }) {
  ...
  return (
    <div className="artboard-shell" style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: rc('paper'), overflow: 'hidden' }}>
      <TauriChrome title="Sensei  先生  ·  observatory"/>
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '240px 1fr', minHeight: 0 }}>
        <aside className="py-5 px-3 gap-4" style={{ borderRight: 'var(--hairline)', background: rc('paper-2'), display: 'flex', flexDirection: 'column' }}>
          <div className="px-1" style={{ fontFamily: rc('font-display'), fontSize: 22, fontWeight: 400, letterSpacing: '-.02em', display: 'flex', alignItems: 'baseline', gap: 8 }}>Sensei <span className="kanji" style={{ fontSize: 14, color: rc('ink-3') }}>先生</span></div>
          <div>
            <div className="pt-0 pb-2 px-2" style={{ fontSize: 11, letterSpacing: '.16em', color: rc('ink-3'), textTransform: 'uppercase' }}>Observatory</div>
            <div className="gap-1" style={{ display: 'flex', flexDirection: 'column' }}>
              {nav.map(([k, l, id]) => {
                const on = id === active;
                return (
                  <div key={l} className="gap-2 py-2 px-2" style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', alignItems: 'center', borderRadius: 6, background: on ? rc('paper') : 'transparent', border: on ? 'var(--hairline)' : '1px solid transparent', color: on ? rc('ink') : rc('ink-2'), fontSize: 13 }}>
                    <span className="kanji" style={{ fontSize: 13, width: 14, color: on ? rc('accent') : rc('ink-3') }}>{k}</span>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>{l}{id === 'relay' && <Dot c={rc('accent')}/>}</span>
        ...
        <main style={{ minHeight: 0, overflow: 'auto', background: rc('paper') }}>{children}</main>
      </div>
    </div>
  );
}
      ...
      <div className="p-6" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontFamily: rc('font-display'), fontSize: 28, fontWeight: 400, letterSpacing: '-.02em' }}>What your agents are relaying</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
          {[['1', 'pending gate', rc('accent')], ['3', 'agents running', rc('success')], ['2', 'machines', rc('ink')], ['2', 'phones paired', rc('ink')]].map(([n, l, c]) => (
            <div key={l} className="p-4 rounded-lg border-1px bg-paper-soft">
              <div style={{ fontFamily: rc('font-display'), fontSize: 30, fontWeight: 400, letterSpacing: '-.03em', color: c }}>{n}</div>
          ...
          <div>
            ...
            <div className="rounded-lg border-1px p-4" style={{ background: rc('accent-soft'), borderColor: 'color-mix(in oklch, var(--accent) 30%, transparent)' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
                ...
                <button className="zs-btn zs-btn-secondary" style={{ flex: 1, justifyContent: 'center' }}>Deny</button>
              </div>
            </div>
          </div>
        </div>
...
function RelayPlanAuthor() {
  ...
  return (
    <RelayWindow>
      <div className="p-6" style={{ display: 'flex', flexDirection: 'column', gap: 22, maxWidth: 900 }}>
        <div>
          <REB>信 · relay · plan</REB>
          <div style={{ fontFamily: rc('font-display'), fontSize: 28, fontWeight: 400, letterSpacing: '-.02em' }}>Refactor auth module</div>
          ...
          {steps.map(([label, mode, why], i) => {
            ...
            return (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 14, alignItems: 'center', padding: '16px 16px', borderBottom: i < steps.length - 1 ? 'var(--hairline)' : 'none' }}>
                ...
                <div style={{ display: 'flex', background: rc('paper-3'), borderRadius: 6, padding: 2 }}>
...
function Plane({ kanji, title, sub, tone, children }) {
  return (
    <div className="rounded-lg border-1px p-5" style={{ background: rc('paper-soft'), flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 14, borderColor: tone ? `color-mix(in oklch, ${tone} 24%, var(--edge))` : rc('edge') }}>
...
function MiniCard({ title, meta, tone }) {
  return (
    <div className="rounded border-1px p-3" style={{ background: rc('paper'), display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
      <Dot c={tone || rc('ink-3')} pulse={!!tone}/>
      <div style={{ minWidth: 0 }}><div className="text-sm" style={{ fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{title}</div><div className="mono text-xs text-ink-mute">{meta}</div></div>
    </div>
  );
}
...
function RelayArchitecture() {
  ...
  return (
    <div className="artboard-shell" style={{ width: '100%', height: '100%', background: rc('paper'), overflow: 'visible' }}>
      <div className="p-6" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        <div>
          <REB>信 · relay architecture</REB>
          <div style={{ fontFamily: rc('font-display'), fontSize: 30, fontWeight: 300, letterSpacing: '-.02em', maxWidth: 720, lineHeight: 1.1 }}>Your agents stay on your machine. Only what needs you leaves it.</div>
        ...
        <div style={{ display: 'flex', gap: 20, alignItems: 'stretch' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', color: rc('ink-4'), fontSize: 20 }}>→</div>
          <Plane kanji="鍵" title="Zero-knowledge relay" sub="end-to-end encrypted" tone={rc('ink-2')}>
            <div className="rounded border-1px p-3 mono text-xs" style={{ background: rc('paper'), color: rc('ink-3'), lineHeight: 1.7, overflowX: 'hidden' }}>
              <div>{'{'} event: "gate",</div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 14 }}>
            {round.map(([n, t, d], i) => (
              <div key={i} className="rounded-lg border-1px p-4 bg-paper-soft" style={{ display: 'flex', flexDirection: 'column', gap: 8, position: 'relative' }}>
                <span className="mono" style={{ fontSize: 12, color: rc('accent'), fontWeight: 600 }}>{n}</span>
...
function DojoRelayGates() {
  ...
  return (
    <div className="artboard-shell" style={{ width: '100%', height: '100%', background: rc('paper'), overflow: 'auto' }}>
      <div className="p-7" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          <div>
            <REB>群 · dōjō · shared relay</REB>
            <div style={{ fontFamily: rc('font-display'), fontSize: 30, fontWeight: 400, letterSpacing: '-.02em' }}>Gates across the team</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
          {[['3', 'awaiting', rc('accent')], ['12', 'resolved today', rc('ink')], ['1.4m', 'median to answer', rc('success')]].map(([n, l, c]) => (
            <div key={l} className="p-4 rounded-lg border-1px bg-paper-soft"><div style={{ fontFamily: rc('font-display'), fontSize: 28, fontWeight: 400, letterSpacing: '-.03em', color: c }}>{n}</div><div className="text-sm text-ink-mute" style={{ marginTop: 2 }}>{l}</div></div>
          ))}
        </div>
