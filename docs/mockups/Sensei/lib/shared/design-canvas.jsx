...
function DCSection({ id, title, subtitle, children, gap = 48 }) {
  ...
  return (
    <div className="relative" data-dc-section={sid} style={{ marginBottom: 48 }}>
      <div style={{ padding: '0 48px 24px' }}>
        <DCEditable tag="div" value={sec.title ?? title}
          onChange={(v) => ctx && sid && ctx.patchSection(sid, { title: v })}
          style={{ fontSize: 28, fontWeight: 600, color: DC.title, letterSpacing: -0.4,
                   display: 'inline-block', marginBottom: 4 }}/>
        {subtitle && <div style={{ fontSize: 15, color: DC.subtitle, maxWidth: 920 }}>{subtitle}</div>}
      </div>
      <div className="flex items-start" style={{ gap, width: 'max-content',
 padding: '0 48px' }}>
        {order.map((k) => (
...
function DCFocusOverlay({ entry, sectionMeta, sectionOrder }) {
 ...
 onMouseLeave={(e) => (e.currentTarget.style.background = 'rgba(255,255,255,.08)')}>
  ...
  return ReactDOM.createPortal(
    <div className="fixed" onClick={() => ctx.setFocus(null)}
 ...
 borderRadius: 16, fontSize: 22, lineHeight: 1, transition: 'background .12s' }}>×</button>
      ...
      <div
 style={{ top: 64, bottom: 56, left: 100, right: 100 }} className="gap-4 absolute flex flex-col items-center justify-center" >
        <div className="relative" onClick={(e) => e.stopPropagation()} style={{ width: width * scale, height: height * scale }}>
 ...
 boxShadow: '0 20px 80px rgba(0,0,0,.4)' }}>
        ...
        <div className="font-medium text-center" onClick={(e) => e.stopPropagation()} style={{ fontSize: 13, opacity: .85 }}>
