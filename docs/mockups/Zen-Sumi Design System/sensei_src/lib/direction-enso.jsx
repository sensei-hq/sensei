...
function EnsoSidebar({ page, setPage, solutions, activeSolution, setActiveSolution, data }) {
  return (
    <aside style={{
      ...
      flexShrink: 0, background: 'var(--paper-soft)'
    }}>
        ...
        {PAGES.map(p => (
          <button key={p.id}
                  ...
                  style={{
                    ...
                    color: page === p.id ? 'var(--accent)' : 'var(--ink-mute)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    transition: 'all .12s'
                  }}>
  ...
  return (
    <div style={{ padding: '40px 56px', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 56 }}>
        ...
        <div style={{ position: 'relative', width: 440, height: 440, margin: '0 auto' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex',
            ...
            <div className="display" style={{ fontSize: 98, fontWeight: 300, lineHeight: 1,
              ...
              <span style={{ fontSize: 30, color: 'var(--ink-mute)' }}>%</span>
            </div>
            <div style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase',
                          color: 'var(--ink-mute)', marginTop: 6 }}>一  First try right</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
        <div style={{ background: 'var(--paper-soft)', borderRadius: 14,
          ...
          <div style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase',
                        color: 'var(--accent)', marginBottom: 18 }}>師 · sensei says</div>
          <p className="display" style={{ fontSize: 28, fontWeight: 300, lineHeight: 1.25,
                        margin: 0, textWrap: 'balance' }}>
            {topCoach.koan}
          </p>
          <p style={{ fontSize: 13, color: 'var(--ink-soft)', lineHeight: 1.6, marginTop: 18, marginBottom: 24 }}>
            {topCoach.body}
          </p>
        ...
        <div>
          <div style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase',
                        color: 'var(--ink-mute)', marginBottom: 16 }}>Quality signals</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {[
              ...
              <div key={s.k} style={{ padding: 14, border: 'var(--hairline)', borderRadius: 8,
                ...
                <div style={{ fontSize: 10.5, color: 'var(--ink-mute)', letterSpacing: '0.05em' }}>{s.k}</div>
                <div className="display" style={{ fontSize: 22, fontWeight: 400, marginTop: 4 }}>{s.v}</div>
                <div className="mono" style={{ fontSize: 10, color: s.good ? 'var(--success)' : 'var(--accent)', marginTop: 2 }}>
                  {s.d}
                </div>
              </div>
            ))}
        ...
        <div style={{ marginTop: 8, color: 'var(--accent)' }}>
          <div style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase',
                        color: 'var(--ink-mute)', marginBottom: 8 }}>FTR · 14 days</div>
...
function Stat({ label, value, suffix }) {
  return (
    <div>
      <div style={{ fontSize: 10, color: 'var(--ink-mute)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{label}</div>
      <div className="display" style={{ fontSize: 20, fontWeight: 400, marginTop: 2 }}>{value}</div>
      <div className="mono" style={{ fontSize: 9.5, color: 'var(--ink-mute)', marginTop: 1 }}>{suffix}</div>
    </div>
  );
}
function Divider() {
          ...
          return (
            <button key={s.id} onClick={() => { setActiveSolution(s.id); setPage("observatory"); }}
                    style={{ padding: '28px 24px', borderRadius: 16, border: 'var(--hairline)',
                            background: 'var(--paper-soft)', textAlign: 'left', position: 'relative',
                            display: 'flex', flexDirection: 'column', alignItems: 'center',
                            transition: 'transform .15s, border-color .15s' }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--ink-mute)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = ''; e.currentTarget.style.transform = ''; }}>
              <div className="kanji" style={{ fontSize: 14, color: 'var(--accent)', alignSelf: 'flex-start' }}>{s.kanji}</div>
              <div className="display" style={{ fontSize: 18, fontWeight: 400, alignSelf: 'flex-start', marginTop: 4 }}>{s.name}</div>
              <div style={{ fontSize: 11, color: 'var(--ink-mute)', alignSelf: 'flex-start', marginBottom: 18 }}>{s.description}</div>
              <div style={{ position: 'relative', width: 180, height: 180 }}>
                ...
                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center',
                  ...
                  <span style={{ fontSize: 10, color: 'var(--ink-mute)', marginTop: 4, letterSpacing: '0.12em' }}>FTR</span>
                </div>
              </div>
...
function EnsoSessionCard({ s, expanded, onToggle }) {
  return (
    <div style={{ border: 'var(--hairline)', borderRadius: 10,
                  background: expanded ? 'var(--paper-soft)' : 'var(--paper)',
          ...
          {(s.events || [{kind:'start'},{kind:'edit'},{kind:'test'},{kind:'end'}]).map((e, i) => (
            <div key={i} style={{
              ...
              background: e.kind === 'correction' ? 'var(--accent)' :
                          ...
                          e.kind === 'edit' ? 'var(--ink-faint)' : 'var(--paper-edge)'
            }} title={e.kind}/>
          ))}
        </div>
        <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)', textAlign: 'right' }}>
          {s.duration}
        </span>
        <span style={{ color: 'var(--ink-mute)', transform: expanded ? 'rotate(90deg)' : '',
                        transition: 'transform .15s' }}>›</span>
      </button>

      {expanded && s.events && (
        <div style={{ padding: '0 18px 20px 122px', borderTop: 'var(--hairline)' }}>
          <div style={{ padding: '16px 0 12px', fontSize: 12, color: 'var(--ink-soft)',
    ...
    <div style={{ padding: '36px 48px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 36 }}>
        ...
        <div style={{ border: 'var(--hairline)', borderRadius: 12, padding: 20,
                      background: 'var(--paper-soft)', position: 'relative', minHeight: 420 }}>
          <div style={{ fontSize: 10.5, color: 'var(--ink-mute)', letterSpacing: '0.12em',
          ...
          <svg viewBox="0 0 500 400" width="100%" height="400" style={{ marginTop: 10 }}>
            ...
            <text x="340" y="92"  fontSize="9.5" fill="var(--ink-mute)" fontFamily="var(--font-mono)">core · 8</text>
            <text x="100" y="120" fontSize="9.5" fill="var(--ink-mute)" fontFamily="var(--font-mono)">api · 14</text>
            <text x="370" y="330" fontSize="9.5" fill="var(--ink-mute)" fontFamily="var(--font-mono)">ui · 22</text>
          </svg>
        </div>
          ...
          {data.hotspots.map((h, i) => (
            <div key={i} style={{ padding: 14, border: 'var(--hairline)', borderRadius: 8,
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                <span className="mono" style={{ fontSize: 11.5, color: 'var(--ink)' }}>{h.name}</span>
              </div>
...
function Mini({ label, value, warn }) {
  return (
    <div>
      <div style={{ fontSize: 9.5, color: 'var(--ink-mute)', letterSpacing: '0.08em' }}>{label}</div>
      <div className="mono" style={{ fontSize: 14, color: warn ? 'var(--accent)' : 'var(--ink)' }}>{value}</div>
    </div>
  );
}
        ...
        <Card title="Libraries" kanji="書">
          {data.libraries.map(l => (
            <div key={l.name} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0',
              ...
              <span className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)' }}>v{l.version}</span>
              <span className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)' }}>{l.pages}p</span>
            </div>
          ))}
        </Card>
      </div>
      <Card title="Assistants (ACPs)" kanji="助">
        {[
          ...
          <div key={a.name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0',
            ...
            <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>v{a.version}</span>
...
function Card({ title, kanji, children }) {
  return (
    <div style={{ padding: 22, border: 'var(--hairline)', borderRadius: 12, background: 'var(--paper-soft)' }}>
  ...
  return (
    <div style={{ padding: 60, display: 'flex', alignItems: 'center', gap: 72, minHeight: '100%' }}>
      ...
      <div style={{ flex: 1, maxWidth: 460 }}>
        <div className="kanji" style={{ fontSize: 48, color: 'var(--accent)' }}>始</div>
        ...
        {steps.map((t, i) => (
          <div key={i} style={{ display: 'flex', gap: 14, padding: '10px 0',
            ...
            <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)', width: 24 }}>
              {i+1 < step ? '✓' : '0' + (i+1)}
            </span>
            <span style={{ flex: 1, fontSize: 14 }}>{t}</span>
            {i+1 === step && <span className="mono" style={{ fontSize: 10, color: 'var(--accent)' }}>active</span>}
          </div>
        ))}

        <button onClick={() => setStep(Math.min(4, step+1))}
                style={{ marginTop: 28, padding: '11px 24px', background: 'var(--accent)',
