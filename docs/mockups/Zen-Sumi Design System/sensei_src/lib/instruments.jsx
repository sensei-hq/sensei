      ...
      {simple ? (
        ...
        <>
          ...
          <div style={{ padding: '26px 56px 18px', display: 'flex',
            ...
            <div className="kanji" style={{ fontSize: 44, color: 'var(--accent)', lineHeight: 1 }}>
              {kanji || "具"}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 10.5, letterSpacing: '0.18em', color: 'var(--ink-mute)',
              ...
              return (
                <button key={t.id} onClick={() => onTab && onTab(t.id)}
                  ...
                  <span className="kanji" style={{ fontSize: 15,
                                color: on ? 'var(--accent)' : 'var(--ink-mute)' }}>{t.kanji}</span>
                  <span className="display" style={{ fontSize: 13 }}>{t.label}</span>
                  <span style={{ fontSize: 10.5, color: 'var(--ink-faint)' }}>· {t.hint}</span>
                </button>
              );
            })}
          </div>
...
function InstrumentsPlayground({ activeTab = "playground", onTab = () => {},
  ...
  return (
    <InstrumentsShell activeTab={activeTab} onTab={onTab} embedded={embedded} simple={simple}
      ...
      chip={
        <div style={{ display: 'flex', gap: 12, fontSize: 11, color: 'var(--ink-mute)' }}>
      ...
      <div style={{ padding: '14px 56px', borderBottom: 'var(--hairline)',
        ...
        <span style={{ fontSize: 9.5, letterSpacing: '0.16em', color: 'var(--ink-mute)',
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          ...
          <button style={{ padding: '7px 12px', fontSize: 11.5,
                            color: 'var(--ink-mute)', border: '1px dashed var(--paper-edge)',
      ...
      <div style={{ padding: '10px 56px', borderBottom: 'var(--hairline)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8,
                       background: 'var(--paper-soft)', borderRadius: 5,
                       padding: '6px 10px', border: 'var(--hairline)', minWidth: 260 }}>
          <span className="kanji" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>探</span>
          ...
          {q && (
            <button onClick={() => setQ("")}
                    style={{ fontSize: 11, color: 'var(--ink-faint)' }}>×</button>
          )}
        </div>
      </div>
...
function Stat2({ label, v }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'baseline', gap: 5 }}>
      ...
      <span style={{ fontSize: 9.5, letterSpacing: '0.14em', color: 'var(--ink-faint)',
  ...
  return (
    <button onClick={onClick}
            style={{ display: 'inline-flex', gap: 8, alignItems: 'center',
                      ...
                      border: on ? '1px solid var(--ink-faint)' : '1px solid transparent',
                      color: on ? 'var(--ink)' : 'var(--ink-soft)',
                      cursor: 'pointer', whiteSpace: 'nowrap' }}>
      <span className="kanji" style={{ fontSize: 13,
                    color: on ? 'var(--accent)' : 'var(--ink-mute)' }}>{mcp.kanji}</span>
      <span>{mcp.name}</span>
      <span className="mono" style={{ fontSize: 10.5,
                    color: on ? 'var(--ink-mute)' : 'var(--ink-faint)' }}>
        {mcp.toolCount}
      </span>
      {!mcp.installed && (
        <span style={{ fontSize: 9.5, color: 'var(--warning)',
...
function KindChip({ kanji, label, on, count, onClick, tone }) {
  const toneColor = tone === "shu"    ? "var(--accent)" :
                    tone === "matcha" ? "var(--matcha)" : "var(--ink-mute)";
  return (
    <button onClick={onClick}
            style={{ display: 'inline-flex', gap: 6, alignItems: 'center',
                      ...
                      border: 'none',
                      color: on ? 'var(--paper)' : 'var(--ink-soft)', cursor: 'pointer' }}>
...
function ToolRowFlat({ tool, active, onClick }) {
  ...
  const kindColor = tool.kind === "action" ? "var(--accent)" : "var(--matcha)";
  return (
    <button onClick={onClick}
    ...
    <div>
      ...
      <div>
        ...
        <pre style={{ margin: 0, padding: '14px 16px',
                       ...
                       borderRadius: 5, color: 'var(--ink-soft)',
...
function InputRow({ input, value, onChange }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      ...
      {control}
      {input.help && (
        <span style={{ fontSize: 10.5, color: 'var(--ink-faint)' }}>{input.help}</span>
      )}
    </div>
  );
}
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '300px 1fr',
        ...
        <aside style={{ overflow: 'auto', borderRight: 'var(--hairline)',
          ...
          <div style={{ padding: '14px 14px 8px',
                         fontSize: 10, letterSpacing: '0.14em', color: 'var(--ink-mute)',
            ...
            return (
              <button key={sid}
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                  <span className="mono" style={{ fontSize: 10.5,
                                color: on ? 'var(--ink)' : 'var(--ink-soft)' }}>{sid}</span>
                  <span style={{ fontSize: 9.5,
                                 color: m.ftr ? 'var(--matcha)' : 'var(--warning)',
                                 letterSpacing: '0.12em', textTransform: 'uppercase' }}>
        ...
        <main style={{ overflow: 'auto', padding: '22px 40px 40px' }}>
          ...
          <div style={{ display: 'grid',
            ...
            <div>
              ...
              <div className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{pickedId}</div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                {sess.calls.map(c => {
                  ...
                  const dot = kindFromName === "action" ? "var(--accent)" : "var(--matcha)";
                  return (
...
const preStyleR = {
  ...
  color: 'var(--ink-soft)', whiteSpace: 'pre-wrap', overflow: 'auto'
};
...
function InstrumentsHealth({ activeTab = "health", onTab = () => {},
  ...
  return (
    <InstrumentsShell activeTab={activeTab} onTab={onTab} embedded={embedded} simple={simple}
      ...
      chip={
        <div style={{ display: 'flex', gap: 2, background: 'var(--paper-soft)',
          ...
          {["7d", "30d", "90d"].map(w => (
            <button key={w} onClick={() => setWindow(w)}
                    style={{ padding: '4px 10px', fontSize: 11, borderRadius: 3,
                              ...
                              color: window_ === w ? 'var(--ink)' : 'var(--ink-mute)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 26 }}>
          ...
          <div>
            ...
            <div style={{ marginTop: 22, padding: '14px 16px',
                           background: 'var(--paper-soft)', border: 'var(--hairline)',
                           borderRadius: 7 }}>
              <div style={{ fontSize: 10.5, letterSpacing: '0.14em', color: 'var(--ink-mute)',
                             textTransform: 'uppercase', marginBottom: 6 }}>
                how insights work
              </div>
  ...
  return (
    <div style={{ padding: '12px 14px', background: 'var(--paper-soft)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
        <span className="kanji" style={{ fontSize: 12, color: 'var(--accent)' }}>{kanji}</span>
        <span style={{ fontSize: 9.5, letterSpacing: '0.14em', color: 'var(--ink-mute)',
                        textTransform: 'uppercase' }}>{label}</span>
      </div>
      <div className="display" style={{ fontSize: 22, color: valueColor,
                    fontFeatureSettings: '"tnum"' }}>{value}</div>
      <div style={{ marginTop: 2, fontSize: 10.5, color: 'var(--ink-mute)' }}>
        {delta && (
          <span style={{ color: deltaTone === "good" ? "var(--matcha)" :
                                deltaTone === "warn" ? "var(--accent)" : "var(--ink-mute)",
                         marginRight: 6 }}>
            {delta}
          </span>
        )}
...
function SignalCard({ s }) {
  const palette = {
    ...
    unused:      { border: "var(--ink-mute)", tintBg: "var(--paper-soft)",         label: "quiet" },
...
function ToolRowHeader() {
  return (
    <div style={{ display: 'grid',
                   ...
                   background: 'var(--paper-soft)', borderBottom: 'var(--hairline)' }}>
      {["tool", "calls", "trend 14d", "usage split", "ftr Δ"].map(h => (
        <div key={h} style={{ fontSize: 9.5, letterSpacing: '0.14em', color: 'var(--ink-mute)',
...
function ToolUsageRow({ t, focus, onFocus, last }) {
  const verdict = {
    ...
    ok:        { color: "var(--ink-mute)",  glyph: "·" },
    warn:      { color: "var(--warning)",   glyph: "!" },
    underused: { color: "var(--ink-mute)",  glyph: "·" },
    unused:    { color: "var(--accent)",     glyph: "○" }
  }[t.verdict] || { color: "var(--ink-mute)", glyph: "·" };
    ...
    <div>
      <button onClick={onFocus}
              style={{ width: '100%', display: 'grid',
                        gridTemplateColumns: '1.8fr 56px 120px 1.2fr 80px',
                        gap: 12, padding: '10px 14px',
                        background: focus ? 'var(--paper-soft)' : 'transparent',
                        border: 'none',
        ...
        <div className="mono" style={{ fontSize: 11.5,
                      color: t.ftrDelta > 0 ? "var(--matcha)" :
                             t.ftrDelta < 0 ? "var(--warning)" : "var(--ink-mute)",
                      fontFeatureSettings: '"tnum"' }}>
  ...
  return (
    <svg width={w} height={h} style={{ display: 'block' }}>
      <polyline points={pts} fill="none"
                stroke="var(--ink-mute)" strokeWidth="1.3"/>
...
function ProjectUsageRow({ p }) {
  return (
    <div style={{ padding: '10px 12px', background: 'var(--paper-soft)',
                   border: 'var(--hairline)', borderRadius: 5,
                   display: 'grid', gridTemplateColumns: '1fr auto',
                   gap: 10, alignItems: 'baseline' }}>
      <div>
        <div style={{ fontSize: 12.5, color: 'var(--ink)' }}>{p.project}</div>
      ...
      <div style={{ textAlign: 'right' }}>
        ...
        <div style={{ fontSize: 9, letterSpacing: '0.14em', color: 'var(--ink-faint)',
