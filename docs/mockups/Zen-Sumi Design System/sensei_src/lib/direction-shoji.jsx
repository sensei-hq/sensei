...
function ShojiTopbar({ page, setPage, solutions, activeSolution, setActiveSolution }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 20, padding: '10px 18px',
      ...
      <div style={{ display: 'flex', gap: 2, padding: 3, background: 'var(--paper-mute)',
                    borderRadius: 8 }}>
        {solutions.map(s => (
          <button key={s.id} onClick={() => setActiveSolution(s.id)}
                  style={{ padding: '5px 12px', borderRadius: 6, fontSize: 12,
                            ...
                            color: activeSolution === s.id ? 'var(--ink)' : 'var(--ink-soft)',
                            boxShadow: activeSolution === s.id ? '0 1px 2px rgba(0,0,0,0.04)' : '',
                            transition: 'all .14s' }}>
            <span className="kanji" style={{ fontSize: 11,
                          color: activeSolution === s.id ? 'var(--accent)' : 'var(--ink-mute)' }}>{s.kanji}</span>
            {s.name}
          </button>
        ))}
      </div>
        ...
        {PAGES.map(p => (
          <button key={p.id} onClick={() => setPage(p.id)}
                  style={{ padding: '8px 12px', fontSize: 12,
                            color: page === p.id ? 'var(--ink)' : 'var(--ink-mute)',
    ...
    <div style={{ padding: 18, display: 'grid', gap: 12, height: '100%',
      ...
      <Pane title="First Try Right" kanji="一" span={{ col: 'span 5', row: 'span 2' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 14 }}>
          ...
          <span style={{ fontSize: 22, color: 'var(--ink-mute)' }}>%</span>
          <span className="mono" style={{ marginLeft: 'auto', fontSize: 12,
                        color: delta >= 0 ? 'var(--success)' : 'var(--accent)' }}>
            {delta >= 0 ? '↗ +' : '↘ '}{delta}% / w
          </span>
        </div>
        <div style={{ marginTop: 14, color: delta >= 0 ? 'var(--success)' : 'var(--accent)' }}>
        ...
        <div className="mono" style={{ display: 'flex', justifyContent: 'space-between',
                      fontSize: 10, color: 'var(--ink-mute)', marginTop: 6 }}>
          <span>14d ago</span>
          <span>7d ago</span>
          <span>today</span>
        </div>
      ...
      <Pane title="Sensei says" kanji="師" span={{ col: 'span 4', row: 'span 2' }}
        ...
        <p style={{ fontSize: 12, color: 'var(--ink-soft)', lineHeight: 1.55, marginBottom: 18 }}>
          {topCoach.body}
        </p>
      ...
      <Pane title="Quality signals" kanji="質" span={{ col: 'span 6', row: 'span 1' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
          {[
            ...
            <div key={s.k}>
              <div style={{ fontSize: 10, color: 'var(--ink-mute)', letterSpacing: '0.05em' }}>{s.k}</div>
      ...
      <Pane title="Recent sessions" kanji="刻" span={{ col: 'span 8', row: 'span 2' }}
            action={<button onClick={() => setPage("sessions")}
                            style={{ fontSize: 11, color: 'var(--ink-mute)' }}>all →</button>}>
        <div>
          ...
          {solSessions.map(s => (
            <button key={s.id} onClick={() => { setFocusedSession(s.id); setPage("sessions"); }}
              ...
              <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>{s.id}</span>
              <span style={{ fontSize: 12, color: 'var(--ink)' }}>{s.title}</span>
              <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>{s.module}</span>
              <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>{s.turns}</span>
              <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>{s.duration}</span>
            </button>
          ))}
        </div>
      </Pane>
      ...
      <Pane title="Hotspots" kanji="熱" span={{ col: 'span 4', row: 'span 2' }}>
        {data.hotspots.map((h, i) => (
          <div key={i} style={{ padding: '8px 0', borderBottom: i < data.hotspots.length-1 ? 'var(--hairline)' : 'none' }}>
            ...
            <div className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)', paddingLeft: 14 }}>
              in {h.fanIn} · out {h.fanOut} · <span style={{ color: h.rework > 3 ? 'var(--accent)' : 'var(--ink-mute)' }}>↻ {h.rework}</span>
            </div>
          </div>
        ))}
      </Pane>
...
function Pane({ title, kanji, span, children, action, accent }) {
  return (
    <section style={{
      ...
      background: accent ? 'var(--paper-soft)' : 'var(--paper)',
      ...
      <header style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
        <span className="kanji" style={{ fontSize: 11, color: accent ? 'var(--accent)' : 'var(--ink-mute)' }}>{kanji}</span>
  ...
  return (
    <div style={{ padding: 22, display: 'grid', gap: 12,
      ...
      <section style={{ gridColumn: 'span 3', border: 'var(--hairline)', borderRadius: 10,
                        padding: 22, background: 'var(--paper-soft)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 36 }}>
          <div>
            <div className="display" style={{ fontSize: 56, fontWeight: 300, lineHeight: 1 }}>
              78<span style={{ fontSize: 20, color: 'var(--ink-mute)' }}>%</span>
            </div>
            <div className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)', marginTop: 4 }}>
              first try right · 14d
            </div>
      ...
      {data.solutions.map(s => (
        <button key={s.id}
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            <span className="kanji" style={{ fontSize: 14, color: 'var(--accent)' }}>{s.kanji}</span>
            ...
            {s.warning && <span className="mono" style={{ fontSize: 9.5, marginLeft: 'auto',
                          padding: '2px 7px', borderRadius: 4, background: 'var(--accent-soft)',
                          color: 'var(--accent)' }}>attention</span>}
          </div>
          <div style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{s.description}</div>
  ...
  return (
    <div style={{ padding: 22 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 20, marginBottom: 18 }}>
        ...
        <div style={{ flex: 1 }}/>
        <div style={{ display: 'flex', gap: 3, padding: 3, background: 'var(--paper-mute)', borderRadius: 6 }}>
      ...
      <div style={{ border: 'var(--hairline)', borderRadius: 10, overflow: 'hidden' }}>
        <div className="mono" style={{ display: 'grid',
                      ...
                      borderBottom: 'var(--hairline)', background: 'var(--paper-mute)' }}>
          ...
          <React.Fragment key={s.id}>
            <button onClick={() => setFocused(focused === s.id ? null : s.id)}
                    style={{ display: 'grid',
                              ...
                              borderBottom: 'var(--hairline)', width: '100%', textAlign: 'left',
                              background: focused === s.id ? 'var(--paper-mute)' : 'transparent',
                              transition: 'background .12s' }}>
              <span className="ink-dot" style={{ background: s.ftr ? 'var(--success)' : 'var(--accent)' }}/>
              <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{s.id}</span>
              <span style={{ fontSize: 12.5, color: 'var(--ink)' }}>{s.title}</span>
              <span className="mono" style={{ fontSize: 11, color: 'var(--ink-soft)' }}>{s.module}</span>
              <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{s.date} {s.started.split(' ').pop()}</span>
              <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)', textAlign: 'right' }}>{s.turns}</span>
              <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)', textAlign: 'right' }}>{(s.tokens/1000).toFixed(1)}k</span>
              <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)', textAlign: 'right' }}>{s.duration}</span>
            </button>
            {focused === s.id && (
              <div style={{ padding: '16px 18px 22px 102px', background: 'var(--paper-soft)',
                    ...
                    {s.events.map((e, i) => (
                      <div key={i} style={{ display: 'grid', gridTemplateColumns: '14px 50px 1fr',
                        ...
                        <span style={{ color: e.kind === 'correction' ? 'var(--accent)' :
                                              e.kind === 'test' ? 'var(--success)' : 'var(--ink-mute)' }}>
                          <EventGlyph kind={e.kind} size={12}/>
                        </span>
                        <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>{e.t}</span>
                        <span style={{ fontSize: 12, color: e.kind === 'correction' ? 'var(--accent)' : 'var(--ink-soft)' }}>
                          {e.text}
                        </span>
                      </div>
                    ))}
    ...
    <div style={{ padding: 22, display: 'grid', gap: 12,
          ...
          {sol.repos.map(r => (
            <button key={r} onClick={() => setRepo(r)}
                    ...
                    style={{ padding: '4px 10px', fontSize: 11, borderRadius: 4,
                              ...
                              color: repo === r ? 'var(--ink)' : 'var(--ink-soft)' }}>
              {r}
            </button>
          ))}
        </div>
        <div style={{ flex: 1 }}/>
        <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>247 nodes · 4 communities · indexed 2m ago</span>
      </section>
      ...
      <Pane title="Graph" kanji="網" span={{ col: 'span 8', row: 'span 1' }}>
        <svg viewBox="0 0 600 280" width="100%" height="280">
          ...
          {/* edges */}
          <g stroke="var(--ink-faint)" strokeWidth="0.6" opacity="0.6">
          ...
          {Array.from({length: 70}).map((_, i) => {
            ...
            return <circle key={i} cx={x} cy={y} r={1.5 + r * 1.5} fill="var(--ink-soft)" opacity="0.6"/>;
          })}
          {/* god nodes */}
          ...
          <text x="120" y="60" fontSize="10" fill="var(--ink-mute)" fontFamily="var(--font-mono)">core</text>
          <text x="400" y="65" fontSize="10" fill="var(--ink-mute)" fontFamily="var(--font-mono)">api</text>
          <text x="220" y="255" fontSize="10" fill="var(--ink-mute)" fontFamily="var(--font-mono)">ui</text>
        </svg>
      </Pane>
      ...
      <Pane title="Communities" kanji="群" span={{ col: 'span 4', row: 'span 1' }}>
        {[
          ...
          { name: "utils", nodes: 8,  color: 'var(--ink-mute)' },
        ].map((c, i) => (
          <div key={c.name} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0',
            ...
            <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{c.nodes} nodes</span>
          </div>
        ))}
      </Pane>
      ...
      <Pane title="Hotspots" kanji="熱" span={{ col: 'span 8', row: 'span 1' }}>
        ...
        {data.hotspots.map((h, i) => (
          <div key={i} style={{ display: 'grid',
            ...
            <span className="mono" style={{ fontSize: 12, color: 'var(--ink)' }}>{h.name}</span>
            <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)', textAlign: 'right' }}>{h.fanIn}</span>
            <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)', textAlign: 'right' }}>{h.fanOut}</span>
            <span className="mono" style={{ fontSize: 11, color: h.rework > 3 ? 'var(--accent)' : 'var(--ink-mute)', textAlign: 'right' }}>
              ↻ {h.rework}
            </span>
          </div>
        ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {[
            ...
            <div key={r.l} style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontSize: 12, color: 'var(--ink-mute)' }}>{r.l}</span>
    ...
    <div style={{ padding: 22, display: 'grid', gap: 12,
      ...
      {data.coaching.map((c) => {
        ...
        const urgencyColor = c.urgency === 'high' ? 'var(--accent)' :
                             c.urgency === 'medium' ? 'var(--warning)' : 'var(--ink-mute)';
        return (
          <section key={c.id} style={{ gridColumn: 'span 4', border: 'var(--hairline)',
            ...
            <p style={{ fontSize: 12, color: 'var(--ink-soft)', lineHeight: 1.55, margin: 0, flex: 1 }}>
              {c.body}
            </p>
            <hr className="hairline"/>
    ...
    <div style={{ padding: 22, display: 'grid', gap: 12,
      ...
      <Pane title="Skills" kanji="技" span={{ col: 'span 7', row: 'span 1' }}>
        {data.skills.map((s, i) => (
          <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '14px 1fr auto auto',
            ...
            <span className="ink-dot" style={{ background: s.active ? 'var(--success)' : 'var(--ink-faint)' }}/>
            <div>
            ...
            <span className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)' }}>{s.id}</span>
            <div style={{ width: 28, height: 16, borderRadius: 999,
                          background: s.active ? 'var(--accent)' : 'var(--paper-mute)',
                          position: 'relative' }}>
      ...
      <Pane title="Libraries" kanji="書" span={{ col: 'span 5', row: 'span 1' }}>
        {data.libraries.map((l, i) => (
          <div key={l.name} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto',
                        gap: 14, padding: '9px 0', alignItems: 'center',
                        borderBottom: 'var(--hairline)' }}>
            <div style={{ fontSize: 12.5 }}>{l.name} <span className="mono" style={{ color: 'var(--ink-mute)', fontSize: 10 }}>v{l.version}</span></div>
            <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>{l.pages}p</span>
            <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>{l.lastIndexed}</span>
          </div>
        ))}
        <button style={{ marginTop: 12, padding: '7px 12px', borderRadius: 6, fontSize: 11,
                      border: '1px dashed var(--ink-mute)', color: 'var(--ink-soft)' }}>
          + Index a library
        </button>
      </Pane>
      ...
      <Pane title="Assistants (ACPs)" kanji="助" span={{ col: 'span 7', row: 'span 1' }}>
        {[
          ...
          <div key={a.name} style={{ display: 'grid', gridTemplateColumns: '14px 1fr auto auto',
            ...
            <span className="ink-dot" style={{ background: a.status === 'connected' ? 'var(--success)' : 'var(--ink-faint)' }}/>
            <div>
              <div style={{ fontSize: 12.5 }}>{a.name}</div>
              <div className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)' }}>MCP · v{a.version} · {a.status}</div>
            </div>
            <span className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)' }}>{a.status}</span>
            <button style={{ fontSize: 11, padding: '4px 10px', borderRadius: 5,
                          border: 'var(--hairline)', color: 'var(--ink-soft)' }}>
      ...
      <Pane title="Daemon" kanji="守" span={{ col: 'span 5', row: 'span 1' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
          ...
          <span style={{ flex: 1 }}/>
          <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>pid 12492</span>
        </div>
        {[
          ["Port",           "9823"],
          ["Uptime",         "4d 2h"],
          ...
          <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0' }}>
            <span style={{ fontSize: 12, color: 'var(--ink-mute)' }}>{l}</span>
  ...
  return (
    <div style={{ padding: 40, display: 'grid', gap: 20,
      ...
      <section style={{ gridColumn: 'span 4' }}>
        <div className="kanji" style={{ fontSize: 36, color: 'var(--accent)' }}>始</div>
        <h1 className="display" style={{ fontSize: 28, fontWeight: 300, margin: '10px 0 4px' }}>Begin.</h1>
        <div style={{ fontSize: 13, color: 'var(--ink-mute)' }}>Four panes, four strokes.</div>
      </section>
      ...
      {steps.map(s => (
        <section key={s.n}
                 onClick={() => setStep(s.n)}
                 style={{ border: 'var(--hairline)', borderRadius: 10, padding: 22,
                          background: s.n === step ? 'var(--paper-soft)' : 'var(--paper)',
          ...
          <div className="display" style={{ fontSize: 48, fontWeight: 300,
                        color: s.n < step ? 'var(--success)' : s.n === step ? 'var(--accent)' : 'var(--ink-mute)',
                        lineHeight: 1 }}>
            {s.n < step ? '✓' : '0' + s.n}
          </div>
          <div className="display" style={{ fontSize: 17, fontWeight: 500, marginTop: 14 }}>{s.t}</div>
          <div style={{ fontSize: 11.5, color: 'var(--ink-mute)', marginTop: 4 }}>{s.d}</div>
        </section>
      ))}

      <section style={{ gridColumn: 'span 4', display: 'flex', gap: 10, marginTop: 6 }}>
        <button onClick={() => setStep(Math.min(4, step+1))}
                style={{ padding: '10px 20px', background: 'var(--ink)', color: 'var(--paper)',
                          borderRadius: 6, fontSize: 12 }}>
          {step === 4 ? "Enter observatory →" : "Continue"}
        </button>
        <button style={{ padding: '10px 16px', color: 'var(--ink-mute)', fontSize: 12 }}>Skip</button>
      </section>
    </div>
  );
}
