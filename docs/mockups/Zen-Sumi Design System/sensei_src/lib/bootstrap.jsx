      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr', minHeight: 0,
        ...
        <div style={{ maxWidth: 760, width: '100%', margin: '0 auto',
          ...
          <div>
            ...
            <p style={{ fontSize: 14, color: 'var(--ink-mute)', lineHeight: 1.7, margin: 0,
                         maxWidth: 540 }}>
              {allReady
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase',
                           color: 'var(--ink-faint)', fontFeatureSettings: '"tnum"' }}>
              {String(readyCount).padStart(2, "0")} <span style={{ color: 'var(--ink-faint)' }}>/ {String(BOOT_GATES.length).padStart(2, "0")} ready</span>
            </div>
            <div style={{ flex: 1, display: 'flex', gap: 3 }}>
              {BOOT_GATES.map((g, i) => {
                const s = statuses[g.id];
                const color = s === "ready" ? 'var(--matcha)'
                            : s === "checking" || s === "starting" ? 'var(--ink-soft)'
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '32px 1fr auto', gap: 16,
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--ink-faint)' }}>· {gate.detail}</div>
          </div>
...
function StatusPill({ status }) {
  const map = {
    ...
    checking: { label: "checking",  color: "var(--ink-soft)", bg: "var(--paper-soft)" },
    starting: { label: "starting",  color: "var(--ink-soft)", bg: "var(--paper-soft)" },
    missing:  { label: "missing",   color: "var(--accent)",    bg: "rgba(192,71,45,.08)" },
    error:    { label: "blocked",   color: "var(--accent)",    bg: "rgba(192,71,45,.08)" },
    pending:  { label: "waiting",   color: "var(--ink-faint)", bg: "transparent" },
  };
      ...
      <RemedyShell title="Install Homebrew" intro="Homebrew is the base that installs everything else. Run the command from the official installer, then return here and retry.">
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginTop: 12 }}>
          <a href="https://brew.sh" target="_blank" rel="noreferrer"
            ...
            Open brew.sh <span style={{ color: 'var(--ink-mute)' }}>↗</span>
          </a>
  ...
  if (gate.remedy === "brew") {
    return (
      <RemedyShell
        ...
        <div style={{ fontSize: 11, color: 'var(--ink-faint)', lineHeight: 1.6, marginTop: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginTop: 12 }}>
          <a href="https://github.com/sensei-dev/sensei" target="_blank" rel="noreferrer"
             style={{ fontSize: 12, color: 'var(--ink)', textDecoration: 'none',
                       padding: '8px 14px', border: 'var(--hairline)', borderRadius: 5,
                       display: 'inline-flex', alignItems: 'center', gap: 6 }}>
            View Brewfile on GitHub <span style={{ color: 'var(--ink-mute)' }}>↗</span>
          </a>
          <button onClick={onRetry}
                  style={{ fontSize: 12, background: 'var(--ink)', color: 'var(--paper)',
                           padding: '8px 16px', borderRadius: 5, border: 'none', cursor: 'pointer' }}>
    ...
    return (
      <RemedyShell
        ...
        <div style={{ fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase',
                       color: 'var(--ink-faint)', marginBottom: 6 }}>Manual create</div>
        <CommandBlock cmd="createdb sensei && psql sensei -c 'CREATE EXTENSION IF NOT EXISTS vector;'"/>

        <div style={{ fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase',
                       color: 'var(--ink-faint)', marginTop: 18, marginBottom: 6 }}>Database URL</div>
    ...
    return (
      <RemedyShell title="Daemon failed to start"
        ...
        <div style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
          ...
          <div><span style={{ color: 'var(--ink-faint)' }}>10:42:18</span> daemon · starting</div>
          <div><span style={{ color: 'var(--ink-faint)' }}>10:42:18</span> daemon · loading config ~/.sensei/config.toml</div>
          <div><span style={{ color: 'var(--ink-faint)' }}>10:42:19</span> daemon · connecting to postgres</div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
          ...
          <button style={{ fontSize: 12, color: 'var(--ink-soft)',
                           padding: '8px 14px', border: 'var(--hairline)', borderRadius: 5,
...
function RemedyShell({ title, intro, children }) {
  return (
    <div style={{ marginTop: 16, marginLeft: 48, padding: '18px 20px',
      ...
      <div style={{ fontSize: 12.5, color: 'var(--ink-mute)', lineHeight: 1.65, marginBottom: 14,
                     maxWidth: 580 }}>
        {intro}
      </div>
      {children}
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 10,
      ...
      <button onClick={doCopy}
              style={{ fontSize: 10.5, letterSpacing: '0.1em', textTransform: 'uppercase',
                       color: copied ? 'var(--matcha)' : 'var(--ink-mute)',
  ...
  return (
    <div style={{ height: '100%', position: 'relative' }}>
      ...
      <div style={{ position: 'absolute', top: 52, right: 16, zIndex: 5,
        ...
        <div style={{ fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase',
                       color: 'var(--ink-faint)', marginBottom: 8 }}>demo · scenario</div>
          ...
          {Object.keys(BOOT_PRESETS).map(k => (
            <button key={k} onClick={() => setScenario(k)}
                    style={{ textAlign: 'left', fontSize: 11.5,
                             ...
                             background: scenario === k ? 'var(--paper-soft)' : 'transparent',
                             color: scenario === k ? 'var(--ink)' : 'var(--ink-mute)',
                             cursor: 'pointer' }}>
              {k.replace(/-/g, ' ')}
            </button>
          ))}
