...
const QUALITY = {
  good: { color: 'var(--success)',     label: "first-try"    },
  bad:  { color: 'var(--warning)',    label: "corrected"    },
  ugly: { color: 'var(--accent)',      label: "abandoned"    }
};
    ...
    <div ref={scrollRef}
      ...
      <section style={{ padding: '24px 40px 56px', minHeight: 1100 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 14,
          ...
          <span className="kanji" style={{ fontSize: 16, color: 'var(--accent)' }}>省</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
          ...
          <RetroLane title="Insights"       accent="var(--ink-soft)" items={retro.insights}/>
        </div>
      </section>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
      ...
      <div style={{ minWidth: 56 }}>
        <div style={{ fontSize: 18, lineHeight: 1, fontWeight: 300, color: 'var(--ink)',
...
function MiniCycler({ mode, onCycle }) {
  return (
    <button onClick={onCycle} title={`viewing ${mode} · click to cycle`}
      ...
      <span className="kanji" style={{ fontSize: 14, color: 'var(--accent)',
                                         lineHeight: 1 }}>
        {MINI_GLYPH[mode]}
      </span>
    </button>
...
function ZenMiniRetro({ going, notGoing, insights }) {
  const dots = [
    { c: 'var(--success)',  n: going,    l: "going well" },
    { c: 'var(--accent)',   n: notGoing, l: "not going" },
    { c: 'var(--ink-soft)', n: insights, l: "insights" }
  ];
  return (
      ...
      <div style={{ display: 'flex', gap: 18 }}>
        {dots.map(d => (
          <div key={d.l} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            ...
            <span style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>{d.l}</span>
          </div>
        ))}
      </div>
    </div>
...
function ZenHero({ totals, range, setRange,
  ...
  return (
    <div style={{ padding: '22px 40px 16px',
      ...
      <div className="kanji"
           style={{ fontSize: 42, color: 'var(--accent)', lineHeight: 1 }}>録</div>
  ...
  return (
    <div style={{ textAlign: 'center', minWidth: 56 }}>
      <div className={mono ? "mono" : ""}
           style={{ fontSize: 18, lineHeight: 1, fontWeight: 300, color: 'var(--ink)',
                     fontFeatureSettings: '"tnum"',
...
function RetroCard({ item, accent, positive }) {
  return (
    <article style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
      ...
      <div style={{ fontSize: 11.5, color: 'var(--ink-soft)', lineHeight: 1.55 }}>{item.body}</div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 8,
                     paddingTop: 8, borderTop: '1px dashed var(--paper-edge)' }}>
        {item.delta && (
          <span className="mono" style={{ fontSize: 10.5,
                        color: positive ? 'var(--success)' :
                               item.delta.startsWith('−') ? 'var(--accent)' :
                               item.tone === "positive" ? 'var(--success)' : 'var(--ink-soft)' }}>
            {item.delta}
          </span>
        )}
        ...
        <span style={{ flex: 1 }}/>
        {item.action && (
          <button style={{ fontSize: 10.5, color: 'var(--accent)' }}>{item.action} →</button>
        )}
      </div>
    </article>
  );
...
function ZenChartFrame({ chart, setChart, children }) {
  ...
  return (
    <section>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 14,
                     paddingBottom: 8, borderBottom: 'var(--hairline)' }}>
        <span className="kanji" style={{ fontSize: 16, color: 'var(--accent)' }}>{m.kanji}</span>
        <h3 className="display" style={{ fontSize: 16, fontWeight: 400, margin: 0,
                      color: 'var(--ink)' }}>Sessions · {m.label}</h3>
        <span style={{ fontSize: 11, color: 'var(--ink-mute)' }}>· {m.sub}</span>
        <span style={{ flex: 1 }}/>

        {/* Tab switch */}
        <div style={{ display: 'flex', gap: 0, padding: 2, background: 'var(--paper-soft)',
          ...
          {["trend","stream","constellation","bands","pulse"].map(c => (
            <button key={c} onClick={() => setChart(c)}
                    style={{ padding: '4px 11px', fontSize: 10.5,
                              ...
                              color: chart === c ? 'var(--ink)' : 'var(--ink-mute)',
...
function ChartLegend() {
  return (
    <div style={{ display: 'flex', gap: 18, marginTop: 12, fontSize: 10.5,
                   color: 'var(--ink-mute)', justifyContent: 'flex-end' }}>
...
function RangeFilter({ value, onChange }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <span style={{ fontSize: 10, color: 'var(--ink-faint)', letterSpacing: '0.14em',
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}>
      ...
      {[0, 0.5, 1].map(t => (
        <g key={t}>
          ...
          <text x={padL - 8} y={yFtr(t) + 3} fontSize={10}
                fill="var(--ink-faint)" textAnchor="end" fontFamily="JetBrains Mono">
            {Math.round(t * 100)}%
          </text>
        </g>
      ))}
      <text x={padL - 8} y={padT - 10} fontSize={9.5} fill="var(--ink-faint)"
        ...
        return (
          <g key={i}>
            ...
            <text x={(x1 + x2) / 2} y={y0 - 5} fontSize={10}
                  fill="var(--accent)" textAnchor="middle" fontFamily="JetBrains Mono">
      ...
      <path d={linePath(trendPts)} fill="none"
            stroke="var(--ink-mute)" strokeWidth={1} opacity={0.4}/>

      {/* Rolling-7 avg — the quiet narrative line */}
      <path d={linePath(rollingPts)} fill="none"
            stroke="var(--success)" strokeWidth={2.2}/>
      ...
      {points.map((p, i) => {
        ...
        return (
          <circle key={p.day} cx={x(i)} cy={yFtr(p.ftr)} r={3.5}
                  ...
                  stroke="var(--paper-soft)" strokeWidth={1}/>
        );
      })}
        ...
        return (
          <g key={c.id}>
            <line x1={cx} x2={cx} y1={padT} y2={H - padB}
                  stroke="var(--ink)" strokeWidth={1} strokeDasharray="3 3" opacity={0.55}/>
            <circle cx={cx} cy={padT - 12} r={11} fill="var(--paper)"
                    stroke="var(--ink)" strokeWidth={1}/>
            <text x={cx} y={padT - 8} fontSize={12} fill="var(--accent)"
      ...
      {points.map((p, i) => {
        ...
        return (
          <text key={p.day} x={x(i)} y={H - 10} fontSize={10}
                fill="var(--ink-mute)" textAnchor="middle" fontFamily="Inter">
            {DAYS_SHORT(p.day)}
          </text>
        );
      })}
      ...
      {/* Caption */}
      <text x={padL} y={padT - 18} fontSize={10} fill="var(--ink-faint)"
            fontFamily="Inter">
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}>
        ...
        return (
          <g key={c.id}>
            ...
            <circle cx={cx} cy={padT - 8} r={9} fill="var(--paper)"
                    stroke="var(--ink)" strokeWidth={1}/>
            <text x={cx} y={padT - 5} fontSize={11} fill="var(--accent)"
                  textAnchor="middle" fontFamily="'Noto Sans JP', sans-serif">
              {c.kanji}
            </text>
          </g>
      ...
      <line x1={x(days.length-1)} x2={x(days.length-1)}
            y1={padT} y2={H - padB}
            stroke="var(--accent)" strokeWidth={1} opacity={0.4} strokeDasharray="2 3"/>

      {/* Caption */}
      <text x={padL} y={padT + 3} fontSize={10} fill="var(--ink-faint)"
            fontFamily="Inter">
        time spent · stacked
      </text>
    </svg>
...
function ConstellationChart({ sessions, days = DAYS_ORDERED }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}>
      ...
      {dayList.map((d, i) => (
        <g key={d}>
          ...
          <text x={padL - 10} y={y(i) + 3} fontSize={10.5}
                fill="var(--ink-mute)" textAnchor="end" fontFamily="Inter">
            {DAYS_SHORT(d)}
          </text>
        </g>
      ))}
      ...
      {xTicks.map(t => (
        <g key={t}>
          ...
          <text x={x(t)} y={H - padB + 16} fontSize={10}
                fill="var(--ink-faint)" textAnchor="middle" fontFamily="JetBrains Mono">
      ...
      {/* Caption */}
      <text x={padL} y={14} fontSize={10} fill="var(--ink-faint)"
            fontFamily="Inter">
...
function BandsChart({ sessions, days = DAYS_ORDERED }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}>
      {/* Caption */}
      <text x={padL} y={14} fontSize={10} fill="var(--ink-faint)" fontFamily="Inter">
        ...
        return (
          <g key={r.day}>
            ...
            <rect x={padL} y={top} width={innerW} height={h}
                  fill="var(--paper-mute)" rx={3} opacity={0.5}/>
            {/* Segments */}
            {["good","bad","ugly"].map(k => {
            ...
            <text x={padL + innerW + 10} y={cy + 3} fontSize={10}
                  fill="var(--ink-mute)" fontFamily="JetBrains Mono">
    ...
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}>
      ...
      {days.map((d, i) => (
        <text key={d}
              ...
              y={padT + innerH + 18}
              fontSize={10.5} fill="var(--ink-mute)"
      ...
      {[30, 60, 120].map(m => (
        <text key={m} x={W - padR + 4}
              ...
              fontSize={9} fill="var(--ink-faint)" fontFamily="JetBrains Mono">
          {m < 60 ? `${m}m` : `${m/60}h`}
        </text>
      ))}
      ...
      {/* Caption */}
      <text x={padL} y={18} fontSize={10} fill="var(--ink-faint)" fontFamily="Inter">
            ...
            {MINI_MODES.map(m => (
              <button key={m} onClick={() => setMiniMode(m)}
                      style={{ padding: '3px 10px', fontSize: 10.5,
                                background: miniMode === m ? 'var(--paper-soft)' : 'transparent',
                                border: 'var(--hairline)', borderRadius: 4,
                                cursor: 'pointer',
                                color: miniMode === m ? 'var(--ink)' : 'var(--ink-mute)',
                                display: 'inline-flex', alignItems: 'center', gap: 6 }}>
          ...
          <Frame>
            ...
            <div style={{ padding: '20px 28px 24px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
                ...
                <RetroLane title="Insights"       accent="var(--ink-soft)" items={D.retro.insights}/>
              </div>
            </div>
          </Frame>
        </div>
