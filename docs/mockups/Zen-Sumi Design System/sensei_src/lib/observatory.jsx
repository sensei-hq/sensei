...
function ObsFtrStrip({ data, value, delta, dim = false }) {
  ...
  const baseColor = dim ? 'var(--ink-mute)' : 'var(--accent)';
  const bgColor   = dim ? 'var(--ink-faint)' : 'var(--ink-5)';
  return (
    <svg width={w} height={h + 14} style={{ display: 'block', overflow: 'visible' }}>
      ...
      <line x1="0" x2={w} y1={h - h*0.5} y2={h - h*0.5}
            stroke="var(--ink-5)" strokeDasharray="2 3"/>
      {data.map((v, i) => {
      ...
      <text x={0} y={h + 11} fontSize="9" fill="var(--ink-mute)"
             fontFamily="var(--font-ui)" letterSpacing="0.08em">14d ago</text>
      <text x={w} y={h + 11} fontSize="9" fill="var(--ink-mute)" textAnchor="end"
...
function ObsSidebar({ section, setSection, activeProjectId, onOpenProject, mode, setMode }) {
  ...
  const NavItem = ({ id, kanji, label, badge }) => (
    <button onClick={() => setSection(id)}
            style={{
              ...
              background: section === id ? 'var(--paper-mute)' : 'transparent',
              color: section === id ? 'var(--ink)' : 'var(--ink-soft)',
              fontSize: 13
            }}>
      <span className="kanji" style={{ fontSize: 13, width: 14,
                    color: section === id ? 'var(--accent)' : 'var(--ink-mute)' }}>{kanji}</span>
      <span>{label}</span>
      {badge != null && (
        <span className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)' }}>{badge}</span>
      )}
    </button>
  );
  ...
  return (
    <aside style={{ borderRight: 'var(--hairline)', padding: '22px 14px',
      ...
      <div>
        <div style={{ fontSize: 9.5, letterSpacing: '0.16em', color: 'var(--ink-mute)',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          ...
          <div style={{ padding: '2px 10px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 10,
              ...
              <span className="kanji" style={{ fontSize: 13, width: 14,
                            color: 'var(--ink-mute)' }}>覚</span>
              <span>Memories</span>
              <span className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)' }}>24</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 1,
                           paddingLeft: 24 }}>
          ...
          <div style={{ padding: '2px 10px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 10,
                           ...
                           color: 'var(--ink-soft)', fontSize: 13 }}>
              <span className="kanji" style={{ fontSize: 13, width: 14,
                            color: 'var(--ink-mute)' }}>具</span>
              <span>Instruments</span>
              <span className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)' }}>7</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 1,
                           paddingLeft: 24 }}>
          ...
          {D.projects.active.map(p => {
            ...
            return (
              <button key={p.id} onClick={() => onOpenProject && onOpenProject(p.id)}
                ...
                color: on ? 'var(--ink)' : 'var(--ink-soft)', fontSize: 12.5,
                cursor: 'pointer'
              }}>
                <span className="kanji" style={{ fontSize: 13, width: 14,
                ...
                <span className="mono" style={{ fontSize: 10,
                              color: p.warn ? 'var(--warning)' : 'var(--ink-mute)' }}>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          {D.projects.recent.map(p => {
            ...
            return (
              <button key={p.id} onClick={() => onOpenProject && onOpenProject(p.id)}
                ...
                padding: '7px 10px', borderRadius: 6, textAlign: 'left',
                background: on ? 'var(--paper-mute)' : 'transparent',
                color: on ? 'var(--ink-soft)' : 'var(--ink-mute)', fontSize: 12.5,
          ...
          <button style={{
            padding: '6px 10px', fontSize: 11, color: 'var(--ink-faint)', textAlign: 'left'
          }}>
      ...
      <div style={{ padding: '10px 10px 0', borderTop: 'var(--hairline)',
        ...
        <span style={{ color: 'var(--ink-faint)' }}>last heartbeat 2s ago</span>
      </div>
    </aside>
  );
}
    ...
    <div style={{ padding: '36px 48px 48px', maxWidth: 1060, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between',
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 22,
                       color: mode === "early" ? 'var(--ink-mute)' : 'var(--ink-soft)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 8,
              ...
              <span className="display"
                     style={{ fontSize: 34, fontWeight: 400, lineHeight: 1,
                               color: mode === "early" ? 'var(--ink-mute)' : 'var(--ink)' }}>
...
function ObsHero({ hero, mode }) {
  return (
    <div style={{
      ...
      background: 'var(--paper-soft)', border: 'var(--hairline)', borderRadius: 12,
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div style={{
          ...
          fontSize: 9, letterSpacing: '0.18em', color: 'var(--ink-mute)',
      ...
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        <div className="display" style={{
          ...
          lineHeight: 1.2, marginBottom: 14, color: 'var(--ink)'
        }}>
          {hero.koan}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap',
          ...
          {hero.action && (
            <button style={{
              padding: '9px 18px', fontSize: 13, background: 'var(--ink)',
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between',
        ...
        <button style={{ fontSize: 11, color: 'var(--ink-mute)' }}>all insights →</button>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        {items.map((x, i) => {
          const toneColor =
            x.tone === "warn" ? 'var(--warning)' :
            x.tone === "good" ? 'var(--success)'  : 'var(--ink-mute)';
          return (
            <button key={i} style={{
        ...
        {items.length === 0 && (
          <div style={{ padding: 20, fontSize: 13, color: 'var(--ink-faint)', fontStyle: 'italic',
                         textAlign: 'center',
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between',
        ...
        <button style={{ fontSize: 11, color: 'var(--ink-mute)' }}>all teachings →</button>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {items.length === 0 && (
          <div style={{ padding: '24px 18px',
                         border: '1px dashed var(--paper-edge)', borderRadius: 8,
                         textAlign: 'center' }}>
            <div className="kanji" style={{ fontSize: 28, color: 'var(--ink-faint)',
  ...
  return (
    <div style={{ marginTop: 40 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between',
        ...
        <button style={{ fontSize: 11, color: 'var(--ink-mute)' }}>all sessions →</button>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        {sessions.map(s => (
...
function ObsPlaceholder({ section, onBack }) {
  ...
  return (
    <div style={{ padding: 60, textAlign: 'center', maxWidth: 520, margin: '0 auto' }}>
      ...
      <p style={{ fontSize: 13, color: 'var(--ink-mute)', margin: '0 0 24px' }}>{L.s}</p>
...
function FirstEntryToast({ onDismiss, mode }) {
  ...
  return (
    <div style={{
      ...
      <button onClick={onDismiss} style={{ fontSize: 11, color: 'var(--ink-faint)',
