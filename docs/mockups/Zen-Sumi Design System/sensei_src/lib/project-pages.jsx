          ...
          return (
            <button key={s.id} onClick={() => setSec(s.id)}
              ...
              <span className="kanji" style={{ fontSize: 12,
                            color: on ? 'var(--accent)' : 'var(--ink-faint)' }}>{s.kanji}</span>
              {s.label}
            </button>
          );
        })}
    ...
    <div className="sensei" data-screen-label="Project · Left rail"
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '180px 1fr', minHeight: 0 }}>
        <aside style={{ borderRight: 'var(--hairline)', padding: '22px 12px',
                         background: 'var(--paper-soft)', display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            return (
              <button key={s.id} onClick={() => setSec(s.id)}
                      style={{
                        ...
                        color: on ? 'var(--ink)' : 'var(--ink-soft)', fontSize: 13
                      }}>
                <span className="kanji" style={{ fontSize: 13, width: 14,
                              color: on ? 'var(--accent)' : 'var(--ink-mute)' }}>{s.kanji}</span>
                <span>{s.label}</span>
              </button>
            );
          })}
          ...
          <button style={{ padding: '8px 10px', fontSize: 12, textAlign: 'left',
                            color: 'var(--ink-soft)' }}>◌ open in terminal</button>
          <button style={{ padding: '8px 10px', fontSize: 12, textAlign: 'left',
                            color: 'var(--ink-soft)' }}>◌ start session</button>
          <button style={{ padding: '8px 10px', fontSize: 12, textAlign: 'left',
                            color: 'var(--ink-soft)' }}>◌ scan now</button>
        </aside>
          ...
          {[
            ...
            <button key={s.id} onClick={() => onChange(s.id)}
                    style={{
                      ...
                      background: s.id === active ? 'var(--paper-mute)' : 'transparent',
                      color: s.id === active ? 'var(--ink)' : 'var(--ink-soft)',
                      fontSize: 13, cursor: 'pointer', border: 'none'
                    }}>
              <span className="kanji" style={{ fontSize: 13, width: 14,
                            color: s.id === active ? 'var(--accent)' : 'var(--ink-mute)' }}>{s.kanji}</span>
              <span>{s.label}</span>
              {s.badge != null && (
                <span className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)' }}>{s.badge}</span>
              )}
            </button>
          ))}
        </div>
