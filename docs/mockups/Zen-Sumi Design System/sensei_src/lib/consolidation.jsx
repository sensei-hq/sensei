  ...
  return (
    <div className="sensei" data-screen-label="Observatory · Memory consolidation"
      ...
      <div style={{ padding: '22px 36px 18px', borderBottom: 'var(--hairline)',
        ...
        <div className="kanji" style={{ fontSize: 42, color: 'var(--accent)', lineHeight: 1 }}>結</div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  ...
                  {d && (
                    <span style={{ fontSize: 9.5, letterSpacing: '0.1em',
                                    textTransform: 'uppercase',
                                    color: d === "merged" ? 'var(--success)' : 'var(--ink-mute)' }}>
                      {d}
                    </span>
                  )}
                </div>
  ...
  return (
    <div style={{ maxWidth: 920 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12,
                     fontSize: 11, color: 'var(--ink-mute)', letterSpacing: '0.12em',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 24px 1fr',
        ...
        <div style={{ display: 'flex', flexDirection: 'column',
          ...
          <span className="kanji" style={{ fontSize: 18, color: 'var(--accent)',
                        margin: '8px 0' }}>→</span>
          <div style={{ width: 1, flex: 1, background: 'var(--paper-edge)' }}/>
        </div>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center',
            ...
            <span style={{ fontSize: 10, letterSpacing: '0.14em', color: 'var(--accent)',
                            textTransform: 'uppercase', fontWeight: 500 }}>
              Proposed merged memory
            </span>
            <span className="kanji" style={{ fontSize: 12, color: 'var(--accent)' }}>新</span>
          </div>
      ...
      {!decision ? (
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', paddingTop: 4 }}>
          ...
          <button onClick={() => onDecide("kept")}
            ...
            <span className="kanji" style={{ fontSize: 12.5, color: 'var(--ink-mute)' }}>別</span>
            Keep separate
          </button>
          <span style={{ flex: 1 }}/>
        ...
        <div style={{ padding: '14px 18px',
                       background: decision === "merged" ? 'var(--success-soft)'
                                                          : 'var(--paper-soft)',
                       border: 'var(--hairline)',
                       borderLeft: `2px solid ${decision === "merged" ? 'var(--success)' : 'var(--ink-mute)'}`,
                       borderRadius: 6,
                       display: 'flex', alignItems: 'center', gap: 12 }}>
          <span className="kanji" style={{ fontSize: 16,
                        color: decision === "merged" ? 'var(--success)' : 'var(--ink-mute)' }}>
            {decision === "merged" ? "結" : "別"}
          </span>
          <div style={{ flex: 1, fontSize: 12.5, color: 'var(--ink)' }}>
            {decision === "merged"
              ? <>Merged. <span className="mono" style={{ color: 'var(--ink-mute)' }}>
                    {p.sourceIds.join(", ")}
  ...
  return (
    <div style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 6 }}>
        <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>{m.id}</span>
        <span style={{ flex: 1 }}/>
...
function MergedMemory({ m }) {
  return (
    <div style={{ background: 'var(--paper)',
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 6 }}>
        <span className="mono" style={{ fontSize: 10.5, color: 'var(--accent)' }}>m-merged-pending</span>
        <span style={{ flex: 1 }}/>
        <span className="mono" style={{ fontSize: 10, color: 'var(--accent)' }}>
          str {m.strength}
        </span>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap',
        ...
        {m.scope.filePatterns?.map(g => (
          <span key={g} className="mono" style={{ fontSize: 9.5, padding: '2px 6px',
                       borderRadius: 3, background: 'var(--paper-soft)',
                       color: 'var(--ink-mute)' }}>{g}</span>
        ))}
      </div>
    </div>
  );
...
function DiffStat({ label, before, after, delta, same, positiveLow }) {
  ...
  const color = same ? 'var(--ink-mute)' :
                positive ? 'var(--success)' :
                positive === false ? 'var(--accent)' : 'var(--ink-mute)';
  return (
    <div style={{ background: 'var(--paper-soft)', padding: '12px 14px' }}>
      <div style={{ fontSize: 9.5, letterSpacing: '0.14em', color: 'var(--ink-faint)',
                     textTransform: 'uppercase', marginBottom: 6 }}>{label}</div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
        <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{before}</span>
