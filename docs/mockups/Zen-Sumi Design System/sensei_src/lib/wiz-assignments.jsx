          ...
          return (
            <button key={r.id} onClick={() => setActive(r.id)}
              ...
              <span style={{ fontSize: 10, fontFeatureSettings: '"tnum"',
                              ...
                              background: on ? 'rgba(255,255,255,.18)' : 'var(--paper-soft)',
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 4 }}>
        <span className="kanji" style={{ fontSize: 24, color: 'var(--accent)' }}>{role.kanji}</span>
        <div className="display" style={{ fontSize: 18 }}>{role.label}</div>
      </div>
      <div style={{ fontSize: 12.5, color: 'var(--ink-mute)', marginBottom: 18 }}>
        {role.hint}
      </div>
...
function PriorityRow({ model, provider, position, canUp, canDown, onUp, onDown, onRemove }) {
  ...
  return (
    <div style={{ display: 'grid',
                   ...
                   padding: '8px 10px',
                   background: primary ? 'var(--ink)' : 'var(--paper-soft)',
  ...
  return (
    <button onClick={onClick} disabled={disabled}
            style={{ width: 22, height: 22, border: 'none',
                     ...
                     color: disabled
                              ? (primary ? 'rgba(255,255,255,.22)' : 'var(--ink-faint)')
                              : (primary ? 'var(--paper)' : 'var(--ink-soft)'),
                     fontSize: 11,
      ...
      {Object.entries(byProvider).map(([pid, g]) => (
        <div key={pid}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
            <span className="kanji" style={{ fontSize: 12, color: 'var(--accent)' }}>{g.kanji}</span>
