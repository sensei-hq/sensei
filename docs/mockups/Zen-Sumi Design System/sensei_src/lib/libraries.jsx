...
function LibIcon({ letter, tone = 'var(--accent)', size = 28 }) {
  return (
    <div style={{
      ...
      background: 'var(--paper-mute)', border: 'var(--hairline)',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--ink-soft)', marginTop: 3, lineHeight: 1.55 }}>
            {d.tagline}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            {d.usage.topSymbols.map(s => (
              <div key={s.symbol} style={{ display: 'grid',
                ...
                <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>{s.n}×</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {d.usage.places.map((p, i) => (
              <div key={i} style={{ padding: '7px 2px', borderBottom: 'var(--hairline)' }}>
                <div className="mono" style={{ fontSize: 11, color: 'var(--ink)' }}>
                  {p.file}<span style={{ color: 'var(--ink-faint)' }}>:{p.line}</span>
                </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {d.rules.map((r, i) => (
              <div key={i} style={{ padding: '10px 12px', borderRadius: 5,
                ...
                <div style={{ fontSize: 12.5, color: 'var(--ink)' }}>"{r.rule}"</div>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between',
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--ink-mute)', marginTop: 4, marginLeft: 25 }}>
    ...
    <button onClick={onClick} style={{
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'baseline' }}>
          ...
          <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>v{item.version}</span>
        </div>
  ...
  return (
    <div className="sensei" data-screen-label="Libraries · Unified"
      ...
      <div style={{ padding: '28px 44px 18px', borderBottom: 'var(--hairline)',
        ...
        <div className="kanji" style={{ fontSize: 48, color: 'var(--accent)', lineHeight: 1 }}>庫</div>
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 12.5, color: 'var(--ink-soft)', margin: '6px 0 0', maxWidth: 620 }}>
      ...
      <div style={{ padding: '14px 44px', borderBottom: 'var(--hairline)',
        ...
        <span style={{ width: 1, height: 18, background: 'var(--ink-5, #00000018)' }}/>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 10.5, color: 'var(--ink-mute)', letterSpacing: '0.12em',
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr',
        ...
        <div style={{ overflow: 'auto', padding: '26px 32px', background: 'var(--paper-soft)' }}>
    ...
    <div className="sensei" data-screen-label="Libraries · Workspace"
      ...
      <div style={{ padding: '20px 44px 0', borderBottom: 'var(--hairline)' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 14 }}>
          <span className="kanji" style={{ fontSize: 24, color: 'var(--accent)' }}>庫</span>
            ...
            return (
              <button key={g.id} onClick={() => setTab(g.id)}
                      style={{ padding: '10px 14px', fontSize: 12.5,
                                ...
                                color: on ? 'var(--ink)' : 'var(--ink-mute)', marginBottom: -1,
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '320px 1fr',
        ...
        <div style={{ overflow: 'auto', padding: '14px 12px',
                       borderRight: 'var(--hairline)', background: 'var(--paper-soft)' }}>
          <div style={{ fontSize: 11, color: 'var(--ink-mute)', padding: '4px 10px 8px' }}>
            {group.sub}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {group.items.map(x => (
              <button key={x.id} onClick={() => setFocus(x.id)}
                ...
                <LibIcon letter={x.icon}
                         ...
                         size={26}/>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 12.5, color: 'var(--ink)' }}>{x.name}</div>
    ...
    <MCPShell activeTab={activeTab} onTab={onTab}
            ...
            return (
              <button key={m.id} onClick={() => setScope(m.id)}
                ...
                <span className="kanji" style={{ fontSize: 11,
                              color: on ? 'var(--accent)' : 'var(--ink-mute)' }}>{m.kanji}</span>
                <span>{m.name}</span>
                <span className="mono" style={{ fontSize: 10, color: 'var(--ink-faint)' }}>
                  {m.tools}
                </span>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '340px 1fr',
        ...
        <aside style={{ overflow: 'auto', borderRight: 'var(--hairline)',
                         background: 'var(--paper-soft)' }}>
          {filtered.length === 0 && (
            <div style={{ padding: '24px 18px', fontSize: 12, color: 'var(--ink-mute)',
                           textAlign: 'center' }}>
              No tools match.
            </div>
          )}
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <button onClick={() => setHasRun(true)}
                  ...
                  style={{ padding: '8px 16px', fontSize: 12,
                            background: missing.length > 0 ? 'var(--paper-mute)' : 'var(--ink)',
                            color: missing.length > 0 ? 'var(--ink-mute)' : 'var(--paper)',
                            borderRadius: 5,
...
function InputField({ input, value, onChange }) {
  const label = (
    <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 6 }}>
      <span style={{ fontSize: 10.5, letterSpacing: '0.1em', color: 'var(--ink-soft)',
                     textTransform: 'uppercase' }}>
        {input.label || input.key}
      </span>
      {input.required
        ? <span className="mono" style={{ fontSize: 9.5, color: 'var(--accent)' }}>required</span>
        : <span className="mono" style={{ fontSize: 9.5, color: 'var(--ink-faint)' }}>optional</span>}
      <span style={{ flex: 1 }}/>
      <span className="mono" style={{ fontSize: 9.5, color: 'var(--ink-faint)' }}>
        {input.kind}
      </span>
            ...
            return (
              <button key={o} onClick={() => onChange(o)}
                      style={{ padding: '6px 10px', fontSize: 11,
                                borderRadius: 4,
                                background: on ? 'var(--ink)' : 'var(--paper-mute)',
                                color: on ? 'var(--paper)' : 'var(--ink-soft)' }}>
                {o}
              </button>
            );
