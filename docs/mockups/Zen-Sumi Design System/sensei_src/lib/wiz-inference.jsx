  ...
  return (
    <div style={{ marginBottom: 22, padding: '12px 18px',
      ...
      <div style={{ fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase',
                     color: 'var(--ink-faint)' }}>this machine</div>
      {[sys.chip, sys.ram, sys.cores.split('·')[1]?.trim() || sys.cores, sys.os].map((v, i, arr) => (
        <React.Fragment key={i}>
          <div style={{ fontSize: 12, color: 'var(--ink-soft)' }}>{v}</div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button onClick={onSave} disabled={!value}
                style={{ fontSize: 12, padding: '8px 14px', borderRadius: 4, border: 'none',
                         background: value ? 'var(--ink)' : 'var(--paper-edge)',
                         color: value ? 'var(--paper)' : 'var(--ink-mute)',
...
function SectionLabel({ children }) {
  return (
    <div style={{ fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase',
                   color: 'var(--ink-faint)', marginBottom: 8 }}>{children}</div>
  );
}
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 14,
      ...
      <div style={{ paddingLeft: m.recommended ? 10 : 0 }}>
        ...
        {m.note && <div style={{ fontSize: 10.5, color: 'var(--ink-faint)', marginTop: 2 }}>{m.note}</div>}
        {active && (
        ...
        {models.map(m => (
          <div key={m.id} style={{
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                <span style={{ fontSize: 13 }}>{m.name}</span>
                {m.context && <span style={{ fontSize: 10, color: 'var(--ink-faint)' }}>{m.context}</span>}
              </div>
              {m.cost && (
                <div style={{ fontSize: 10.5, color: 'var(--ink-faint)',
            ...
            <span style={{ fontSize: 10.5, color: isConfigured ? 'var(--matcha)' : 'var(--ink-faint)',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 14,
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            ...
            <button onClick={() => setShowAdd(true)}
                    style={{ fontSize: 10.5, color: 'var(--ink-soft)', padding: '2px 8px',
                ...
                <button key={p.id} onClick={() => setFocusId(p.id)}
                  ...
                  <span style={{ fontSize: 9.5, letterSpacing: '0.08em',
                                  ...
                                  color: cfg
                                    ...
                                    : (active ? 'var(--paper)' : 'var(--ink-faint)') }}>
                    {cfg ? `${count}` : "—"}
                  </span>
                </button>
              );
        ...
        <div style={{ background: 'var(--paper)', border: 'var(--hairline)',
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10,
                         marginBottom: 4 }}>
            <span className="kanji" style={{ fontSize: 24, color: 'var(--accent)' }}>{focus.kanji}</span>
          ...
          {D.addable.map(p => (
            <button key={p.id} onClick={() => onAdd(p.id)}
                    style={{ display: 'flex', alignItems: 'center', gap: 12,
                             ...
                             border: 'var(--hairline)', background: 'var(--paper)',
                             cursor: 'pointer', textAlign: 'left' }}>
              <span className="kanji" style={{ fontSize: 18, color: 'var(--accent)' }}>{p.kanji}</span>
