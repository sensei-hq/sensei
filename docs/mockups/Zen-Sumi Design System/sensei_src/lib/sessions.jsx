...
function SsHero({ totals }) {
  return (
    <div style={{ padding: '22px 36px 16px', borderBottom: 'var(--hairline)',
      ...
      <div className="kanji" style={{ fontSize: 42, color: 'var(--accent)', lineHeight: 1 }}>録</div>
...
const LANG_TONE = {
  ...
  "TypeScript": { color: 'var(--ai, var(--ink-soft))' },
...
function SessionTags({ project }) {
  ...
  if (!meta) return null;
  const lang = LANG_TONE[meta.lang] || { color: 'var(--ink-mute)' };
  return (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            {filtered.length === 0 &&
              <div style={{ padding: 36, textAlign: 'center', fontSize: 12,
                             color: 'var(--ink-faint)' }}>no sessions match.</div>}
          </div>
        </section>
      </div>
    </div>
      ...
      {options.map(o => (
        <button key={o} onClick={() => setValue(o)}
                style={{ padding: '3px 10px', fontSize: 11,
                          ...
                          border: value === o ? '1px solid var(--ink)' : '1px solid var(--paper-edge)',
    ...
    <article style={{ display: 'grid',
      ...
      <span style={{ width: 7, height: 7, borderRadius: '50%',
                      background: session.ftr ? 'var(--success)' :
                                  session.outcome === "abandoned" ? 'var(--ink-mute)' :
                                  'var(--warning)' }}/>
      ...
      <div style={{ textAlign: 'right' }}>
        ...
        <div className="mono" style={{ fontSize: 10, color: session.corrections === 0 ?
                      'var(--success)' : 'var(--ink-mute)', marginTop: 2 }}>
    ...
    <div className="sensei" data-screen-label="Observatory · Sessions · Timeline"
      ...
      <div style={{ flex: 1, overflow: 'auto', minHeight: 0,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
          ...
          <RetroSummary kanji="観" tone="var(--ink-soft)" label="Insights"
  ...
  return (
    <section style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
      ...
      <ul style={{ margin: 0, padding: 0, listStyle: 'none',
        ...
        {items.map(it => (
          <li key={it.id} style={{ fontSize: 11.5, color: 'var(--ink)', lineHeight: 1.5,
  ...
  return (
    <section style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 12 }}>
        <span className="kanji" style={{ fontSize: 14, color: 'var(--accent)' }}>表</span>
        <span style={{ fontSize: 11, letterSpacing: '0.14em', color: 'var(--ink-mute)',
      ...
      <div style={{ display: 'grid',
        ...
        {days.map(d => (
          <div key={d} className="mono"
               style={{ fontSize: 10, color: 'var(--ink-faint)', textAlign: 'center' }}>
            {dayShort[d]}
          </div>
        ))}
        {/* Project rows */}
          ...
          return (
            <React.Fragment key={p}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7,
                ...
                <span className="mono" style={{ fontSize: 9.5, color: 'var(--ink-faint)' }}>{meta.lang}</span>
              </div>
              {days.map(d => {
                const c = cell(p, d);
                if (!c) {
