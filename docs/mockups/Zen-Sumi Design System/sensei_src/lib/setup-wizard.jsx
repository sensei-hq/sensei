  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <span style={{ width: 7, height: 7, borderRadius: 4,
                      background: anyBad ? 'var(--accent)' : 'var(--matcha)' }}/>
          ...
          return (
            <button key={s.id}
                    ...
                    style={{
                      ...
                      color: isCur ? 'var(--ink)' : isDone ? 'var(--ink-soft)' : 'var(--ink-faint)',
...
function WizBottom({ stage, stageIdx, total, back, next, onDone, state }) {
  ...
  return (
    <div style={{ borderTop: 'var(--hairline)', padding: '14px 64px',
      ...
      <div style={{ fontSize: 11, letterSpacing: '0.12em', color: 'var(--ink-mute)',
                    textTransform: 'uppercase' }}>
        {String(stageIdx + 1).padStart(2, "0")} <span style={{ color: 'var(--ink-faint)' }}>/ {total}</span>
        <span style={{ marginLeft: 12, color: 'var(--ink-soft)', textTransform: 'none',
      ...
      <div style={{ flex: 1, display: 'flex', gap: 4, alignItems: 'center' }}>
        {Array.from({ length: total }).map((_, i) => (
          <span key={i} style={{
            ...
            background: i <= stageIdx ? 'var(--ink)' : 'var(--paper-edge)',
      ...
      <button onClick={back} disabled={isFirst}
              style={{ fontSize: 12, color: isFirst ? 'var(--ink-faint)' : 'var(--ink-soft)',
                       padding: '8px 14px' }}>
        ← Back
      </button>
      ...
      {isLast ? (
        ...
        <button onClick={next} disabled={!canAdvance}
                style={{ fontSize: 13,
                         background: canAdvance ? 'var(--ink)' : 'var(--paper-edge)',
                         color: canAdvance ? 'var(--paper)' : 'var(--ink-mute)',
                         padding: '10px 22px', borderRadius: 6, letterSpacing: 0.2 }}>
          Continue →
        </button>
      )}
...
function WizWelcome() {
  return (
    <div style={{ maxWidth: 680, margin: '10px auto 0' }}>
      <div style={{ fontSize: 11, color: 'var(--ink-mute)', letterSpacing: '0.12em',
      ...
      <p style={{ fontSize: 16, color: 'var(--ink-soft)', lineHeight: 1.7, maxWidth: 560,
                   margin: '0 0 28px' }}>
      ...
      <p style={{ fontSize: 14, color: 'var(--ink-mute)', lineHeight: 1.7, maxWidth: 560,
                   margin: '0 0 44px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 18,
        ...
        {[
          ...
          <div key={item.k}>
            <div className="kanji" style={{ fontSize: 26, color: 'var(--accent)', marginBottom: 8 }}>{item.k}</div>
            <div className="display" style={{ fontSize: 20 }}>{item.t}</div>
            <div style={{ fontSize: 12, color: 'var(--ink-mute)', marginTop: 3 }}>{item.s}</div>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{ maxWidth: 820, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 22 }}>
        <span style={{ fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase',
                        color: 'var(--ink-faint)' }}>demo · starting state</span>
        <div style={{ display: 'flex', gap: 0, background: 'var(--paper-soft)', padding: 2,
          ...
          {D.componentsVariants.map(v => (
            <button key={v.id}
                    ...
                    style={{ padding: '5px 10px', fontSize: 10.5, borderRadius: 3,
                             ...
                             color: state.components.variant === v.id ? 'var(--ink-soft)' : 'var(--ink-mute)',
                             border: 'none' }}>
              {v.label}
            </button>
          ))}
        ...
        {variant.components.map(c => {
          ...
          return (
            <div key={c.id} style={{
              ...
              background: 'var(--paper-soft)', alignItems: 'center',
              transition: 'all .3s'
            }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span className="mono" style={{ fontSize: 11,
                               color: phase === "ready" ? 'var(--success)' : 'var(--ink-soft)',
      ...
      <p style={{ fontSize: 12, color: 'var(--ink-mute)', marginTop: 20, lineHeight: 1.7 }}>
...
function StatusDot({ status }) {
  const map = {
    ...
    missing:   { c: 'var(--ink-faint)', l: "Missing" },
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <span className="mono" style={{ fontSize: 11, color: 'var(--ink-soft)' }}>{m.l}</span>
...
function WizAcps({ state, upd }) {
  ...
  return (
    <div style={{ maxWidth: 820, margin: '0 auto' }}>
          ...
          return (
            <button key={family.id}
                    ...
                    style={{
                      ...
                      background: on ? 'var(--paper-soft)' : 'var(--paper)',
                    ...
                    {family.products.map(p => (
                      <span key={p.id}
                            ...
                            style={{
                              ...
                              border: p.found ? 'var(--hairline)' : '1px dashed var(--ink-faint, #d8d4cc)',
                              color: p.found ? 'var(--ink)' : 'var(--ink-mute)'
                            }}>
                        <span style={{
                          width: 5, height: 5, borderRadius: '50%',
                          background: p.found ? 'var(--success, #6b8e7f)' : 'var(--ink-faint, #c8c4bc)'
                        }}/>
                        {p.label}
                        {p.version && <span className="mono" style={{ color: 'var(--ink-mute)' }}>v{p.version}</span>}
                      </span>
                    ))}
                  </div>
                ) : (
      ...
      <div style={{ marginTop: 20, fontSize: 12, color: 'var(--ink-mute)', lineHeight: 1.7 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        <input value={state.newFolder}
               ...
               style={{
                 ...
                 background: 'var(--paper-soft)', border: 'var(--hairline)', borderRadius: 6,
                 outline: 'none'
               }}/>
        <button onClick={add}
        ...
        {state.folders.map(f => (
          <div key={f.id} style={{
            ...
            <div>
              ...
              <div style={{ fontSize: 10.5, color: 'var(--ink-mute)', marginTop: 2 }}>{f.note}</div>
            </div>
            ...
            <button onClick={() => remove(f.id)}
                    style={{ fontSize: 11, color: 'var(--ink-mute)', padding: '4px 8px' }}>
              remove
            </button>
          </div>
        ))}
  ...
  if (!started) {
    return (
      <div style={{ maxWidth: 820, margin: '0 auto' }}>
        ...
        <div style={{ padding: '50px 40px', background: 'var(--paper-soft)',
                       borderRadius: 10, border: 'var(--hairline)', textAlign: 'center' }}>
          <div className="kanji" style={{ fontSize: 60, color: 'var(--accent)', opacity: 0.4,
                                           marginBottom: 20 }}>探</div>
          <p style={{ fontSize: 15, color: 'var(--ink-soft)', margin: '0 0 20px',
          ...
          <button onClick={start}
                  style={{ padding: '12px 28px', fontSize: 14, background: 'var(--ink)',
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0,
                     border: 'var(--hairline)', borderRadius: 8, background: 'var(--paper-soft)',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minHeight: 360 }}>
          {discoveredSolutions.length === 0 && (
            <div style={{ padding: 40, border: '1px dashed var(--paper-edge)',
                           borderRadius: 10, textAlign: 'center', color: 'var(--ink-faint)',
                           fontSize: 13, fontStyle: 'italic' }}>
              <div className="kanji" style={{ fontSize: 38, color: 'var(--accent)',
          ...
          <div style={{ height: 360, overflow: 'auto', padding: '8px 14px' }}>
            {events.slice().reverse().map((e, i) => (
              <div key={e.t} style={{
                ...
                <span className="mono" style={{ color: 'var(--ink-mute)' }}>+{(e.t/1000).toFixed(2)}s</span>
                <span className="mono" style={{
                  color: e.level === "success"  ? 'var(--success)' :
                         e.level === "discover" ? 'var(--accent)'  :
                         e.level === "process"  ? 'var(--ink)' :
                         e.level === "queue"    ? 'var(--warning)' : 'var(--ink-mute)'
                }}>
                  {e.level}
                </span>
      ...
      {done && (
        <div style={{ marginTop: 20, padding: '14px 18px', borderRadius: 8,
          ...
          <span className="kanji" style={{ fontSize: 18, color: 'var(--success)' }}>✓</span>
...
function ScanSolutionCard({ sol, solState, repoStates }) {
  ...
  return (
    <div style={{
      border: allDone ? '1.5px solid var(--ink-soft)' : 'var(--hairline)',
      borderRadius: 10, background: 'var(--paper-soft)',
      padding: '16px 18px',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 14, alignItems: 'center' }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)', marginTop: 1 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div className="mono" style={{ fontSize: 10,
                         color: allDone ? 'var(--success)' : 'var(--ink-mute)',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{
                    ...
                    background: isDone ? 'var(--success)' :
                                ...
                                rs.state === "queued" ? 'var(--warning)' : 'var(--ink-faint)',
  ...
  return (
    <div style={{ padding: '16px 18px', borderRight: 'var(--hairline)' }}>
      ...
      <div style={{ fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase',
                     color: 'var(--ink-mute)', marginTop: 2 }}>{label}</div>
    </div>
  );
}
    ...
    <div style={{ maxWidth: 940, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {sols.map(s => {
          ...
          <div key={s.id} style={{
            border: 'var(--hairline)',
            borderRadius: 10, background: s.confirmed ? 'var(--paper-soft)' : 'var(--paper)',
            ...
            <div style={{ display: 'grid',
              ...
              <div>
                <input value={s.renamed ?? s.name}
                       ...
                       onFocus={e => e.target.style.borderBottom = '1px dashed var(--ink-mute)'}
              ...
              {mergeTargets.length > 0 ? (
                <button onClick={() => { setMergeMenu(isMergeOpen ? null : s.id); setRepoMenu(null); }}
                        style={{ fontSize: 11, color: 'var(--ink-mute)', padding: '6px 10px',
                                  borderRadius: 4 }}>
                  merge…
                </button>
              ) : <span/>}
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {mergeTargets.map(t => (
                    <button key={t.id} onClick={() => mergeInto(s.id, t.id)}
                      ...
                      <span className="kanji" style={{ color: 'var(--accent)', fontSize: 12 }}>{t.kanji}</span>
                      {t.renamed ?? t.name}
                      <span style={{ color: 'var(--ink-faint)', fontSize: 10 }}>({t.projects.length})</span>
                    </button>
                  ))}
                  <button onClick={() => setMergeMenu(null)} className="mono" style={{
                    fontSize: 10.5, padding: '6px 10px', color: 'var(--ink-mute)'
                  }}>cancel</button>
                </div>
              </div>
            )}
            ...
            <div style={{ marginTop: 14, display: 'flex', flexWrap: 'wrap', gap: 6,
                ...
                return (
                  <span key={p.id} style={{ position: 'relative', display: 'inline-block' }}>
                    <span className="mono" style={{
                      ...
                      background: 'var(--paper)', border: 'var(--hairline)',
                      borderRadius: 3, color: 'var(--ink-soft)',
                      display: 'inline-flex', alignItems: 'center', gap: 8
                    }}>
                      {p.name}
                      <span style={{ color: 'var(--ink-faint)' }}>{p.files}f</span>
                      {role && (
                      ...
                      <button onClick={(e) => {
                              ...
                              style={{
                                padding: '0 6px', fontSize: 13, color: 'var(--ink-mute)',
                                borderLeft: '1px solid var(--paper-edge)',
                    ...
                    {isOpen && (
                      <div style={{
                        ...
                        <div style={{ fontSize: 9.5, letterSpacing: '0.14em', color: 'var(--ink-mute)',
                        ...
                        {moveTargets.map(t => (
                          <button key={t.id} onClick={() => moveRepo(s.id, p.id, t.id)}
                                  className="mono" style={{
                                    ...
                                    fontSize: 11, color: 'var(--ink-soft)', borderRadius: 3,
                        ...
                        {isMulti && (
                          <button onClick={() => moveRepo(s.id, p.id, "__new__")}
                                  style={{
                                    display: 'flex', width: '100%', padding: '7px 8px',
                                    fontSize: 11, color: 'var(--ink-mute)', borderRadius: 3,
              ...
              {isMulti && (
                <button onClick={() => split(s.id)} className="mono" style={{
                  fontSize: 10.5, padding: '4px 10px', color: 'var(--ink-mute)',
              ...
              <div style={{
                ...
                {s.projects.map(p => (
                  <div key={p.id} style={{
                    ...
                    <div>
                      <div className="mono" style={{ fontSize: 12, color: 'var(--ink-soft)' }}>{p.name}</div>
                      ...
                      {D.roles.map(r => (
                        <button key={r.id} onClick={() => setRole(p.id, r.id)}
                                ...
                                style={{
                                  ...
                                  color: state.roles[p.id] === r.id ? 'var(--paper)' : 'var(--ink-mute)'
                                }}>
                          {r.label}
                        </button>
                      ))}
                  ...
                  {D.metadata.statuses.map(st => (
                    <button key={st.id}
                            ...
                            style={{
                              ...
                              color: state.metadata[s.id].status === st.id ? 'var(--paper)' : 'var(--ink-mute)'
                            }}>
                      {st.label}
                    </button>
                  ))}
...
function MetaField({ label, children }) {
  return (
    <div>
      <div style={{ fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase',
                     color: 'var(--ink-mute)', marginBottom: 6 }}>{label}</div>
      {children}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column',
                     ...
                     background: 'var(--paper-soft)', marginBottom: 24 }}>
              ...
              return (
                <div key={lib.id}
                  ...
                  <button onClick={() => removeExtra(lib.id)}
                          style={{ fontSize: 11, color: 'var(--ink-faint)' }}>remove</button>
                </div>
              );
            })}
          </div>
      ...
      {!showAdd ? (
        <button onClick={() => setShowAdd(true)}
                style={{ padding: '10px 16px', fontSize: 12.5,
                          background: 'var(--paper-soft)',
                          border: '1px dashed var(--ink-faint)', borderRadius: 6,
                          color: 'var(--ink-soft)' }}>
          + Add a library
        </button>
      ) : (
        <div style={{ padding: '18px 20px', background: 'var(--paper-soft)',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 0.5fr',
            ...
            <div>
              <div style={{ fontSize: 10.5, color: 'var(--ink-soft)', marginBottom: 4 }}>Name</div>
            ...
            <div>
              <div style={{ fontSize: 10.5, color: 'var(--ink-soft)', marginBottom: 4 }}>
                Docs URL <span style={{ color: 'var(--ink-faint)' }}>· optional</span>
              </div>
            ...
            <div>
              <div style={{ fontSize: 10.5, color: 'var(--ink-soft)', marginBottom: 4 }}>Lang</div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={addExtra} disabled={!form.name.trim()}
                    style={{ padding: '7px 14px', fontSize: 12,
                              background: form.name.trim() ? 'var(--ink)' : 'var(--paper-mute)',
                              color: form.name.trim() ? 'var(--paper)' : 'var(--ink-mute)',
                              borderRadius: 4 }}>
              Add
            </button>
...
function WizRegistry({ state, upd }) {
  ...
  return (
    <div>
      ...
      <div style={{ padding: '14px 18px', background: 'var(--paper-soft)',
                     border: 'var(--hairline)', borderRadius: 8,
                     marginBottom: 24 }}>
        <div style={{ fontSize: 10, letterSpacing: '0.14em', color: 'var(--ink-mute)',
                       textTransform: 'uppercase', marginBottom: 10 }}>
  ...
  return (
    <div style={{ display: 'grid',
      ...
      <div style={{ width: 32, height: 32, borderRadius: 6,
                     background: 'var(--paper-mute)', display: 'flex',
      ...
      <div>
        <div style={{ fontSize: 13, color: 'var(--ink)' }}>
          {mcp.name}
          <span className="mono" style={{ fontSize: 10, color: 'var(--ink-faint)', marginLeft: 8 }}>
            by {mcp.publisher}
          </span>
          {mcp.verified && (
            <span className="mono" style={{ fontSize: 9.5, color: 'var(--success)',
      ...
      {mcp.trigger && mcp.trigger.length > 0 ? (
        <span className="mono" style={{ fontSize: 10, color: 'var(--success)',
                      padding: '2px 7px', background: 'var(--success-soft)',
...
function LibDocChip({ status }) {
  const map = {
    ...
    schema:  { label: "schema only",  tone: 'var(--ink-soft)', bg: 'var(--paper-mute)'   },
    none:    { label: "no docs",      tone: 'var(--ink-mute)', bg: 'var(--paper-mute)'   }
  };
...
function WizPreferences({ state, upd }) {
    ...
    <section style={{ paddingTop: 24, paddingBottom: 4,
      ...
      <header style={{ display: 'flex', alignItems: 'baseline', gap: 14,
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {sub && (
            <p style={{ fontSize: 12, color: 'var(--ink-mute)', margin: '3px 0 0',
  ...
  const Row = ({ label, hint, children }) => (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 32,
      ...
      <div style={{ minWidth: 0 }}>
        ...
        {hint && <div style={{ fontSize: 11.5, color: 'var(--ink-mute)', marginTop: 3,
  ...
  const Toggle = ({ value, onChange }) => (
    <button onClick={() => onChange(!value)}
            style={{ width: 36, height: 20, borderRadius: 999,
                      background: value ? 'var(--accent)' : 'var(--paper-edge)',
      ...
      {options.map((opt, i) => (
        <button key={opt.value} onClick={() => onChange(opt.value)}
                style={{ padding: '6px 12px', fontSize: 11.5,
                          ...
                          background: value === opt.value ? 'var(--paper-mute)' : 'var(--paper)',
                          color: value === opt.value ? 'var(--ink)' : 'var(--ink-mute)',
                          cursor: 'pointer' }}>
          {opt.label}
        </button>
      ))}
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto' }}>
      ...
      <p style={{ fontSize: 12, color: 'var(--ink-mute)', margin: '28px 0 0',
                   fontStyle: 'italic', lineHeight: 1.6, textAlign: 'center' }}>
        These save when you press <span style={{ color: 'var(--ink-soft)' }}>Enter observatory</span>.
        Re-open this wizard from the sidebar's <span style={{ color: 'var(--ink-soft)' }}>調 Configure</span> link to change them later.
      </p>
    </div>
  );
}
...
function WizDone({ state }) {
  ...
  return (
    <div style={{ maxWidth: 680, margin: '20px auto 0', textAlign: 'center' }}>
      <div className="kanji" style={{ fontSize: 92, color: 'var(--accent)', marginBottom: 10 }}>観</div>
      <h1 className="display" style={{ fontSize: 44, fontWeight: 300, letterSpacing: '-0.02em',
                    margin: '0 0 16px' }}>
        The observatory is ready.
      </h1>
      <p style={{ fontSize: 15, color: 'var(--ink-soft)', lineHeight: 1.6, maxWidth: 480,
                   margin: '0 auto 36px' }}>
      ...
      <p className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)',
...
function WizHeader({ n, title, tagline }) {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: 13.5, color: 'var(--ink-mute)', margin: 0 }}>{tagline}</p>
    </div>
  );
}
    ...
    <div className="sensei" data-screen-label="Empty Observatory"
      ...
      <main style={{ flex: 1, overflow: 'auto', position: 'relative',
        ...
        <div className="kanji" style={{
          ...
          fontSize: 640, color: 'var(--accent)', opacity: 0.035,
        ...
        <div style={{ maxWidth: 680, width: '100%', position: 'relative', zIndex: 1,
          ...
          <div>
            ...
            <div style={{ fontSize: 10, letterSpacing: '0.18em', color: 'var(--ink-mute)',
            ...
            <button onClick={onBeginSetup}
                    style={{ padding: '13px 28px', fontSize: 14, background: 'var(--ink)',
                              color: 'var(--paper)', borderRadius: 6, letterSpacing: 0.2 }}>
              Begin setup →
            </button>

            <div style={{ marginTop: 18, fontSize: 11, color: 'var(--ink-mute)' }}>
          ...
          <div style={{
            ...
            <div style={{ marginTop: 20, paddingTop: 16, borderTop: 'var(--hairline)',
              ...
              Works with{' '}
              <span className="mono" style={{ color: 'var(--ink-soft)' }}>claude-code</span>,{' '}
              <span className="mono" style={{ color: 'var(--ink-soft)' }}>cursor</span>,{' '}
              <span className="mono" style={{ color: 'var(--ink-soft)' }}>codex</span>,{' '}
              <span className="mono" style={{ color: 'var(--ink-soft)' }}>aider</span>.
            </div>
          </div>
        </div>
      </main>
