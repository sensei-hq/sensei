...
function MergedSidebar({ page, setPage, solutions, activeSolution, setActiveSolution, collapsed, setCollapsed }) {
  ...
  return (
    <aside style={{
      ...
      gap: collapsed ? 18 : 22,
      background: collapsed ? 'var(--paper-soft)' : 'var(--paper)',
      ...
      {collapsed ? (
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={() => setCollapsed(true)} title="Collapse sidebar"
                  style={{ width: 24, height: 24, borderRadius: 4, color: 'var(--ink-mute)',
      ...
      {collapsed ? (
        <>
          <button onClick={() => setCollapsed(false)} title="Expand sidebar"
                  style={{ width: 28, height: 28, borderRadius: 6, color: 'var(--ink-mute)',
                            display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.4">
  ...
  return (
    <div style={{ padding: '40px 56px', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 56, position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 24, marginBottom: 8 }}>
          ...
          <div style={{ paddingBottom: 18 }}>
            <div style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase',
                          color: 'var(--ink-mute)', marginBottom: 6 }}>一 First try right</div>
        ...
        <div style={{ marginTop: 18, color: trendUp ? 'var(--success)' : 'var(--accent)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 28, position: 'relative', zIndex: 1 }}>
        <div style={{ background: 'var(--paper-soft)', borderRadius: 14, padding: '28px 26px',
                      border: 'var(--hairline)' }}>
          <div style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase',
          ...
          <p style={{ fontSize: 13, color: 'var(--ink-soft)', lineHeight: 1.6, marginTop: 16, marginBottom: 22 }}>
            {topCoach.body}
          </p>
        ...
        <div>
          <div style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase',
                        color: 'var(--ink-mute)', marginBottom: 14 }}>Quality signals</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {[
              ...
              <div key={s.k} style={{ padding: 14, border: 'var(--hairline)', borderRadius: 8,
                ...
                <div style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>{s.k}</div>
                <div className="display" style={{ fontSize: 22, fontWeight: 400, marginTop: 4 }}>{s.v}</div>
                <div className="mono" style={{ fontSize: 10,
                              color: s.good ? 'var(--success)' : 'var(--accent)', marginTop: 2 }}>{s.d}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {data.coaching[1] && (
          <div style={{ padding: 16, border: 'var(--hairline)', borderRadius: 10, background: 'var(--paper)' }}>
            <div style={{ fontSize: 10, color: 'var(--ink-mute)', letterSpacing: '0.12em',
            ...
            <div className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)', marginTop: 6 }}>
...
function MergedOverview({ data, setPage, setActiveSolution }) {
  return (
    <div style={{ padding: '40px 56px', maxWidth: 1120 }}>
      ...
      <div style={{ color: 'var(--ink-mute)', fontSize: 13, marginBottom: 32 }}>
        Global FTR <span className="mono" style={{ color: 'var(--ink)' }}>78%</span> · week to date.
      </div>

      <div style={{ marginBottom: 40, color: 'var(--accent)' }}>
        ...
        {data.solutions.map(s => {
          ...
          return (
            <button key={s.id} onClick={() => { setActiveSolution(s.id); setPage("observatory"); }}
              ...
              <div style={{ fontSize: 11, color: 'var(--ink-mute)', marginBottom: 20 }}>{s.description}</div>
              ...
              <div style={{ marginTop: 14, color: up ? 'var(--success)' : 'var(--accent)' }}>
