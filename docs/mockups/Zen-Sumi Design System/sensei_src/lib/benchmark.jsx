// Benchmark runner — A/B compare assistant runs with vs without sensei tools.
// Two layouts:
//   A · Dashboard view — corpora list, runs table, single-run detail card
//   B · Lab notebook view — single run as a long-scroll narrative report

const { useState: bnS } = React;

// ─── Shared bits ───────────────────────────────────────────
function BnHero({ subtitle, title, blurb, stats }) {
  return (
    <div style={{ padding: '22px 36px 18px', borderBottom: 'var(--hairline)',
                   display: 'flex', alignItems: 'center', gap: 22 }}>
      <div className="kanji" style={{ fontSize: 42, color: 'var(--accent)', lineHeight: 1 }}>較</div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 10.5, letterSpacing: '0.18em', color: 'var(--ink-mute)',
                       textTransform: 'uppercase', marginBottom: 5 }}>{subtitle}</div>
        <h1 className="display" style={{ fontSize: 22, fontWeight: 400, margin: 0,
                                          color: 'var(--ink)' }}>{title}</h1>
        <p style={{ fontSize: 12, color: 'var(--ink-soft)', margin: '4px 0 0',
                     maxWidth: 760, lineHeight: 1.55 }}>{blurb}</p>
      </div>
      <div style={{ paddingLeft: 22, borderLeft: 'var(--hairline)',
                     display: 'flex', gap: 22 }}>
        {stats.map((s, i) => (
          <div key={i} style={{ textAlign: 'right' }}>
            <div className={s.mono ? "mono" : "display"} style={{
              fontSize: s.mono ? 13 : 22,
              color: s.accent ? 'var(--accent)' : 'var(--ink)',
              fontWeight: 400, lineHeight: 1
            }}>{s.n}</div>
            <div style={{ fontSize: 9.5, letterSpacing: '0.14em', color: 'var(--ink-faint)',
                           textTransform: 'uppercase', marginTop: 4 }}>{s.l}</div>
          </div>
        ))}
      </div>
    </div>
  );
}

// ─── Layout A: Dashboard ───────────────────────────────────
function BenchmarkRunnerDashboard() {
  const B = window.EXT_DATA.benchmark;
  const [activeRun, setActiveRun] = bnS(B.runs[0].id);
  const [activeCorpus, setActiveCorpus] = bnS(B.corpora[0].id);

  const run = B.runs.find(r => r.id === activeRun);
  const corpus = B.corpora.find(c => c.id === activeCorpus);

  return (
    <div className="sensei" data-screen-label="Benchmark · Dashboard"
         style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column',
                  background: 'var(--paper)', overflow: 'hidden' }}>
      <BnHero
        subtitle="Configure · Benchmark"
        title="Sensei vs no-sensei. Same model, same tasks."
        blurb="A corpus is a repo with a /tasks folder. Sensei runs each task twice: once without its tools, once with. The diff is the value."
        stats={[
          { n: B.corpora.length, l: "corpora" },
          { n: B.runs.length, l: "runs", mono: true },
          { n: `+${Math.round(B.runs.reduce((s,r)=>s+(r.b.score-r.a.score),0)/B.runs.length*100)}%`,
            l: "avg score lift", mono: true, accent: true },
        ]}
      />

      <div style={{ flex: 1, minHeight: 0, display: 'grid',
                     gridTemplateColumns: '1fr 1.4fr' }}>
        {/* Left: corpora + runs lists */}
        <div style={{ overflow: 'auto', padding: '22px 28px',
                       borderRight: 'var(--hairline)' }}>
          <BnSection title="Corpora">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {B.corpora.map(c => (
                <button key={c.id} onClick={() => setActiveCorpus(c.id)} style={{
                  textAlign: 'left', padding: '12px 14px', borderRadius: 5,
                  background: activeCorpus === c.id ? 'var(--paper-mute)' : 'var(--paper-soft)',
                  border: activeCorpus === c.id ? '1px solid var(--ink-mute)' : 'var(--hairline)',
                  cursor: 'pointer', fontFamily: 'var(--font-ui)'
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8,
                                 marginBottom: 5 }}>
                    <span style={{ fontSize: 13, color: 'var(--ink)' }}>{c.label}</span>
                    <span style={{ fontSize: 9.5,
                      color: c.kind === "private" ? 'var(--warning)' : 'var(--success)',
                      letterSpacing: '0.12em', textTransform: 'uppercase' }}>{c.kind}</span>
                  </div>
                  <div className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)',
                                                   marginBottom: 4 }}>{c.repo}</div>
                  <div style={{ display: 'flex', gap: 10, fontSize: 10.5,
                                 color: 'var(--ink-faint)' }}>
                    <span>{c.tasks} tasks</span>
                    <span>· {c.langs.join(', ')}</span>
                    <span>· {c.lastSync}</span>
                  </div>
                </button>
              ))}
              <button style={{ fontSize: 11, color: 'var(--ink-mute)',
                background: 'transparent', border: '1px dashed var(--paper-4)',
                borderRadius: 4, padding: '10px', cursor: 'pointer' }}>
                + import corpus from repo
              </button>
            </div>
          </BnSection>

          <BnSection title="Recent runs">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {B.runs.map(r => {
                const win = r.delta.passed > 0;
                return (
                  <button key={r.id} onClick={() => setActiveRun(r.id)} style={{
                    textAlign: 'left', padding: '10px 12px', borderRadius: 4,
                    background: activeRun === r.id ? 'var(--paper-soft)' : 'transparent',
                    border: activeRun === r.id ? '1px solid var(--ink-mute)' : 'var(--hairline)',
                    cursor: 'pointer', fontFamily: 'var(--font-ui)',
                    display: 'grid', gridTemplateColumns: '1fr auto auto',
                    gap: 10, alignItems: 'center'
                  }}>
                    <div>
                      <div style={{ fontSize: 12.5, color: 'var(--ink)' }}>
                        {r.corpus}  <span className="mono" style={{ color: 'var(--ink-faint)' }}>· {r.id}</span>
                      </div>
                      <div style={{ fontSize: 10.5, color: 'var(--ink-mute)', marginTop: 3 }}>
                        {r.started}  ·  {r.duration}
                      </div>
                    </div>
                    <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>
                      {r.b.passed}/{r.b.total}
                    </span>
                    <span style={{ fontSize: 11,
                      color: win ? 'var(--success)' : 'var(--warning)',
                      fontFamily: 'var(--font-mono)' }}>
                      {win ? '+' : ''}{r.delta.passed}
                    </span>
                  </button>
                );
              })}
            </div>
          </BnSection>

          {/* Run new */}
          <div style={{ padding: '14px 16px', background: 'var(--paper-soft)',
                         border: 'var(--hairline)', borderRadius: 6 }}>
            <div style={{ fontSize: 9.5, letterSpacing: '0.16em', color: 'var(--ink-faint)',
                           textTransform: 'uppercase', marginBottom: 8 }}>New run</div>
            <p style={{ fontSize: 11.5, color: 'var(--ink-mute)', margin: '0 0 10px',
                         lineHeight: 1.55 }}>
              Will execute every task on <strong style={{ color: 'var(--ink-soft)', fontWeight: 500 }}>
              {corpus.label}</strong> twice — first without sensei, then with sensei + MCPs enabled.
            </p>
            <button style={{
              padding: '8px 14px', fontSize: 12, background: 'var(--ink)',
              color: 'var(--paper)', borderRadius: 5, border: 'none',
              cursor: 'pointer', width: '100%', fontFamily: 'var(--font-ui)'
            }}>Run benchmark  ({corpus.tasks} tasks · ~{Math.ceil(corpus.tasks*2.5)}m)</button>
          </div>
        </div>

        {/* Right: run detail */}
        <div style={{ overflow: 'auto', padding: '24px 32px' }}>
          {/* Run header */}
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 10.5, letterSpacing: '0.18em', color: 'var(--ink-mute)',
                           textTransform: 'uppercase', marginBottom: 6 }}>
              Run {run.id}  ·  {run.corpus}  ·  {run.started}
            </div>
            <h2 className="display" style={{ fontSize: 22, fontWeight: 400, margin: 0,
                                              color: 'var(--ink)' }}>{run.verdict}</h2>
          </div>

          {/* A vs B card */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 24px 1fr',
                         gap: 12, marginBottom: 22 }}>
            <BnRunCard side="A" data={run.a} accent="var(--ink-mute)"/>
            <div style={{ display: 'flex', flexDirection: 'column',
                           alignItems: 'center', justifyContent: 'center',
                           color: 'var(--ink-faint)' }}>
              <span className="display" style={{ fontSize: 26, lineHeight: 1 }}>vs</span>
            </div>
            <BnRunCard side="B" data={run.b} accent="var(--accent)" highlight/>
          </div>

          {/* Delta strip */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)',
                         gap: 12, marginBottom: 22 }}>
            <BnDelta label="passed"  v={run.delta.passed}  unit="" good={run.delta.passed > 0}/>
            <BnDelta label="score"   v={run.delta.score}   unit="" pct good={run.delta.score > 0}/>
            <BnDelta label="tool calls" v={run.delta.toolCalls} unit="" good={run.delta.toolCalls < 0} invert/>
            <BnDelta label="tokens"  v={run.delta.tokens}   unit="" good={run.delta.tokens < 0} invert k/>
          </div>

          {/* Task table */}
          <div style={{ marginBottom: 14 }}>
            <h3 className="display" style={{ fontSize: 15, fontWeight: 400,
                                              margin: '0 0 4px', color: 'var(--ink)' }}>
              Per-task results
            </h3>
            <p style={{ fontSize: 11.5, color: 'var(--ink-mute)', margin: 0 }}>
              {B.taskBreakdown.length} of {run.b.total} tasks shown.
            </p>
          </div>
          <div style={{ border: 'var(--hairline)', borderRadius: 6, overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr 70px 70px 1.4fr',
                           padding: '8px 14px', background: 'var(--paper-soft)',
                           borderBottom: 'var(--hairline)',
                           fontSize: 9.5, letterSpacing: '0.14em', color: 'var(--ink-faint)',
                           textTransform: 'uppercase' }}>
              <span>id</span><span>task</span>
              <span style={{ textAlign: 'center' }}>without</span>
              <span style={{ textAlign: 'center' }}>with</span>
              <span>note</span>
            </div>
            {B.taskBreakdown.map((t, i) => (
              <div key={t.id} style={{
                display: 'grid', gridTemplateColumns: '60px 1fr 70px 70px 1.4fr',
                padding: '10px 14px',
                borderBottom: i < B.taskBreakdown.length-1 ? 'var(--hairline)' : 'none',
                fontSize: 12, alignItems: 'center'
              }}>
                <span className="mono" style={{ color: 'var(--ink-mute)' }}>{t.id}</span>
                <span style={{ color: 'var(--ink)' }}>{t.title}</span>
                <BnPF v={t.a}/>
                <BnPF v={t.b}/>
                <span style={{ color: 'var(--ink-mute)', fontSize: 11.5 }}>{t.note}</span>
              </div>
            ))}
          </div>
        </div>
      </div>
    </div>
  );
}

function BnRunCard({ side, data, accent, highlight }) {
  return (
    <div style={{
      padding: '16px 18px', borderRadius: 6,
      background: highlight ? 'var(--paper-soft)' : 'transparent',
      border: highlight ? '1px solid var(--accent)' : 'var(--hairline)'
    }}>
      <div style={{ display: 'flex', alignItems: 'baseline',
                     justifyContent: 'space-between', marginBottom: 12 }}>
        <span className="display" style={{ fontSize: 28, color: accent, lineHeight: 1 }}>{side}</span>
        <span style={{ fontSize: 9.5, letterSpacing: '0.14em', color: accent,
                        textTransform: 'uppercase' }}>
          {side === "A" ? "without sensei" : "with sensei"}
        </span>
      </div>
      <div className="mono" style={{ fontSize: 11.5, color: 'var(--ink-soft)',
                                       marginBottom: 14 }}>{data.label}</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr',
                     gap: '6px 14px', fontSize: 11.5 }}>
        <span style={{ color: 'var(--ink-faint)' }}>passed</span>
        <span className="mono" style={{ color: 'var(--ink)' }}>
          {data.passed} / {data.total}
        </span>
        <span style={{ color: 'var(--ink-faint)' }}>score</span>
        <span className="mono" style={{ color: 'var(--ink)' }}>
          {(data.score * 100).toFixed(0)}%
        </span>
        <span style={{ color: 'var(--ink-faint)' }}>tool calls</span>
        <span className="mono" style={{ color: 'var(--ink-soft)' }}>{data.toolCalls}</span>
        <span style={{ color: 'var(--ink-faint)' }}>tokens</span>
        <span className="mono" style={{ color: 'var(--ink-soft)' }}>
          {(data.tokens/1000).toFixed(0)}k
        </span>
      </div>
    </div>
  );
}

function BnDelta({ label, v, good, pct, k, invert }) {
  const color = v === 0 ? 'var(--ink-mute)' :
                good ? 'var(--success)' : 'var(--warning)';
  const sign = v > 0 ? '+' : '';
  let display;
  if (pct) display = `${sign}${(v*100).toFixed(0)}%`;
  else if (k) display = `${sign}${(v/1000).toFixed(0)}k`;
  else display = `${sign}${v}`;
  return (
    <div style={{ padding: '12px 14px', background: 'var(--paper-soft)',
                   border: 'var(--hairline)', borderRadius: 5 }}>
      <div style={{ fontSize: 9.5, letterSpacing: '0.14em', color: 'var(--ink-faint)',
                     textTransform: 'uppercase', marginBottom: 6 }}>{label}</div>
      <div className="mono" style={{ fontSize: 18, color, lineHeight: 1 }}>{display}</div>
    </div>
  );
}

function BnPF({ v }) {
  return (
    <span style={{ textAlign: 'center',
      color: v === "pass" ? 'var(--success)' : 'var(--warning)',
      fontSize: 10.5, letterSpacing: '0.12em', textTransform: 'uppercase',
      fontFamily: 'var(--font-mono)' }}>{v}</span>
  );
}

function BnSection({ title, children }) {
  return (
    <section style={{ marginBottom: 24 }}>
      <h3 className="display" style={{ fontSize: 15, fontWeight: 400, margin: '0 0 10px',
                                        color: 'var(--ink)' }}>{title}</h3>
      {children}
    </section>
  );
}

// ─── Layout B: Lab notebook ────────────────────────────────
function BenchmarkRunnerNotebook() {
  const B = window.EXT_DATA.benchmark;
  const run = B.runs[0];
  const corpus = B.corpora.find(c => c.id === run.corpus);

  return (
    <div className="sensei" data-screen-label="Benchmark · Notebook"
         style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column',
                  background: 'var(--paper)', overflow: 'hidden' }}>
      <BnHero
        subtitle="Benchmark · run report"
        title={`${corpus.label} · ${run.id}`}
        blurb={`Two runs of the same corpus. ${run.a.label} vs ${run.b.label}.`}
        stats={[
          { n: run.duration, l: "duration", mono: true },
          { n: run.b.total, l: "tasks", mono: true },
          { n: `+${(run.delta.score*100).toFixed(0)}%`, l: "score lift", mono: true, accent: true },
        ]}
      />

      <div style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: '32px 60px 80px',
                     maxWidth: 1100, margin: '0 auto', width: '100%' }}>

        {/* Abstract */}
        <NbBlock label="Abstract">
          <p style={{ fontSize: 14, color: 'var(--ink-soft)', lineHeight: 1.7,
                       margin: 0, maxWidth: 760 }}>
            We executed the <strong>{corpus.label}</strong> corpus ({corpus.tasks} tasks)
            twice with <span className="mono" style={{ color: 'var(--accent)' }}>{run.a.label.split('·')[0].trim()}</span>:
            once with sensei's tools, MCPs and memory disabled (run A), and once with them
            fully active (run B). <strong>{run.verdict}</strong>
          </p>
        </NbBlock>

        {/* Setup */}
        <NbBlock label="Setup · what changed between A and B">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <div style={{ padding: '14px 16px', borderRadius: 5,
                           background: 'var(--paper-soft)', border: 'var(--hairline)' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8,
                             marginBottom: 8 }}>
                <span className="display" style={{ fontSize: 20, color: 'var(--ink-mute)' }}>A</span>
                <span style={{ fontSize: 9.5, letterSpacing: '0.14em',
                                color: 'var(--ink-mute)', textTransform: 'uppercase' }}>
                  baseline
                </span>
              </div>
              <ul style={{ fontSize: 12.5, color: 'var(--ink-soft)', lineHeight: 1.7,
                            margin: 0, paddingLeft: 18 }}>
                <li>Bare assistant. No sensei extensions, no MCPs.</li>
                <li>No project memory loaded.</li>
                <li>Single fallback model.</li>
                <li>Tool budget: assistant defaults.</li>
              </ul>
            </div>
            <div style={{ padding: '14px 16px', borderRadius: 5,
                           background: 'var(--paper-soft)', border: '1px solid var(--accent)' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8,
                             marginBottom: 8 }}>
                <span className="display" style={{ fontSize: 20, color: 'var(--accent)' }}>B</span>
                <span style={{ fontSize: 9.5, letterSpacing: '0.14em',
                                color: 'var(--accent)', textTransform: 'uppercase' }}>
                  with sensei
                </span>
              </div>
              <ul style={{ fontSize: 12.5, color: 'var(--ink-soft)', lineHeight: 1.7,
                            margin: 0, paddingLeft: 18 }}>
                <li>All installed extensions enabled.</li>
                <li>Project memories surfaced on context match.</li>
                <li>Fallback chain · MOE on for high-stakes calls.</li>
                <li>MCPs: tsserver, fs-read, react-devtools, session-replay.</li>
              </ul>
            </div>
          </div>
        </NbBlock>

        {/* Headline numbers */}
        <NbBlock label="Headline numbers">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)',
                         gap: 12, marginBottom: 12 }}>
            <BnDelta label="passed"     v={run.delta.passed} good={run.delta.passed > 0}/>
            <BnDelta label="score"      v={run.delta.score} pct good={run.delta.score > 0}/>
            <BnDelta label="tool calls" v={run.delta.toolCalls} good={run.delta.toolCalls < 0} invert/>
            <BnDelta label="tokens"     v={run.delta.tokens} k good={run.delta.tokens < 0} invert/>
          </div>
          <p style={{ fontSize: 12, color: 'var(--ink-mute)', margin: 0, lineHeight: 1.6 }}>
            Sensei produced more passes with fewer tool calls and fewer tokens —
            efficiency improved on every axis we measure.
          </p>
        </NbBlock>

        {/* Per-task narrative table */}
        <NbBlock label="Per-task results">
          <div style={{ border: 'var(--hairline)', borderRadius: 6, overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr 60px 60px 1.6fr',
                           padding: '8px 14px', background: 'var(--paper-soft)',
                           borderBottom: 'var(--hairline)',
                           fontSize: 9.5, letterSpacing: '0.14em', color: 'var(--ink-faint)',
                           textTransform: 'uppercase' }}>
              <span>id</span><span>task</span>
              <span style={{ textAlign: 'center' }}>A</span>
              <span style={{ textAlign: 'center' }}>B</span>
              <span>commentary</span>
            </div>
            {B.taskBreakdown.map((t, i) => (
              <div key={t.id} style={{
                display: 'grid', gridTemplateColumns: '60px 1fr 60px 60px 1.6fr',
                padding: '10px 14px',
                borderBottom: i < B.taskBreakdown.length-1 ? 'var(--hairline)' : 'none',
                fontSize: 12.5, alignItems: 'center'
              }}>
                <span className="mono" style={{ color: 'var(--ink-mute)' }}>{t.id}</span>
                <span style={{ color: 'var(--ink)' }}>{t.title}</span>
                <BnPF v={t.a}/>
                <BnPF v={t.b}/>
                <span style={{ color: 'var(--ink-mute)', fontSize: 11.5 }}>{t.note}</span>
              </div>
            ))}
          </div>
        </NbBlock>

        {/* Where sensei made the difference */}
        <NbBlock label="Where sensei made the difference">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <div style={{ padding: '14px 16px', background: 'var(--paper-soft)',
                           border: 'var(--hairline)', borderRadius: 5 }}>
              <div style={{ fontSize: 9.5, letterSpacing: '0.14em', color: 'var(--accent)',
                             textTransform: 'uppercase', marginBottom: 7 }}>
                Tasks won by skill / agent triggers
              </div>
              <ul style={{ fontSize: 12.5, color: 'var(--ink-soft)', lineHeight: 1.7,
                            margin: 0, paddingLeft: 18 }}>
                <li><strong>t01</strong> — react-perf-watch caught render-thrash.</li>
                <li><strong>t02</strong> — boundary memory surfaced before the touch.</li>
                <li><strong>t04</strong> — migration-runner agent generated the SQL.</li>
                <li><strong>t08</strong> — doc-drift skill caught README mismatch.</li>
              </ul>
            </div>
            <div style={{ padding: '14px 16px', background: 'var(--paper-soft)',
                           border: 'var(--hairline)', borderRadius: 5 }}>
              <div style={{ fontSize: 9.5, letterSpacing: '0.14em', color: 'var(--warning)',
                             textTransform: 'uppercase', marginBottom: 7 }}>
                Where both still failed
              </div>
              <ul style={{ fontSize: 12.5, color: 'var(--ink-soft)', lineHeight: 1.7,
                            margin: 0, paddingLeft: 18 }}>
                <li><strong>t06</strong> — borrow-check fix in canvas/event.rs.
                    No memory exists yet for this pattern. Tagged as a candidate.</li>
              </ul>
            </div>
          </div>
        </NbBlock>

        {/* Reproduce */}
        <NbBlock label="Reproduce">
          <pre className="mono" style={{ fontSize: 12, color: 'var(--ink-soft)',
            background: 'var(--paper-soft)', border: 'var(--hairline)',
            borderRadius: 5, padding: '12px 14px', margin: 0,
            lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>
{`$ sensei bench run --corpus ${corpus.repo} \\
    --model claude-sonnet-4.5 \\
    --baseline none \\
    --variant default

# resume:
$ sensei bench resume ${run.id}`}
          </pre>
        </NbBlock>

        <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
          <button style={{
            padding: '9px 18px', fontSize: 12, background: 'var(--ink)',
            color: 'var(--paper)', borderRadius: 5, border: 'none',
            cursor: 'pointer', fontFamily: 'var(--font-ui)'
          }}>Re-run on latest sensei</button>
          <button style={{
            padding: '9px 18px', fontSize: 12, background: 'transparent',
            color: 'var(--ink-soft)', borderRadius: 5,
            border: '1px solid var(--ink-mute)', cursor: 'pointer',
            fontFamily: 'var(--font-ui)'
          }}>Export markdown</button>
          <button style={{
            padding: '9px 14px', fontSize: 12, background: 'transparent',
            color: 'var(--ink-mute)', borderRadius: 5, border: 'none',
            cursor: 'pointer', fontFamily: 'var(--font-ui)'
          }}>Share with collective →</button>
        </div>
      </div>
    </div>
  );
}

function NbBlock({ label, children }) {
  return (
    <section style={{ marginBottom: 32 }}>
      <div style={{ fontSize: 9.5, letterSpacing: '0.18em', color: 'var(--ink-faint)',
                     textTransform: 'uppercase', marginBottom: 12,
                     paddingBottom: 8, borderBottom: 'var(--hairline)' }}>
        {label}
      </div>
      {children}
    </section>
  );
}

window.BenchmarkRunnerDashboard = BenchmarkRunnerDashboard;
window.BenchmarkRunnerNotebook = BenchmarkRunnerNotebook;
