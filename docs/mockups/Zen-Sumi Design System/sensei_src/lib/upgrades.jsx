...
const KIND_META = {
  ...
  lint:    { glyph: "禁", label: "Lint",    color: "var(--ink-soft)"}
};
  ...
  return (
    <div className="sensei" data-screen-label="Observatory · Upgrades"
      ...
      <div style={{ padding: '22px 36px 18px', borderBottom: 'var(--hairline)',
        ...
        <div className="kanji" style={{ fontSize: 42, color: 'var(--accent)', lineHeight: 1 }}>贈</div>
...
function UpgradeDetail({ item }) {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      ...
      <div className="mono" style={{ fontSize: 12, color: 'var(--ink-mute)', marginBottom: 22 }}>
        {item.name}
      </div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <ol style={{ margin: 0, padding: '0 0 0 18px',
          ...
          {item.preview.map((p, i) => (
            <li key={i} style={{ fontSize: 12.5, color: 'var(--ink-soft)', lineHeight: 1.5 }}>
              {p}
            </li>
          ))}
        </ol>
      ...
      {item.conflicts.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 10, letterSpacing: '0.14em', color: 'var(--warning)',
    ...
    <button onClick={onClick}
      ...
      {glyph && (
        <span className="kanji" style={{ fontSize: 12,
                      color: active ? 'var(--accent)' : 'var(--ink-mute)' }}>{glyph}</span>
      )}
      {children}
    </button>
  );
