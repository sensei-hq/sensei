...
function StatusDot({ ftr, warn }) {
  const color = warn ? 'var(--warning)' :
                ftr >= 0.8 ? 'var(--success)' :
                ftr >= 0.6 ? 'var(--ink-mute)' : 'var(--warning)';
  ...
  return (
    <div className="sensei" data-screen-label="Projects · Grid"
      ...
      <div style={{ padding: '28px 56px 20px',
        ...
        <div className="kanji" style={{ fontSize: 46, color: 'var(--accent)', lineHeight: 1 }}>場</div>
        <div style={{ flex: 1 }}>
      ...
      <main style={{ flex: 1, overflow: 'auto', padding: '24px 56px 40px',
        ...
        {filtered.length === 0 && (
          <div style={{ gridColumn: '1/-1', textAlign: 'center',
                         padding: '40px 0', fontSize: 12, color: 'var(--ink-mute)' }}>
            No projects match.
          </div>
        )}
...
function ProjectCard({ p, onOpen }) {
  ...
  return (
    ...
    onMouseEnter={(e) => { if (onOpen) e.currentTarget.style.background = 'var(--paper-mute)'; }}
    onMouseLeave={(e) => { if (onOpen) e.currentTarget.style.background = 'var(--paper-soft)'; }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <span className="kanji" style={{ fontSize: 20, color: 'var(--accent)', lineHeight: 1,
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)', marginTop: 1 }}>
            {p.client}
          </div>
        </div>
        {p.status !== "active" && (
...
function Stat({ label, value, tone = 'var(--ink)' }) {
  return (
    <div>
      <div style={{ fontSize: 9, letterSpacing: '0.12em', color: 'var(--ink-mute)',
    ...
    <div className="sensei" data-screen-label="Projects · Command palette"
      ...
      <div style={{ position: 'absolute', left: '50%', top: 110,
        ...
        <div style={{ padding: '14px 18px', borderBottom: 'var(--hairline)',
          ...
          <span className="kanji" style={{ fontSize: 16, color: 'var(--accent)' }}>探</span>
  ...
  return (
    <div style={{ marginBottom: 6 }}>
      <div style={{ padding: '8px 18px 4px', fontSize: 9.5, letterSpacing: '0.16em',
        ...
        {label}
        <span className="mono" style={{ marginLeft: 6, color: 'var(--ink-faint)' }}>· {count}</span>
      </div>
      {children}
    </div>
  );
...
function PaletteRow({ kanji, title, sub, trail, highlight, warn }) {
  return (
    <div style={{
      ...
      <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>{trail}</span>
    </div>
  );
}
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '260px 1fr', minHeight: 0 }}>
        ...
        <main style={{ overflow: 'auto', padding: '32px 48px' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 8 }}>
            <span className="kanji" style={{ fontSize: 32, color: 'var(--accent)' }}>場</span>
            <div>
              <div style={{ fontSize: 10.5, letterSpacing: '0.18em', color: 'var(--ink-mute)',
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ padding: '2px 10px 6px',
                     fontSize: 9.5, letterSpacing: '0.16em', color: 'var(--ink-mute)',
        ...
        return (
          <button key={p.id} onClick={() => setSelected(p.id)}
            ...
            <span className="kanji" style={{ fontSize: 12, width: 12,
                          color: on ? 'var(--accent)' : 'var(--ink-mute)' }}>{p.kanji}</span>
  ...
  return (
    <button onClick={onClick}
      style={{
        ...
        background: selected ? 'var(--paper)' : 'var(--paper-soft)',
      ...
      {big && (
        <div style={{ display: 'flex', gap: 14, fontSize: 10.5,
                       color: 'var(--ink-mute)', marginTop: 'auto' }} className="mono">
