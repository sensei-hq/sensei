...
const fieldLabel = {
  ...
  textTransform: 'uppercase', marginBottom: 7, fontFamily: 'var(--font-ui)'
};
const fieldBox = {
...
const monoBox = {
  ...fieldBox, fontFamily: 'var(--font-mono)', fontSize: 12,
...
function SkHero({ skill, layout }) {
  return (
    <div style={{ padding: '22px 36px 18px', borderBottom: 'var(--hairline)',
      ...
      <div className="kanji" style={{ fontSize: 42, color: 'var(--accent)', lineHeight: 1 }}>技</div>
...
function SkAssembledPreview({ skill }) {
  ...
  return (
    <div style={{ padding: '18px 22px', background: 'var(--paper-soft)',
...
{`# system\n${a.systemSnippet}\n\n# memory\n${a.memorySnippet}\n\n# tools available\n${a.toolList.map(t => `  · ${t}`).join('\n')}`}
      ...
      <div style={{ borderTop: 'var(--hairline)', marginTop: 14, paddingTop: 10,
      ...
      <div style={{ height: 4, background: 'var(--paper-4)', borderRadius: 2,
...
function SkTriggerRow({ t }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '110px 70px 1fr auto',
      ...
      <span style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{t.op}</span>
      <span className="mono" style={{ fontSize: 12, color: 'var(--ink)' }}>{t.value}</span>
      <button style={{ fontSize: 14, color: 'var(--ink-faint)', background: 'transparent',
...
function SkToolRow({ tool }) {
  return (
    <label style={{ display: 'grid', gridTemplateColumns: '20px 1fr auto',
                     ...
                     background: tool.allowed ? 'var(--paper-soft)' : 'transparent',
                     border: 'var(--hairline)' }}>
      <span style={{
        width: 14, height: 14, borderRadius: 3,
        border: '1px solid ' + (tool.allowed ? 'var(--accent)' : 'var(--ink-faint)'),
        background: tool.allowed ? 'var(--accent)' : 'transparent',
        display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
        color: 'var(--paper)', fontSize: 10
      }}>{tool.allowed ? '✓' : ''}</span>
      <span className="mono" style={{ fontSize: 12,
        color: tool.allowed ? 'var(--ink)' : 'var(--ink-mute)' }}>{tool.label}</span>
      <span style={{ fontSize: 10, color: tool.allowed ? 'var(--success)' : 'var(--ink-faint)',
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'grid',
          ...
          <SkSection title="Identity">
            ...
            <SkField label="Tags">
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                {skill.tags.map(t => (
                  <span key={t} style={{ fontSize: 11, color: 'var(--ink-soft)',
                    background: 'var(--paper-mute)', padding: '4px 10px', borderRadius: 3,
                    fontFamily: 'var(--font-mono)' }}>{t} <span style={{ color: 'var(--ink-faint)', marginLeft: 4 }}>×</span></span>
                ))}
                <button style={{ fontSize: 11, color: 'var(--ink-mute)', background: 'transparent',
                  border: 'var(--hairline)', borderRadius: 3, padding: '4px 10px',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {skill.examples.map((ex, i) => (
                <div key={i} style={{ border: 'var(--hairline)', borderRadius: 6,
                  background: 'var(--paper-soft)', padding: '12px 14px' }}>
                  <div style={{ fontSize: 9.5, color: 'var(--ink-faint)', letterSpacing: '0.14em',
                                 textTransform: 'uppercase', marginBottom: 5 }}>Input</div>
                  <div style={{ fontSize: 13, color: 'var(--ink)', lineHeight: 1.55,
                                 marginBottom: 10 }}>{ex.in}</div>
                  <div style={{ fontSize: 9.5, color: 'var(--accent)', letterSpacing: '0.14em',
                                 textTransform: 'uppercase', marginBottom: 5 }}>Sensei's response</div>
                  <div style={{ fontSize: 13, color: 'var(--ink)', lineHeight: 1.55 }}>{ex.out}</div>
                </div>
              ))}
              <button style={{ fontSize: 11, color: 'var(--ink-mute)',
                background: 'transparent', border: '1px dashed var(--paper-4)',
                borderRadius: 4, padding: '10px', cursor: 'pointer', textAlign: 'center' }}>
                + example pair
              </button>
            </div>
          </SkSection>
              ...
              <SkField label="Sources">
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                  {skill.evidence.sources.map(s => (
                    <span key={s} className="mono" style={{ fontSize: 10.5,
                      color: 'var(--ink-soft)', background: 'var(--paper-mute)',
                      padding: '3px 8px', borderRadius: 3 }}>{s}</span>
                  ))}
                </div>
              </SkField>
              <SkField label="Memory refs">
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                  {skill.evidence.memoryRefs.map(m => (
                    <span key={m} className="mono" style={{ fontSize: 10.5,
                      color: 'var(--accent)', background: 'var(--paper-mute)',
        ...
        <div style={{ overflow: 'auto', padding: '24px 28px',
          ...
          <div style={{ position: 'sticky', top: 0 }}>
            ...
            {/* Validation panel */}
            <div style={{ marginTop: 18, padding: '16px 20px', borderRadius: 6,
                           border: 'var(--hairline)' }}>
              <div style={{ fontSize: 9.5, letterSpacing: '0.16em', color: 'var(--ink-faint)',
                             textTransform: 'uppercase', marginBottom: 12 }}>
                Health check
              ...
              {[
                ...
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '14px 1fr',
                                       ...
                                       fontSize: 12, color: c.ok ? 'var(--ink-soft)' : 'var(--warning)' }}>
            ...
            <div style={{ marginTop: 18, padding: '16px 20px', borderRadius: 6,
              ...
              <select style={{ ...fieldBox, marginBottom: 10 }}>
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'grid',
        ...
        <div style={{ borderRight: 'var(--hairline)', display: 'flex',
          ...
          <div style={{ flex: 1, overflow: 'auto', display: 'grid',
            ...
            <div style={{ background: 'var(--paper-soft)', borderRight: 'var(--hairline)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16,
                         padding: '6px 18px', borderTop: 'var(--hairline)',
                         background: 'var(--paper-soft)', fontSize: 10,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '5px 12px',
        ...
        {rows.map(([l, v], i) => (
          <React.Fragment key={i}>
            <span className="mono" style={{ color: 'var(--ink-mute)' }}>{l}</span>
