...
const outcomeTone = o =>
  ...
  o === 'failed'  ? 'var(--accent)'   : 'var(--ink-mute)';
...
function PLogActionBadge({ type }) {
  const map = {
    check:    { label: 'CHECK',    color: 'var(--ink-mute)', bg: 'var(--paper-mute)' },
    ...
    <div style={{ borderBottom: 'var(--hairline)' }}>
      ...
      {expanded && (
        <div style={{
          ...
          background: 'var(--paper-soft)', borderRadius: 5, border: 'var(--hairline)'
        }}>
          {trace.out && (
            <div style={{ marginBottom: trace.err ? 10 : 0 }}>
              <div style={{ fontSize: 9.5, letterSpacing: '0.14em', textTransform: 'uppercase',
                              color: 'var(--ink-faint)', marginBottom: 4 }}>stdout</div>
          ...
          {trace.err && (
            <div>
              <div style={{ fontSize: 9.5, letterSpacing: '0.14em', textTransform: 'uppercase',
                              color: 'var(--accent)', marginBottom: 4 }}>stderr</div>
              <pre className="mono" style={{ margin: 0, fontSize: 11.5,
                              color: 'var(--accent)', lineHeight: 1.6 }}>{trace.err}</pre>
            </div>
          )}
          {trace.fix_attempted && (
            <div style={{
              ...
              <div style={{ fontSize: 9.5, letterSpacing: '0.14em', textTransform: 'uppercase',
                              color: 'var(--warning)', marginBottom: 4 }}>auto-fix attempted</div>
              <pre className="mono" style={{ margin: 0, fontSize: 11.5,
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '20px 28px 24px',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, minHeight: 0 }}>
            <div style={{ fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase',
                            color: 'var(--ink-mute)' }}>Issue preview · anonymized</div>
            ...
            {/* Body */}
            <pre className="mono" style={{ flex: 1, margin: 0, background: 'var(--paper-soft)',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
              ...
              <div style={{ fontSize: 9.5, letterSpacing: '0.14em', textTransform: 'uppercase',
                              color: 'var(--ink-mute)', marginBottom: 9 }}>
                Included in report
              </div>
              {[
                ['Project', project.name],
            ...
            <div>
              ...
              <textarea value={context} onChange={e => setContext(e.target.value)}
                          ...
                          style={{ width: '100%', height: 96, resize: 'none', padding: '8px 10px',
                                    ...
                                    borderRadius: 5, fontSize: 12.5, color: 'var(--ink)',
                                    fontFamily: 'inherit', lineHeight: 1.5 }}/>
            </div>

            {/* Privacy note */}
            <div style={{ fontSize: 10.5, color: 'var(--ink-mute)', lineHeight: 1.7 }}>
              Paths like{' '}
              <code className="mono" style={{ background: 'var(--paper-mute)',
...
function ProjLogs({ project, scope = "project" }) {
  ...
  return (
    <div style={{ height: '100%', position: 'relative',
      ...
      {/* ── Session list ── */}
      <aside style={{ borderRight: 'var(--hairline)', background: 'var(--paper-soft)',
                       display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '22px 20px 14px', borderBottom: 'var(--hairline)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span className="kanji" style={{ fontSize: 18, color: 'var(--accent)' }}>診</span>
            <span style={{ fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase',
                             color: 'var(--ink-mute)' }}>diagnostic logs</span>
          </div>
          <div style={{ fontSize: 10.5, color: 'var(--ink-faint)', marginTop: 6 }}>
            {collective ? (
              <>every session sensei has run · <span className="mono" style={{ color: 'var(--ink-mute)' }}>{PLOG_SESSIONS.length} total</span></>
            ) : (
              <>scoped to <span className="mono" style={{ color: 'var(--ink-mute)' }}>{project?.name}</span></>
            )}
          </div>
            ...
            return (
              <div key={date}>
                <div onClick={() => toggleDate(date)}
                  ...
                  <span style={{ fontSize: 9, color: 'var(--ink-faint)', lineHeight: 1,
                ...
                {open && groups.map(({ mod, sessions }) => (
                  <div key={mod}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6,
                      ...
                      <span className="kanji" style={{ fontSize: 11, color: 'var(--ink-faint)',
                                    lineHeight: 1 }}>
                        ...
                        <div key={s.id}
                          ...
                          {isCurrent && (
                            <div style={{ fontSize: 9, letterSpacing: '0.16em',
                                            ...
                                            color: 'var(--accent)', marginBottom: 2 }}>current</div>
                          )}
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                            ...
                            {collective && s.project && (
                              <span style={{ marginLeft: 'auto', display: 'inline-flex',
                                              ...
                                              color: 'var(--ink-mute)' }}>
                                <span className="kanji" style={{ fontSize: 10, color: 'var(--ink-faint)' }}>
                                  {s.project.kanji}
                                </span>
        ...
        <div style={{ padding: '10px 20px', borderTop: 'var(--hairline)',
                       fontSize: 10, color: 'var(--ink-faint)', lineHeight: 1.6 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between',
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                <span className="kanji" style={{ fontSize: 13, color: 'var(--ink-mute)' }}>
                ...
                {collective && session.project && (
                  <>
                    ...
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5,
                      ...
                      <span className="kanji" style={{ fontSize: 12, color: 'var(--accent)' }}>
          ...
          <div style={{ display: 'flex', gap: 28 }}>
            {stats.map(st => (
              <div key={st.label} style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                <div style={{ fontSize: 9.5, letterSpacing: '0.14em', textTransform: 'uppercase',
                                color: 'var(--ink-mute)' }}>{st.label}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '74px 168px 1fr 60px 40px',
                       gap: 14, padding: '8px 36px', background: 'var(--paper-soft)',
