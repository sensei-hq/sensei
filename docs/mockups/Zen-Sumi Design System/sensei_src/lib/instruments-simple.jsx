      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '300px 1fr',
        ...
        <aside style={{ borderRight: 'var(--hairline)', background: 'var(--paper-soft)',
          ...
          <div style={{ padding: '10px 14px', borderTop: 'var(--hairline)',
            ...
            <button style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>+ add MCP</button>
          </div>
        </aside>
  ...
  return (
    <div style={{ marginBottom: 2 }}>
      <button onClick={onToggle}
        ...
        {!mcp.installed && (
          <span style={{ fontSize: 9, color: 'var(--warning)',
...
function ToolLine({ tool, active, onClick }) {
  ...
  const kindColor = isAction ? "var(--accent)" : "var(--matcha)";
    ...
    <div>
      ...
      <div style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10,
          ...
          <span style={{ flex: 1 }}/>
          {status === "running" && (
            <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>calling …</span>
          )}
          {status === "done" && (
