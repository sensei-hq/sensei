...
function AnatomyStageV2({ memory }) {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '4px auto 0',
      ...
      {memory.violated > 0 && (
        <p style={{ fontSize: 14, color: 'var(--ink-mute)', lineHeight: 1.7,
                     margin: '0 0 32px' }}>
          ...
          {memory.references.bad_example && (
            <> — most often in <span className="mono" style={{ color: 'var(--ink-soft)' }}>
              {memory.references.bad_example}
            </span></>
          )}.
        </p>
      ...
      {memory.violated === 0 && (
        <p style={{ fontSize: 14, color: 'var(--ink-mute)', lineHeight: 1.7,
                     margin: '0 0 32px' }}>
          Reinforced{" "}
          <span style={{ color: 'var(--ink-soft)' }}>{memory.reinforced} times</span>
      ...
      <div style={{ borderTop: 'var(--hairline)',
        ...
        <ObservationRow kanji={how.glyph} title="Surfaced as"
                        ...
                        sub={<span className="mono" style={{ fontSize: 11.5,
                                       color: 'var(--ink-soft)' }}>{how.target}</span>}/>
      ...
      {(memory.references.good_example || memory.references.bad_example) && (
        <div style={{ borderTop: 'var(--hairline)', padding: '20px 0',
          ...
          <div style={{ fontSize: 10, letterSpacing: '0.16em', color: 'var(--ink-faint)',
                         textTransform: 'uppercase', marginBottom: 4 }}>
            In the codebase
          </div>
          {memory.references.good_example && (
            <div className="mono" style={{ fontSize: 12, color: 'var(--success)' }}>
              ✓ {memory.references.good_example}
            </div>
          )}
          {memory.references.bad_example && (
  ...
  return (
    <div style={{ padding: '20px 24px 20px 0', display: 'flex', gap: 14,
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {sub && <div style={{ fontSize: 11.5, color: 'var(--ink-mute)',
