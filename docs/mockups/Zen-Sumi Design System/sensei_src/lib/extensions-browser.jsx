...
const SCOPE_META = {
  global:  { glyph: "球", label: "Global only",     color: "var(--ink-soft)" },
...
function ExtKindChip({ kind, active, count, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      border: active ? '1px solid var(--ink)' : '1px solid var(--paper-4)',
      ...
      <span className="mono" style={{ fontSize: 10,
        color: active ? 'var(--paper-mute)' : 'var(--ink-faint)' }}>{count}</span>
    </button>
  );
}
...
function ExtListRow({ ext, kind, active, onClick, projectScoped, projectId }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      padding: '14px 18px', borderBottom: 'var(--hairline)',
        ...
        {ext.installed && (
          <span style={{ width: 5, height: 5, borderRadius: '50%',
            background: isPinnedHere ? 'var(--accent)' :
                        isInherited  ? 'var(--success)' : 'var(--ink-mute)' }}/>
        )}
      </div>
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 4 }}>
          ...
          {projectScoped && isInherited && (
            <span style={{ fontSize: 9.5, color: 'var(--success)',
                            letterSpacing: '0.12em', textTransform: 'uppercase' }}>
              inherited
            </span>
          )}
      ...
      <div style={{ textAlign: 'right', paddingTop: 2 }}>
        ...
        <div className="mono" style={{ fontSize: 10, color: 'var(--ink-faint)', marginTop: 5 }}>
          {ext.downloads}
        </div>
      </div>
    </button>
...
function ExtDetail({ ext, kind, projectScoped, projectId, projectName }) {
  ...
  return (
    <div style={{ padding: '28px 32px 36px', overflow: 'auto', height: '100%' }}>
      {/* header */}
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 18,
                     paddingBottom: 22, borderBottom: 'var(--hairline)' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <h2 className="display" style={{ fontSize: 24, fontWeight: 400, margin: '0 0 6px',
                                            color: 'var(--ink)' }}>
            {ext.name}
          </h2>
          ...
          <div style={{ display: 'flex', gap: 14, marginTop: 12,
            ...
            <span>by <strong style={{ color: 'var(--ink-soft)', fontWeight: 500 }}>{ext.author}</strong></span>
            <span style={{ color: source.color }}>{source.label}</span>
            {ext.stars && <span className="mono">★ {ext.stars}</span>}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)',
                     gap: 20, padding: '22px 0', borderBottom: 'var(--hairline)' }}>
        <ExtProp label="Scope">
          <span className="kanji" style={{ fontSize: 14, color: scope.color, marginRight: 6 }}>
            {scope.glyph}
          </span>
        ...
        <ExtProp label="Tags">
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {ext.tags.map(t => (
              <span key={t} style={{ fontSize: 10.5, color: 'var(--ink-soft)',
                background: 'var(--paper-mute)', padding: '2px 8px', borderRadius: 3,
        ...
        <ExtProp label={projectScoped ? "Project status" : "Pinned to"}>
          {projectScoped ? (
            ...
            isInherited  ? <span style={{ color: 'var(--success)', fontSize: 13 }}>inherited from global</span> :
                            <span style={{ color: 'var(--ink-mute)', fontSize: 13 }}>not active here</span>
            ...
            <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
              {ext.pinnedTo.map(p => (
                <span key={p} style={{ fontSize: 11, color: 'var(--ink-soft)',
                  background: 'var(--paper-mute)', padding: '2px 8px', borderRadius: 3 }}>{p}</span>
              ))}
            </div>
          )}
        </ExtProp>
      ...
      {/* evidence */}
      {ext.evidence != null && (
        <div style={{ padding: '20px 0', borderBottom: 'var(--hairline)' }}>
          <div style={{ fontSize: 10, letterSpacing: '0.18em', color: 'var(--ink-mute)',
          ...
          <div style={{ fontSize: 13, color: 'var(--ink-soft)', lineHeight: 1.6 }}>
            <span className="display" style={{ fontSize: 22, color: 'var(--ink)',
                                                 marginRight: 7 }}>{ext.evidence}</span>
      ...
      {/* CTA */}
      <div style={{ padding: '22px 0 0', display: 'flex', gap: 10,
                     alignItems: 'center' }}>
        {ext.installed ? (
          <>
  ...
  return (
    <div className="sensei" data-screen-label={projectScoped ? "Extensions · Project" : "Extensions · Collective"}
      ...
      <div style={{ padding: '22px 36px 18px', borderBottom: 'var(--hairline)',
        ...
        <div className="kanji" style={{ fontSize: 42, color: 'var(--accent)', lineHeight: 1 }}>具</div>
        <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ padding: '12px 36px', borderBottom: 'var(--hairline)',
        ...
        {["all", "installed", "available"].map(f => (
          <button key={f} onClick={() => setInstalledFilter(f)} style={{
            ...
            background: installedFilter === f ? 'var(--paper-mute)' : 'transparent',
            color: installedFilter === f ? 'var(--ink)' : 'var(--ink-mute)',
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'grid',
        ...
        <div style={{ overflow: 'hidden' }}>
          {item ? (
            ...
            <div style={{ padding: 40, color: 'var(--ink-mute)', fontSize: 12 }}>
              Select an extension.
            </div>
          )}
        </div>
