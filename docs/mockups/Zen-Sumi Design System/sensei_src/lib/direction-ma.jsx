    ...
    <aside style={{
          ...
          {solutions.map(s => (
            <button key={s.id}
                    ...
                    style={{
                      ...
                      background: activeSolution === s.id ? 'var(--paper-mute)' : 'transparent',
                      color: activeSolution === s.id ? 'var(--ink)' : 'var(--ink-soft)',
                      fontSize: 13, transition: 'background .12s'
                    }}>
              <span className="kanji" style={{ fontSize: 14, color: activeSolution === s.id ? 'var(--accent)' : 'var(--ink-mute)', width: 16 }}>{s.kanji}</span>
              <span style={{ flex: 1 }}>{s.name}</span>
              <span className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)' }}>{pct(s.ftr)}</span>
            </button>
          ))}
        </div>
      </div>
          ...
          {PAGES.map(p => (
            <button key={p.id}
                    ...
                    style={{
                      ...
                      color: page === p.id ? 'var(--paper)' : 'var(--ink-soft)',
                      fontSize: 13, transition: 'background .12s'
                    }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 12, color: 'var(--ink-mute)' }}>
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          <div style={{ color: 'var(--ink-soft)', fontSize: 12 }}>Aiko</div>
...
function MaObservatory({ data, sol, setPage, setFocusedSession, applied, setApplied }) {
  ...
  return (
    <div style={{ padding: '56px 72px 80px', position: 'relative', maxWidth: 1100 }}>
      ...
      <div style={{ color: 'var(--ink-mute)', fontSize: 13, marginBottom: 56 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 24, marginBottom: 12, position: 'relative' }}>
        <div className="display" style={{ fontSize: 148, fontWeight: 300, lineHeight: 0.9,
          ...
          <span style={{ fontSize: 48, color: 'var(--ink-mute)', fontWeight: 300, marginLeft: 4 }}>%</span>
        </div>
        <div style={{ paddingBottom: 18 }}>
          <div style={{ fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase',
                        color: 'var(--ink-mute)', marginBottom: 6 }}>First try right</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span className="mono" style={{ fontSize: 13,
                      color: trendUp ? 'var(--success)' : 'var(--accent)' }}>
              {trendUp ? "↗" : "↘"} {delta >= 0 ? "+" : ""}{delta}%
            </span>
            <span style={{ fontSize: 12, color: 'var(--ink-mute)' }}>vs. last week</span>
          </div>
          <div style={{ marginTop: 14, color: trendUp ? 'var(--success)' : 'var(--accent)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 56 }}>
        <div>
          <div style={{ fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase',
                        color: 'var(--ink-mute)', marginBottom: 14 }}>Sensei says</div>
          <blockquote style={{ margin: 0 }}>
            <p className="display" style={{ fontSize: 30, fontWeight: 300, lineHeight: 1.25,
                                            margin: 0, color: 'var(--ink)', textWrap: 'balance' }}>
              {topCoach.koan}
            </p>
            <p style={{ fontSize: 14, color: 'var(--ink-soft)', lineHeight: 1.55, margin: '22px 0 0',
          ...
          <div style={{ marginTop: 28, display: 'flex', gap: 12, alignItems: 'center' }}>
            <button onClick={() => setApplied({...applied, [topCoach.id]: true})}
                    style={{
                      padding: '9px 18px', background: applied[topCoach.id] ? 'var(--success-soft)' : 'var(--ink)',
            ...
            <span style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{topCoach.impact}</span>
          </div>
        </div>

        <div>
          <div style={{ fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase',
                        color: 'var(--ink-mute)', marginBottom: 14 }}>Recent sessions</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 40, marginBottom: 8 }}>
        {[
          ...
          <div key={s.label}>
            <div style={{ fontSize: 11, color: 'var(--ink-mute)', letterSpacing: '0.08em',
            ...
            <div className="mono" style={{ fontSize: 10.5, color: s.good ? 'var(--success)' : 'var(--accent)',
...
function MaOverview({ data, setPage, setActiveSolution }) {
  return (
    <div style={{ padding: '56px 72px 80px', maxWidth: 1100 }}>
      ...
      <div style={{ color: 'var(--ink-mute)', fontSize: 13, marginBottom: 48 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 28 }}>
        {data.solutions.map(s => (
          <button key={s.id}
            ...
            <div style={{ fontSize: 11, color: 'var(--ink-mute)', marginBottom: 28 }}>{s.description}</div>
            <div className="display" style={{ fontSize: 54, fontWeight: 300, letterSpacing: '-0.03em', lineHeight: 1 }}>
              {Math.round(s.ftr*100)}<span style={{ fontSize: 18, color: 'var(--ink-mute)' }}>%</span>
            </div>
            <div style={{ fontSize: 10.5, color: 'var(--ink-mute)', marginTop: 6, letterSpacing: '0.08em',
          ...
          {sessions.map((s, i) => (
            <button key={s.id}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                {!s.ftr && <span className="mono" style={{ fontSize: 10, color: 'var(--accent)' }}>
                  {s.corrections} corrections
                </span>}
              </div>
              <div style={{ fontSize: 13, color: 'var(--ink)', marginBottom: 3 }}>{s.title}</div>
        ...
        <div style={{ flex: 1, padding: '48px 56px', overflow: 'auto', position: 'relative' }}>
          <button onClick={() => setFocused(null)}
                  style={{ position: 'absolute', top: 22, right: 22, fontSize: 18,
                           color: 'var(--ink-mute)', width: 30, height: 30 }}>×</button>
          <div style={{ fontSize: 11, color: 'var(--ink-mute)', letterSpacing: '0.12em',
                        textTransform: 'uppercase', marginBottom: 6 }}>
          ...
          <div className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)', marginBottom: 24 }}>
              ...
              <div style={{ position: 'relative', paddingLeft: 22 }}>
                ...
                {selectedSession.events.map((e, i) => (
                  <div key={i} style={{ display: 'flex', gap: 14, marginBottom: 14,
                    ...
                    <div className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)', width: 42, paddingTop: 1 }}>
                      {e.t}
                    </div>
                    <div style={{ fontSize: 12.5, color: e.kind === 'correction' ? 'var(--accent)' : 'var(--ink-soft)',
                                  lineHeight: 1.5 }}>
                      {e.text}
                    </div>
                  </div>
        ...
        {sol.repos.map(r => (
          <button key={r}
                  ...
                  style={{
                    ...
                    color: selectedRepo === r ? 'var(--paper)' : 'var(--ink-soft)'
                  }}>{r}</button>
        ))}
      </div>
        ...
        <svg viewBox="0 0 600 240" width="100%" height="240" style={{ marginTop: 8 }}>
          ...
          {Array.from({length: 60}).map((_, i) => {
            ...
            return <circle key={i} cx={x} cy={y} r={big ? 5 : 2} fill="currentColor"
                           opacity={big ? 0.8 : 0.25} style={{ color: 'var(--ink-soft)' }}/>;
          })}
          ...
          <g stroke="var(--ink-faint)" strokeWidth="0.5" opacity="0.4" fill="none">
        ...
        <div>
          ...
          {data.hotspots.map((h, i) => (
            <div key={i} style={{ padding: '12px 0', borderBottom: i < data.hotspots.length - 1 ? 'var(--hairline)' : 'none',
              ...
              <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>in {h.fanIn}</span>
              <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>out {h.fanOut}</span>
              <span className="mono" style={{ fontSize: 10.5, color: h.rework > 3 ? 'var(--accent)' : 'var(--ink-mute)' }}>
                ↻{h.rework}
              </span>
            </div>
          ))}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[
              ...
              <div key={r.l} style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span className="mono" style={{ fontSize: 12, color: 'var(--ink)' }}>{r.v}</span>
              </div>
            ))}
          </div>
        </div>
...
function MaCoaching({ data, sol, applied, setApplied }) {
  return (
    <div style={{ padding: '48px 72px', maxWidth: 880 }}>
      ...
      <div style={{ color: 'var(--ink-mute)', fontSize: 13, marginBottom: 44 }}>
        ...
        {data.coaching.map((c, i) => {
          ...
          return (
            <div key={c.id}
              ...
              <div>
                <div className="display" style={{ fontSize: 48, fontWeight: 300, color: 'var(--accent)',
              ...
              <div>
                ...
                <div className="mono" style={{ marginTop: 14, fontSize: 11, color: 'var(--ink-mute)' }}>
                  module: {c.module}
                </div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'flex-start' }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{c.actionDetail}</div>
                <div style={{ fontSize: 11, color: 'var(--ink-soft)', fontStyle: 'italic' }}>{c.impact}</div>
              </div>
            </div>
          );
        })}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 10 }}>
          {data.personas.map(p => (
            <div key={p.id} style={{ padding: 12, border: 'var(--hairline)',
                          borderRadius: 6, background: 'var(--paper-soft)' }}>
              <div style={{ fontSize: 13, color: 'var(--ink)' }}>{p.name}</div>
              <div className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)', marginTop: 3 }}>
                {p.triggers}
              </div>
            </div>
          ))}
...
function MaConfig({ data }) {
  ...
  return (
    <div style={{ padding: '48px 56px', maxWidth: 960 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {data.libraries.map((l, i) => (
            <div key={l.name} style={{ padding: '14px 0', borderBottom: i < data.libraries.length-1 ? 'var(--hairline)' : 'none',
              ...
              <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>v{l.version}</span>
              <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{l.pages} pages</span>
              <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{l.lastIndexed}</span>
            </div>
          ))}
      ...
      {tab === "daemon" && (
        <div>
          <div style={{ padding: 20, background: 'var(--paper-soft)', border: 'var(--hairline)',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginTop: 16 }}>
              <div>
                <div style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>Port</div>
                <div className="mono" style={{ fontSize: 13 }}>9823</div>
              </div>
              <div>
                <div style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>Events today</div>
                <div className="mono" style={{ fontSize: 13 }}>1,842</div>
              </div>
              <div>
                <div style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>Memory</div>
...
function MaOnboarding() {
  ...
  return (
    <div style={{ padding: '72px 80px', maxWidth: 820 }}>
      <div className="kanji" style={{ fontSize: 54, color: 'var(--accent)', opacity: 0.85,
      ...
      <p style={{ color: 'var(--ink-soft)', fontSize: 15, lineHeight: 1.6, marginBottom: 48, maxWidth: 520 }}>
        ...
        {steps.map(s => (
          <button key={s.n}
            ...
            <div>
              ...
              <div style={{ fontSize: 12.5, color: 'var(--ink-mute)', marginTop: 2 }}>{s.detail}</div>
            </div>
            {s.n === step && (
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 36 }}>
        ...
        <button style={{ padding: '12px 20px', color: 'var(--ink-mute)', fontSize: 13 }}>
          Skip for now
        </button>
      </div>
    </div>
