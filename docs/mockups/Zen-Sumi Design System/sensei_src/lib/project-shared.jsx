...
function ProjHeader({ project, onBack, showBack = true }) {
  return (
    <div style={{ padding: '28px 44px 20px', borderBottom: 'var(--hairline)',
      ...
      <div className="kanji" style={{ fontSize: 52, color: 'var(--accent)', lineHeight: 1, marginTop: -4 }}>
        {project.kanji}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 24,
                    padding: '2px 0', color: 'var(--ink-soft)' }}>
        <div>
          <div style={{ fontSize: 10, letterSpacing: '0.14em', color: 'var(--ink-mute)',
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1.35fr 1fr', gap: 30, padding: '26px 44px' }}>
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {project.repos.map(r => (
            <div key={r.id} style={{
              ...
              <div>
                <div style={{ fontSize: 13.5, color: 'var(--ink)' }}>{r.id}</div>
                <div className="mono" style={{ fontSize: 10.5, color: 'var(--ink-faint)' }}>{r.path}</div>
              </div>
              <div className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{r.lang}</div>
              <div className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{r.size}</div>
            </div>
          ))}
        </div>
        ...
        <SectionHeading k="師" label="What sensei recommends"
                        right={<span className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)' }}>
                          {recs.length} open
                        </span>}/>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1, marginBottom: 24 }}>
          {D.files.filter(f => f.tags.includes("hot") || f.tags.includes("god-node")).slice(0, 5).map(f => (
            <div key={f.path} style={{
              ...
              <div>
                <div className="mono" style={{ fontSize: 11.5, color: 'var(--ink)' }}>{f.path}</div>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
                ...
                <span className="mono" style={{ fontSize: 10,
                      color: p.status === "rule" ? 'var(--success)' :
                             p.status === "gap" ? 'var(--warning)' : 'var(--ink-mute)' }}>
                  {p.status}
                </span>
              </div>
        ...
        {D.patterns.antiPatterns.length > 0 && (
          <div style={{ marginTop: 14, padding: '10px 12px',
                         ...
                         borderRadius: 5, fontSize: 11.5, color: 'var(--ink-soft)',
                         lineHeight: 1.5 }}>
            <span className="kanji" style={{ fontSize: 12, color: 'var(--warning)',
...
function ProjRecCard({ rec, openAction }) {
  const tone =
    ...
    rec.urgency === "medium" ? 'var(--warning)' : 'var(--ink-mute)';
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontSize: 13.5, color: 'var(--ink)', marginBottom: 4, lineHeight: 1.45 }}>
          {rec.title}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 8,
                      fontSize: 10.5, color: 'var(--ink-mute)' }} className="mono">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <button onClick={() => openAction(rec, "palette")}
                style={{ padding: '6px 12px', fontSize: 11,
                         color: 'var(--ink-soft)', border: 'var(--ink-line)',
                         borderRadius: 5, whiteSpace: 'nowrap' }}>
          customize prompt
        </button>
      </div>
    ...
    <div style={{ padding: '26px 44px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 14 }}>
          <span className="kanji" style={{ fontSize: 18, color: 'var(--accent)' }}>構</span>
        ...
        <div style={{ display: 'flex', gap: 4, padding: 3, background: 'var(--paper-mute)', borderRadius: 6 }}>
          {[["graph","Call graph"],["matrix","Matrix"],["hairball","Clusters"]].map(([id, lbl]) => (
            <button key={id} onClick={() => setLens(id)}
                    style={{ padding: '6px 12px', fontSize: 11.5,
                             ...
                             color: lens === id ? 'var(--ink)' : 'var(--ink-mute)' }}>
              {lbl}
            </button>
          ))}
        </div>
      ...
      <div style={{ position: 'relative', background: 'var(--paper-soft)',
...
function LensGraph({ D, overlay }) {
  ...
  const nodeColor = (n) => {
    ...
    if (overlay === "stale"     && n.stale >= 10) return 'var(--ink-faint)';
  ...
  return (
    <div style={{ padding: 18 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between',
                    marginTop: 6, fontSize: 10, color: 'var(--ink-faint)' }} className="mono">
      ...
      {sorted.map(n => {
        ...
        return (
          <div key={n.id} style={{
            ...
            <div>
              <div className="mono" style={{ fontSize: 10.5, color: 'var(--ink)',
                              wordBreak: 'break-all', lineHeight: 1.3 }}>
                {n.id}
              </div>
              <div className="mono" style={{ fontSize: 9.5, color: 'var(--ink-mute)', marginTop: 3 }}>
                {n.repo}
              </div>
            </div>
    ...
    <div style={{ padding: 18 }}>
      <svg width={W} height={H}>
        {groups.map((g, gi) => {
          ...
          return (
            <g key={g.id}>
              <circle cx={cx} cy={cy} r={r} fill="var(--paper-mute)" opacity={0.5}/>
              <text x={cx} y={cy - r - 6} textAnchor="middle"
                    fontFamily="var(--font-display)" fontSize="13" fill="var(--ink-soft)">
                {g.id}
              </text>
              {g.nodes.map((n, ni) => {
                ...
                const color =
                  ...
                  overlay === "stale"      && n.stale >= 10 ? 'var(--ink-faint)' :
                ...
                return (
                  <g key={n.id}>
                    ...
                    <text x={nx} y={ny + 18} textAnchor="middle"
                          fontFamily="var(--font-mono)" fontSize="9" fill="var(--ink-mute)">
                      {n.id.split('/').pop()}
                    </text>
                  </g>
                );
  ...
  return (
    <div style={{ padding: '26px 44px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
        <div style={{ display: 'flex', background: 'var(--paper-soft)',
        ...
        <span style={{ fontSize: 11.5, color: 'var(--ink-mute)', lineHeight: 1.5, maxWidth: 440 }}>
          {side === "follow"
...
function PatSideBtn({ on, onClick, kanji, label, count, warn }) {
  return (
    <button onClick={onClick}
      ...
      {label}
      <span className="mono" style={{ fontSize: 10,
                    color: on ? 'var(--paper)' : 'var(--ink-faint)', opacity: 0.85 }}>
        {count}
      </span>
    </button>
  );
...
function FollowRow({ p, on, onClick }) {
  const tone =
    ...
    p.status === "suggested" ? 'var(--accent)'  : 'var(--ink-mute)';
  const bg =
    p.status === "rule"      ? 'var(--success-soft)' :
    p.status === "gap"       ? 'var(--warning-soft)' :
    p.status === "suggested" ? 'var(--accent-soft)'  : 'var(--paper-mute)';
  return (
    <button onClick={onClick}
            style={{
...
function AntiRow({ a, on, onClick }) {
  const sevTone =
    ...
    a.severity === "medium" ? 'var(--warning)' : 'var(--ink-mute)';
  const sevBg =
    a.severity === "high"   ? 'var(--accent-soft)' :
    a.severity === "medium" ? 'var(--warning-soft)' : 'var(--paper-mute)';
  return (
    <button onClick={onClick}
            style={{
              display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 12,
...
function FollowDetail({ p, openAction }) {
  return (
    <>
      ...
      <div style={{ fontSize: 13, color: 'var(--ink-soft)', marginBottom: 14, lineHeight: 1.55 }}>
        {p.summary}
      </div>
      <div style={{ fontSize: 12, color: 'var(--ink-mute)', marginBottom: 14, lineHeight: 1.6 }}>
      ...
      {p.enforcement && (
        <div style={{ padding: '10px 14px', background: 'var(--success-soft)', borderRadius: 6,
                       borderLeft: '2px solid var(--success)', fontSize: 12,
                       color: 'var(--ink-soft)', marginBottom: 12 }}>
          {p.enforcement}
        </div>
      )}
      {p.status === "gap" && (
        <div style={{ padding: 14, background: 'var(--warning-soft)', borderRadius: 6,
    ...
    <>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
        ...
        <span className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)',
                      padding: '2px 7px', borderRadius: 3, background: 'var(--paper-mute)' }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2,
                       ...
                       border: 'var(--hairline)', borderRadius: 6 }}>
          {a.places.map((p, i) => (
            <div key={i} className="mono" style={{ fontSize: 11, color: 'var(--ink-soft)',
                          padding: '3px 0' }}>
              · {p}
            </div>
          ))}
      ...
      {a.suggest && (
        <div style={{ padding: 14, background: 'var(--success-soft)', borderRadius: 6,
                       borderLeft: '2px solid var(--success)' }}>
          <div style={{ display: 'flex', gap: 10, alignItems: 'baseline', marginBottom: 6 }}>
            <span className="kanji" style={{ fontSize: 13, color: 'var(--success)' }}>紋</span>
            <span style={{ fontSize: 10, letterSpacing: '0.14em', color: 'var(--ink-mute)',
      ...
      {!a.suggest && (
        <div style={{ padding: 14, background: 'var(--paper-soft)', borderRadius: 6,
      ...
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        {window.PROJECT_DATA.recentSessions.map(s => (
          <button key={s.id} style={{
            ...
            <span style={{ fontSize: 13, color: 'var(--ink-soft)' }}>{s.title}</span>
            <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>
              {s.corrections === 0 ? "first-try" : `${s.corrections}×`}
            </span>
            <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)',
                          minWidth: 50, textAlign: 'right' }}>{s.duration}</span>
            <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-faint)' }}>{s.time}</span>
          </button>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ padding: '26px 44px',
        ...
        {project.repos.map(r => (
          <div key={r.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto',
            ...
            <div>
              ...
              <div className="mono" style={{ fontSize: 10, color: 'var(--ink-faint)' }}>{r.path}</div>
            </div>
            <button style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>remove</button>
          </div>
        ))}
      </SettingsCard>
        ...
        {S.links.map(l => (
          <div key={l.id} style={{ display: 'grid', gridTemplateColumns: '80px 1fr auto',
            ...
            <div>
              ...
              <div className="mono" style={{ fontSize: 10, color: 'var(--ink-faint)' }}>{l.url}</div>
            </div>
            <button style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>edit</button>
          </div>
        ))}
      </SettingsCard>
        ...
        {S.backlog.map(b => (
          <div key={b.id} style={{ display: 'flex', alignItems: 'baseline', gap: 10,
            ...
            <span style={{ width: 6, height: 6, borderRadius: '50%',
                            background: 'var(--ink-faint)', marginTop: 5 }}/>
            <div style={{ flex: 1, fontSize: 12.5, color: 'var(--ink)' }}>{b.task}</div>
            <span className="mono" style={{ fontSize: 10, color: 'var(--ink-faint)' }}>{b.added}</span>
          </div>
        ))}
      </SettingsCard>
      ...
      <SettingsCard title="Skills enabled">
        {S.skills.map(s => (
          <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto',
            ...
            <span style={{ fontSize: 12.5, color: s.on ? 'var(--ink)' : 'var(--ink-mute)' }}>
              {s.name}
            </span>
            <ToggleChip on={s.on}/>
          </div>
      ...
      <SettingsCard title="Excluded paths" action="+ add pattern">
        {S.excluded.map(p => (
          <div key={p} className="mono" style={{ fontSize: 11, color: 'var(--ink-soft)',
...
function IdentityStrip({ project }) {
  const icon = project.icon || { kind: "kanji", value: project.kanji, bg: 'var(--paper-mute)', fg: 'var(--ink)' };
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '64px 1fr auto',
                   gap: 18, alignItems: 'center',
                   padding: '16px 20px',
                   background: 'var(--paper-soft)', border: 'var(--hairline)', borderRadius: 8 }}>
      {/* icon slot — swappable */}
      <button title="Change icon"
              style={{ width: 64, height: 64, borderRadius: 10,
                        background: icon.bg, color: icon.fg,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        border: '1px solid var(--ink-5, rgba(0,0,0,0.06))',
                        position: 'relative' }}>
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 4 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--ink-mute)' }}>· {project.client}</div>
        </div>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr',
      ...
      <aside style={{ borderRight: 'var(--hairline)',
        ...
        <div style={{ height: 1, background: 'var(--ink-5, rgba(0,0,0,0.06))' }}/>
          ...
          {sections.map(id => (
            <button key={id} onClick={() => setActive(id)}
                    style={{ textAlign: 'left', padding: '8px 10px',
                              fontSize: 12, color: active===id ? 'var(--ink)' : 'var(--ink-mute)',
      ...
      <div style={{ padding: '32px 44px', overflow: 'auto', maxHeight: '100%' }}>
              ...
              <div key={label} style={{ display: 'grid', gridTemplateColumns: '120px 1fr auto',
                ...
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                  {items.length > 0 ? items.map(it => (
                    <span key={it} className="mono" style={{ fontSize: 11,
                                  padding: '2px 8px', background: 'var(--paper-soft)',
                                  border: 'var(--hairline)', borderRadius: 3,
                                  color: 'var(--ink-soft)' }}>{it}</span>
        ...
        <V2Block id="repos" title="Repos" desc="Folders sensei watches for this project.">
          {project.repos.map(r => (
            <div key={r.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto',
              ...
              <button style={{ fontSize: 11, color: 'var(--ink-mute)' }}>remove</button>
            </div>
          ))}
          <button style={{ marginTop: 10, fontSize: 12, color: 'var(--accent)' }}>+ add repo</button>
        </V2Block>
          ...
          {S.links.map(l => (
            <div key={l.id} style={{ display: 'grid', gridTemplateColumns: '90px 1fr auto',
              ...
              <div>
                ...
                <div className="mono" style={{ fontSize: 10.5, color: 'var(--ink-faint)', marginTop: 2 }}>{l.url}</div>
              </div>
              <button style={{ fontSize: 11, color: 'var(--ink-mute)' }}>edit</button>
            </div>
          ))}
          <button style={{ marginTop: 10, fontSize: 12, color: 'var(--accent)' }}>+ add link</button>
        </V2Block>
        ...
        <V2Block id="guidelines" title="Guidelines"
          ...
          <button style={{ marginTop: 10, fontSize: 12, color: 'var(--accent)' }}>+ add rule</button>
        </V2Block>
        ...
        <V2Block id="backlog" title="Backlog"
          ...
          {S.backlog.map(b => (
            <div key={b.id} style={{ display: 'grid', gridTemplateColumns: '12px 1fr auto',
                                      ...
                                      padding: '11px 0', borderBottom: 'var(--hairline)' }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%',
                              background: 'var(--ink-faint)', marginTop: 6 }}/>
              <div style={{ fontSize: 13, color: 'var(--ink)' }}>{b.task}</div>
              <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-faint)' }}>{b.added}</span>
            </div>
          ))}
          <button style={{ marginTop: 10, fontSize: 12, color: 'var(--accent)' }}>+ add task</button>
        </V2Block>
      </div>
    </div>
  );
  ...
  return (
    <section id={id}
             style={{ paddingBottom: 36, marginBottom: 36,
                       borderBottom: '1px solid var(--ink-5, rgba(0,0,0,0.06))' }}>
      <div style={{ marginBottom: 16 }}>
...
function V2Field({ label, value, multiline, action }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr auto',
      ...
      <button style={{ fontSize: 11, color: action ? 'var(--accent)' : 'var(--ink-faint)' }}>
        {action || "edit"}
      </button>
    </div>
  );
...
function SettingsCard({ title, action, children }) {
  return (
    <div style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between',
        ...
        {action && (
          <button style={{ fontSize: 10.5, color: 'var(--accent)' }}>{action}</button>
        )}
      </div>
      {children}
    </div>
  ...
  return (
    <div style={{ padding: '8px 0', borderBottom: 'var(--hairline)' }}>
      ...
      <div style={{ fontSize: 12.5, marginTop: 3,
                     fontStyle: multiline ? 'italic' : 'normal',
                     color: multiline ? 'var(--ink-soft)' : 'var(--ink)' }}>{value}</div>
    </div>
  );
}
function ToggleChip({ on }) {
  return (
    <span className="mono" style={{
      ...
      background: on ? 'var(--success-soft)' : 'var(--paper-mute)',
      color: on ? 'var(--success)' : 'var(--ink-mute)'
    }}>{on ? "on" : "off"}</span>
  );
}
function ToggleRow({ label, on, value, tone }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 10,
      ...
      <span style={{ fontSize: 12.5, color: tone === "warn" ? 'var(--warning)' : 'var(--ink)' }}>
        {label}
      </span>
      {value ? (
        <span className="mono" style={{ fontSize: 10, color: 'var(--ink-mute)' }}>{value}</span>
      ) : (
        <ToggleChip on={on}/>
      )}
    </div>
    ...
    <div style={{
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div style={{ padding: '22px 28px 14px', borderBottom: 'var(--hairline)',
          ...
          <span className="kanji" style={{ fontSize: 26, color: 'var(--accent)', lineHeight: 1 }}>
            {rec.kanji || "送"}
          </span>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 10, letterSpacing: '0.16em', color: 'var(--ink-mute)',
          ...
          <button onClick={onClose} style={{ fontSize: 13, color: 'var(--ink-mute)' }}>✕</button>
        </div>
        ...
        <div style={{ flex: 1, padding: '14px 28px', overflow: 'auto' }}>
          ...
          {rec.evidence && (
            <div style={{ marginTop: 14, fontSize: 10.5, color: 'var(--ink-mute)' }}>
        ...
        <div style={{ padding: '14px 28px', borderTop: 'var(--hairline)',
          ...
          <button onClick={onClose}
                  style={{ padding: '8px 14px', fontSize: 12,
                           color: 'var(--ink-soft)', border: 'var(--ink-line)', borderRadius: 5 }}>
            cancel
          </button>
