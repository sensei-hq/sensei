...
function L2Hero({ title, sub, kanji, right }) {
  return (
    <div style={{ padding: '22px 36px 16px', borderBottom: 'var(--hairline)',
      ...
      <div className="kanji" style={{ fontSize: 42, color: 'var(--accent)', lineHeight: 1 }}>{kanji}</div>
...
function Quiet({ text }) {
  return (
    <div style={{ padding: '40px 8px', textAlign: 'center', fontSize: 11.5,
                   color: 'var(--ink-faint)', fontStyle: 'italic' }}>{text}</div>
  );
}
...
function ViolationCard({ memory }) {
  ...
  return (
    <div style={{ background: 'var(--accent-soft)', border: '1px solid transparent',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
        ...
        <span style={{ flex: 1 }}/>
        <span className="mono" style={{ fontSize: 10, color: 'var(--ink-faint)' }}>{memory.lastRelevant}</span>
      </div>
      <div style={{ fontSize: 12.5, color: 'var(--ink)', lineHeight: 1.4, fontWeight: 500 }}>
        {memory.what}
      </div>
...
function RecCardSlim({ rec }) {
  ...
  return (
    <div style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
        <span className="kanji" style={{ fontSize: 12, color: 'var(--accent)' }}>{kindGlyph}</span>
        <span style={{ fontSize: 9.5, color: 'var(--ink-mute)', letterSpacing: '0.14em',
                        textTransform: 'uppercase' }}>{rec.action}</span>
        <span style={{ flex: 1 }}/>
        <span className="mono" style={{ fontSize: 10, color: 'var(--ink-faint)' }}>{rec.targetKind}</span>
      </div>
      <div style={{ fontSize: 12, color: 'var(--ink)', lineHeight: 1.4 }}>{rec.title}</div>
      <div style={{ fontSize: 10.5, color: 'var(--ink-mute)', marginTop: 5,
...
function CorrectionMini({ c }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', gap: 8,
                   padding: '7px 12px', background: 'var(--paper-soft)',
      ...
      <span className="mono" style={{ fontSize: 11, color: 'var(--ink-soft)' }}>{c.count}×</span>
    </div>
  );
}
  ...
  return (
    <div style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 4 }}>
        <span className="mono" style={{ fontSize: 12, color: 'var(--ink)' }}>{pattern.name}</span>
        <span style={{ flex: 1 }}/>
...
function ChallengedMini({ memory }) {
  return (
    <div style={{ background: 'var(--warning-soft)', border: '1px solid transparent',
                   borderRadius: 5, padding: '9px 12px' }}>
      <div style={{ fontSize: 9.5, color: 'var(--warning)', letterSpacing: '0.14em',
                     textTransform: 'uppercase', marginBottom: 4 }}>challenged</div>
      <div style={{ fontSize: 12, color: 'var(--ink)', lineHeight: 1.4 }}>{memory.what}</div>
    </div>
  );
}
  ...
  return (
    <div title={`strength ${value}/5`}
      ...
      <div style={{ position: 'absolute', inset: 0,
                     width: `${(value/5)*100}%`,
                     background: value === 5 ? 'var(--success)' : 'var(--ink-soft)',
                     borderRadius: 2 }}/>
    </div>
  );
}
            ...
            return (
              <div key={g.id} style={{ marginBottom: 14 }}>
                <div style={{ padding: '4px 18px 6px', display: 'flex',
                  ...
                  <span className="mono" style={{ fontSize: 10, color: 'var(--ink-faint)' }}>{items.length}</span>
                </div>
                {items.map(m => (
                  <button key={m.id} onClick={() => setOpen(m.id)}
                          style={{ width: '100%', textAlign: 'left',
                                    padding: '7px 18px 7px 22px',
                                    background: openId === m.id ? 'var(--paper-soft)' : 'transparent',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
          ...
          <span style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>{memory.state}</span>
          <span style={{ flex: 1 }}/>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr',
        ...
        <AnatomyBlock label="How · surface" kanji={how.glyph} tone="shu">
          ...
          {memory.references.good_example && (
            <div className="mono" style={{ fontSize: 10.5, color: 'var(--success)', marginTop: 8 }}>
              ✓ {memory.references.good_example}
            </div>
          )}
            ...
            {scopeChips(memory.scope, L).map((c, i) => (
              <span key={i} className={c.mono ? "mono" : ""}
                    style={{ fontSize: 10.5, padding: '2px 8px',
                              ...
                              borderRadius: 12, color: c.dim ? 'var(--ink-mute)' : 'var(--ink-soft)',
...
function AnatomyBlock({ label, kanji, tone, children }) {
  const accent = tone === "shu" ? "var(--accent)" :
                 tone === "jade" ? "var(--success)" : "var(--ink-soft)";
  return (
    <section style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
                       borderRadius: 8, padding: '14px 16px',
                       display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 8 }}>
        <span className="kanji" style={{ fontSize: 16, color: accent, lineHeight: 1 }}>{kanji}</span>
        <span style={{ fontSize: 10, letterSpacing: '0.16em', color: 'var(--ink-mute)',
...
function FlatBtn({ glyph, label, subtle }) {
  return (
    <button style={{ padding: '7px 12px', fontSize: 11.5,
                      ...
                      borderRadius: 5, color: subtle ? 'var(--ink-mute)' : 'var(--ink)',
                      cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7 }}>
      <span className="kanji" style={{ fontSize: 13,
                    color: subtle ? 'var(--ink-mute)' : 'var(--accent)' }}>{glyph}</span>
      {label}
    </button>
  );
}
          ...
          return (
            <g key={i}>
              <rect x={i*(bw+gap)} y={H - h} width={bw} height={h}
                    fill={i === 4 ? 'var(--success)' : i >= 2 ? 'var(--ink-soft)' : 'var(--ink-mute)'}
                    rx={1}/>
              <text x={i*(bw+gap) + bw/2} y={H + 11}
                    fontSize={9} fill="var(--ink-faint)" textAnchor="middle"
...
function BriefHeader({ kanji, title, sub, count }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 12,
      ...
      <span className="kanji" style={{ fontSize: 16, color: 'var(--accent)' }}>{kanji}</span>
      <h3 className="display" style={{ fontSize: 16, fontWeight: 400, margin: 0,
                    color: 'var(--ink)' }}>{title}</h3>
      <span style={{ fontSize: 11, color: 'var(--ink-mute)' }}>· {sub}</span>
      <span style={{ flex: 1 }}/>
...
function ActRow({ act }) {
  const dot = act.impact === "high" ? 'var(--accent)' :
              act.impact === "medium" ? 'var(--warning)' : 'var(--ink-faint)';
  return (
    <article style={{ display: 'grid',
      ...
      <button style={{ padding: '6px 12px', fontSize: 11,
                        ...
                        color: act.impact === "high" ? 'var(--paper)' : 'var(--ink-soft)',
    ...
    <div style={{ border: 'var(--hairline)', borderRadius: 6,
      ...
      <button onClick={onToggle}
              style={{ width: '100%', padding: '11px 16px',
                        ...
                        textAlign: 'left' }}>
        <span className="kanji" style={{ fontSize: 14, color: 'var(--accent)' }}>{cat.glyph}</span>
        <span style={{ fontSize: 13, color: 'var(--ink)', fontWeight: 500 }}>{cat.label}</span>
        <span style={{ fontSize: 11, color: 'var(--ink-mute)' }}>· {cat.desc}</span>
        <span style={{ flex: 1 }}/>
        <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{items.length}</span>
    ...
    <div style={{ display: 'grid',
      ...
      <div>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 5,
          ...
          <span>
            <span className="kanji" style={{ color: 'var(--accent)', marginRight: 4 }}>{how.glyph}</span>
            {how.label}
          </span>
          <L2WhereLine scope={memory.scope}/>
        </div>
...
function MemoryShapeChart({ memories }) {
  const cats = [
    ...
    { id: "convention",  label: "conventions", color: 'var(--ink-soft)' },
  ...
  return (
    <section style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 12 }}>
        <span className="kanji" style={{ fontSize: 14, color: 'var(--accent)' }}>形</span>
        ...
        {meta.map(b => (
          <g key={b.id}>
            ...
            <text x={b.x + 6} y={H - 6} fontSize={10}
                  fill="var(--ink-mute)" fontFamily="Inter">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between',
                     fontSize: 10, color: 'var(--ink-faint)', marginTop: 8 }}>
