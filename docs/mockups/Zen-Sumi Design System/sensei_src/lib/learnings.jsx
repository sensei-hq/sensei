...
function LearnHero({ counts }) {
  return (
    <div style={{ padding: '22px 40px 16px', borderBottom: 'var(--hairline)',
      ...
      <div className="kanji" style={{ fontSize: 46, color: 'var(--accent)', lineHeight: 1 }}>学</div>
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{ fontSize: 9.5, letterSpacing: '0.14em', textTransform: 'uppercase',
                     color: 'var(--ink-faint)', marginTop: 4 }}>{label}</div>
    </div>
  );
}
    ...
    <section>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between',
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
          <span className="kanji" style={{ fontSize: 14, color: 'var(--accent)' }}>薦</span>
          <span style={{ fontSize: 11, letterSpacing: '0.16em', color: 'var(--ink-mute)',
...
function RecCard({ rec, onDismiss }) {
  const kindMap = {
    ...
    "archive-memory":  { glyph: "納", label: "archive", color: "var(--ink-mute)" },
  ...
  const impactDot = rec.impact === "high" ? "var(--accent)" : rec.impact === "medium" ? "var(--warning)" : "var(--ink-faint)";
  return (
    <div style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ fontSize: 10, color: 'var(--ink-faint)' }}>{rec.impact}</span>
        <span style={{ flex: 1 }}/>
        <button onClick={onDismiss}
                style={{ fontSize: 12, color: 'var(--ink-faint)', lineHeight: 1,
        ...
        <>
          ...
          <select value={sort} onChange={e => setSort(e.target.value)}
                  style={{ fontSize: 11, border: 'var(--hairline)', background: 'var(--paper)',
                            padding: '3px 7px', borderRadius: 4, color: 'var(--ink-soft)' }}>
...
function EmptyState({ text }) {
  return (
    <div style={{ padding: 40, textAlign: 'center', fontSize: 12, color: 'var(--ink-faint)' }}>
      {text}
    </div>
  );
}
...
function MemoryCard({ memory, onClick }) {
  ...
  return (
    <article onClick={onClick}
             style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
      ...
      {chips.map((c, i) => (
        <span key={i} className={c.k === "module" ? "mono" : ""}
              style={{ fontSize: 10, padding: '1px 7px',
                        ...
                        borderRadius: 10, color: 'var(--ink-mute)',
...
function RefLink({ kind, path }) {
  ...
  return (
    <span className="mono" style={{ display: 'inline-flex', alignItems: 'center', gap: 4,
                   fontSize: 10, color: isGood ? 'var(--matcha)' : 'var(--accent)' }}>
      <span>{isGood ? "✓" : "✗"}</span>
      ...
      <div style={{ display: 'flex', gap: 3 }}>
        {dots.map((on, i) => (
          <span key={i} style={{ width: 6, height: 6, borderRadius: '50%',
                                  background: on ? 'var(--ink)' : 'var(--paper-edge)' }}/>
        ))}
      </div>
    </div>
  );
    ...
    <article style={{ background: 'var(--paper-soft)', border: 'var(--hairline)',
      ...
      <div style={{ textAlign: 'right', minWidth: 110 }}>
        ...
        <div className="mono" style={{ fontSize: 10, marginTop: 3,
                      color: pattern.ftrDelta > 0 ? 'var(--matcha)' : 'var(--accent)',
                      fontFeatureSettings: '"tnum"' }}>
...
function LifecycleRow({ ev, onOpen }) {
  ...
  const kindMap = {
    ...
    archived:   { glyph: "納", label: "archived",   color: "var(--ink-mute)" }
  };
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
          ...
          <button onClick={() => onOpen(ev.memoryId)}
                  style={{ fontSize: 12, color: 'var(--ink)', background: 'transparent',
      ...
      <aside style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: 520,
        ...
        <div style={{ padding: '18px 24px 14px', borderBottom: 'var(--hairline)',
          ...
          <div style={{ flex: 1 }}>
            ...
            <h2 style={{ fontSize: 16, fontWeight: 500, margin: 0, color: 'var(--ink)',
                          lineHeight: 1.4 }}>
              {memory.what}
            </h2>
          </div>
          <button onClick={onClose}
                  style={{ fontSize: 18, color: 'var(--ink-mute)', background: 'transparent',
                            border: 'none', cursor: 'pointer', padding: 0,
                            lineHeight: 1 }}>×</button>
        </div>
...
function DrawerRef({ kind, text, label }) {
  const map = {
    ...
    doc:      { glyph: "文", color: "var(--ink-soft)" },
    evidence: { glyph: "証", color: "var(--ink-mute)" },
