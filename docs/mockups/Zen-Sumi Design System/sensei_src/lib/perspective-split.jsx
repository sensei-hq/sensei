  ...
  return (
    <button onClick={onClick}
            style={{
              ...
              background: active ? 'var(--paper-mute)' : 'transparent',
              color: active ? 'var(--ink)' : (dim ? 'var(--ink-mute)' : 'var(--ink-soft)'),
              fontSize: 13, cursor: 'pointer', border: 'none'
            }}>
      <span className="kanji" style={{ fontSize: 13, width: 14,
                    color: active ? 'var(--accent)' : 'var(--ink-mute)' }}>{kanji}</span>
      <span>{label}</span>
      {badge != null && (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          {D.projects.active.map(p => (
            <button key={p.id} onClick={() => onProjectClick && onProjectClick(p.id)}
              ...
              color: 'var(--ink-soft)', fontSize: 12.5, cursor: 'pointer', border: 'none'
            }}>
  ...
  return (
    <aside style={{ borderRight: 'var(--hairline)', padding: '22px 14px',
      ...
      <div style={{ padding: '0 6px' }}>
        <div style={{ fontSize: 9.5, letterSpacing: '0.18em', color: 'var(--ink-mute)',
        ...
        <button onClick={onSwitchProject}
                style={{ marginTop: 10, fontSize: 10.5, color: 'var(--ink-mute)',
      ...
      <div>
        ...
        <div style={{ padding: '0 10px', fontSize: 11, color: 'var(--ink-mute)',
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span className="mono" style={{ color: 'var(--ink-soft)' }}>{project.sessions7d || 28}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
...
function PerspectiveChrome({ title, accent = "var(--accent)", subtitle, onClose }) {
  return (
    <div style={{
      height: 38, background: 'var(--paper-soft)',
      ...
      <div style={{ flex: 1, textAlign: 'center', display: 'flex',
        ...
        <span style={{ fontSize: 12, color: 'var(--ink)', letterSpacing: '0.04em' }}>
          {title}
        </span>
        {subtitle && (
...
function ProjectWindowContent({ project }) {
  return (
    <div style={{ padding: '32px 40px 48px' }}>
      ...
      <div style={{
        padding: '24px 26px',
        background: 'var(--paper-soft)', border: 'var(--hairline)', borderRadius: 10,
        display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 22, marginBottom: 28
      }}>
        <div className="kanji" style={{ fontSize: 64, color: 'var(--accent)', lineHeight: 1 }}>聴</div>
        <div>
          <div style={{ fontSize: 10.5, letterSpacing: '0.16em', color: 'var(--ink-mute)',
                         textTransform: 'uppercase', marginBottom: 4 }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--ink-soft)', lineHeight: 1.65, margin: 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
          {[
            ...
            <div key={i} style={{
              ...
              <span className="kanji" style={{ fontSize: 18, color: 'var(--accent)' }}>{x.k}</span>
              <div>
                <div style={{ fontSize: 13, color: 'var(--ink)' }}>{x.t}</div>
                <div style={{ fontSize: 11, color: 'var(--ink-mute)', marginTop: 2 }}>{x.s}</div>
              </div>
              <span className="mono" style={{ fontSize: 11, color: 'var(--ink-mute)' }}>{x.n}</span>
            </div>
          ))}
        </div>
      </div>
...
function ProjStat({ label, value, sub, tone = "var(--ink)" }) {
  return (
    <div style={{ padding: '14px 16px', background: 'var(--paper-soft)',
      ...
      <div style={{ fontSize: 10.5, color: 'var(--ink-mute)', marginTop: 6 }}>{sub}</div>
    </div>
  );
}
