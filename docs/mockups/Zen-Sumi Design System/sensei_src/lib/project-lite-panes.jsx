...
function SimpleRow({ left, right, leftSub, rightTone = 'var(--ink-mute)' }) {
  return (
    <div style={{
        ...
        <div>
          {(D.recentSessions || []).slice(0, 4).map(s => (
            <SimpleRow key={s.id}
              ...
              right={s.time}
              rightTone={s.ftr ? 'var(--success)' : 'var(--ink-mute)'}/>
          ))}
        </div>
      </div>
    </div>
        ...
        {memories.map((m, i) => (
          <div key={i} style={{
            ...
            <span className="kanji" style={{ fontSize: 16,
                  color: m.status === 'share' ? 'var(--matcha)' : 'var(--accent)' }}>{m.kanji}</span>
            <div>
          ...
          <div key={i} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between',
              ...
              <span className="mono" style={{ fontSize: 10.5,
                    color: d.lastSync === 'broken' ? 'var(--accent)' : 'var(--ink-mute)' }}>
                {d.lastSync === 'broken' ? '⚠ broken link' : 'synced ' + d.lastSync}
              </span>
            </div>
            <div className="mono" style={{ fontSize: 10.5, color: 'var(--ink-faint)', marginBottom: 6 }}>
              ↪ {d.symbol}
            </div>
      ...
      <div>
        ...
        {libs.map((l, i) => (
          <SimpleRow key={i}
            ...
            rightTone={l.hasInstr ? 'var(--matcha)' : 'var(--warning)'}/>
        ))}
      </div>
    </div>
  );
      ...
      <div>
        ...
        {tools.map((t, i) => (
          <SimpleRow key={i}
            ...
            rightTone={t.ftr >= 0.95 ? 'var(--matcha)' : 'var(--warning)'}/>
        ))}
      </div>
    </div>
  );
      ...
      <div>
        ...
        {verdicts.map((v, i) => {
          const tone = v.verdict === 'positive' ? 'var(--success)' :
                       v.verdict === 'negative' ? 'var(--accent)'  : 'var(--ink-mute)';
          return (
            <div key={i} style={{
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%',
      ...
      <div style={{
        ...
        background: editing ? 'var(--paper-soft)' : 'var(--paper)'
      }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
          <span className="kanji" style={{ fontSize: 16, color: 'var(--accent)' }}>識</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button onClick={() => setEditing(e => !e)}
                  style={{
                    ...
                    color: editing ? 'var(--paper)' : 'var(--ink-soft)',
                    cursor: 'pointer'
                  }}>
            {editing ? "✓  Done" : "✎  Edit"}
          </button>
