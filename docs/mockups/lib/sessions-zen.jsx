...
function ZenScrollLayout({ chart, setChart, chartNode, sessions, days, totals, retro,
  ...
  return (
    <div ref={scrollRef}
      ...
      <section style={{ padding: '24px 32px 48px', minHeight: 1100 }}>
...
function MiniNumbers({ totals }) {
  return (
    <div style={{ display: 'flex', gap: 16 }}>
      ...
      <div style={{ width: 1, background: 'var(--edge)' }}/>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <div style={{ minWidth: 56 }}>
        ...
        <div style={{ fontSize: 11, letterSpacing: '0.12em', color: 'var(--ink-4)',
                       marginTop: 4, textTransform: 'uppercase' }}>{headline.l}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ padding: '24px 32px 16px',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 0,
                      paddingLeft: 24, borderLeft: '1px solid var(--edge)' }}>
        {collapsed ? (
  ...
  return (
    <div style={{ textAlign: 'center', minWidth: 56 }}>
      <div className={mono ? "mono" : ""}
           style={{ fontSize: 17, lineHeight: 1, fontWeight: 300, color: 'var(--ink)',
                     fontFeatureSettings: '"tnum"',
  ...
  return (
    <section>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 12,
        ...
        {/* Tab switch */}
        <div style={{ display: 'flex', gap: 0, padding: 4, background: 'var(--paper-2)',
...
function TrendChart({ sessions, days = DAYS_ORDERED, checkpoints = [] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}>
      ...
      {[0, 0.5, 1].map(t => (
        <g key={t}>
          <line x1={padL} x2={W - padR} y1={yFtr(t)} y2={yFtr(t)}
                stroke="var(--edge)" strokeDasharray="2 4"/>
          <text x={padL - 8} y={yFtr(t) + 3} fontSize={10}
                fill="var(--ink-4)" textAnchor="end" fontFamily="JetBrains Mono">
            {Math.round(t * 100)}%
          </text>
        </g>
      ))}
      <text x={padL - 8} y={padT - 10} fontSize={9.5} fill="var(--ink-4)"
      ...
      {/* Caption */}
      <text x={padL} y={padT - 18} fontSize={10} fill="var(--ink-4)"
            fontFamily="Inter">
...
function StreamChart({ sessions, days = DAYS_ORDERED, checkpoints = [] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}>
      ...
      {/* Caption */}
      <text x={padL} y={padT + 3} fontSize={10} fill="var(--ink-4)"
            fontFamily="Inter">
        time spent · stacked
      </text>
    </svg>
...
function ConstellationChart({ sessions, days = DAYS_ORDERED }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}>
      ...
      {dayList.map((d, i) => (
        <g key={d}>
          <line x1={padL} x2={W - padR} y1={y(i)} y2={y(i)}
                stroke="var(--edge)" strokeDasharray="1 3"/>
          <text x={padL - 10} y={y(i) + 3} fontSize={10.5}
                fill="var(--ink-3)" textAnchor="end" fontFamily="Inter">
            {DAYS_SHORT(d)}
          </text>
        </g>
      ))}
      ...
      {xTicks.map(t => (
        <g key={t}>
          ...
          <text x={x(t)} y={H - padB + 16} fontSize={10}
                fill="var(--ink-4)" textAnchor="middle" fontFamily="JetBrains Mono">
      ...
      {/* Caption */}
      <text x={padL} y={14} fontSize={10} fill="var(--ink-4)"
            fontFamily="Inter">
    ...
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}>
      ...
      {[30, 60, 120].map(m => (
        <line key={m}
              ...
              stroke="var(--edge)" strokeDasharray="1 4" opacity={0.7}/>
      ))}
      {[30, 60, 120].map(m => (
        <text key={m} x={W - padR + 4}
              y={padT + innerH - (m / maxMins) * innerH + 3}
              fontSize={9} fill="var(--ink-4)" fontFamily="JetBrains Mono">
          {m < 60 ? `${m}m` : `${m/60}h`}
        </text>
      ))}
      ...
      {/* Caption */}
      <text x={padL} y={18} fontSize={10} fill="var(--ink-4)" fontFamily="Inter">
  ...
  const Caption = ({ tag, title, body }) => (
    <div style={{ marginBottom: 8 }}>
      ...
      <div style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 4,
                      paddingLeft: 12, borderLeft: '2px solid var(--edge)' }}>{body}</div>
    </div>
  );
