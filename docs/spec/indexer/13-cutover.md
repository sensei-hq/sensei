...
Prove the indexer is better than the legacy indexer on the real corpus, switch
RUST ONLY, re-index, run acceptance, and retire the superseded module. The
existing indexer keeps running until the switch, so the graph never goes
stale.

## 2. Inputs and outputs

    differential(legacy: &Facts, current: &Facts) -> DiffReport   PURE
...
- **S1.** Run the legacy indexer and this one over the same corpus and diff
  the facts. Every
difference is classified: IMPROVEMENT, REGRESSION, or EXPLAINED. A regression
blocks cutover.
- **S2.** This indexer's RESOLVED set must be a superset of the legacy one's,
  or each exception
justified in writing. Compare resolved edges and node identity — **not raw
reference counts**, because the legacy indexer drops references silently and
the indexer will show far more. That is the intended result, not a regression.
...
`TRUNCATE sensei.nodes, sensei.edges CASCADE`, then a full re-index.
...
segment both come from re-parsing. An index built here over the old rows never
matches the merge contract and builds a SECOND disjoint graph beside the
first.

| destroyed and rebuilt | | |---|---:| | nodes | 386,884 | | edges | 810,216 |
| symbol_names | 134,866 | | embeddings to regenerate | **326,716** | |
...
| the diff shows far more references in the indexer | EXPECTED (S2). Compare resolved edges, not raw counts. |
...
| a deliberately introduced regression in the indexer is REPORTED as a regression | make the comparison count raw references instead of resolved edges |
...
| only Rust is switched; js/ts/svelte still use the legacy indexer | switch the dispatch unconditionally |
...
The unresolved-caller check is the one to do properly. "This has no callers"
is exactly the claim that needs `rg --no-ignore -g '!target'` and a confirmed
non-truncated count, not a quick grep.

## 6. Watch out

**Raw reference counts will look alarming and are the intended result.** The
legacy indexer drops references silently — that is the defect. The indexer
emits an `Unresolved` for every one of them. Comparing totals will show a
large increase that is a fix, not a regression. S2 exists because this is the
easiest possible misinterpretation of the harness's own output.
...
**Retirement is last.** The superseded module keeps running until S5 passes,
so there is never a window where the graph has no indexer.

## 7. Definition of done

- The diff report has ZERO unexplained regressions and the indexer's resolved set is a
proven superset.
...
untouched and still on the legacy indexer.
