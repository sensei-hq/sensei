...
| D8 | Auto-fire lightweight refocus via PreCompact hook | Complements manual `/sensei:session refocus`. Both mechanisms: autopilot + steering wheel. |
| D9 | Guardrails are a living document at project level | Grow organically from feedback. AI asks clarifying questions and incorporates corrections into rules file. Loaded automatically next session. |
...
| D14 | Workflow state file (`.sensei/state.yaml`) as single source of truth | Commands update it, hooks read it, `/sensei:session status` displays it. Solves "where am I" problem. Also the testable artifact — verify state after each command. |
| D15 | `/sensei:session status` command for orientation | The "where am I" query. Reads state file + project structure. Callable by AI internally when lost, not just user-facing. Pre-compact hook reminds AI to call it after compaction. |
...
| D18 | Vertical feature slices, not siloed layers. Innermost layer first, build up, fully tested. No half-baked implementations. When in doubt, ask — don't jump to code. | Features span daemon → MCP → command. No siloed issues per layer — each feature issue includes ALL layers. Implementation order: innermost layer first (with tests), build upward, each layer feature-complete and 100% tested before building on top. No stubs — real implementations bottom-up. **This is a sensei-wide principle** — baked into skills, guardrails, and command instructions. The AI should ask for clarity when information is incomplete rather than proceeding with assumptions that lead to rework. Patience in design prevents cycles in implementation. |
