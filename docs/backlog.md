...
| [#75](https://github.com/sensei-hq/sensei/issues/75) | Cold-start the analyzer on install — import historical transcripts (a superset of the hook stream) as synthesized sessions + assistant_events + turns, then reuse L0/L1 enrichment to derive FTR/churn/corrections for all past sessions. Bridges the gap before hooks accumulate. Extends #73; dedups vs live; `backfilled` flag; pairs with #74. |
