...
- **Operating-model reframe — Phase 1 continuation** *(parked — resume after the in-flight Relay-P3 session lands)* — sensei+dōjō vision reframe as the **"OS for AI-assisted work"**; spec [`plan/operating-model.md`](plan/operating-model.md) (all 7 open decisions resolved). Phase 1 (Foundations) first unit **SHIPPED on `develop`**: `sensei scaffold` doc-structure command (plan [`plan/2026-07-17-doc-scaffold-command.md`](plan/2026-07-17-doc-scaffold-command.md); commits `b546db74`·`0b8c91f3`·`5bf594a1` — TDD + two-stage reviews = Ship; NOT merged to `main`). **NEXT sub-units:** `sensei scaffold feature <name>` dossier scaffolder · memory-anchoring (L0/L1/L2 + memories → spine slots, the "push not pull" fix) · baseline capability-contract scaffold (`--kind`). Then Phase 2 (front-door + playbooks). Minor polish parked in the plan's post-impl notes.
...
- **P4** *(in progress on `develop` 2026-07-17)* — away-from-keyboard: Web Push + realtime swap + offline + "what's blocked on me" home. Decomposed plan: [`plan/2026-07-17-relay-p4-away-from-keyboard.md`](plan/2026-07-17-relay-p4-away-from-keyboard.md) (P4.1 relay RLS+realtime publication → P4.2 realtime swap → P4.3 push client/SW → P4.4 push send → P4.5 offline → P4.6 blocked-on-me home). **Infra/secrets split flagged** (dev VAPID/local-supabase autonomous; prod VAPID keys + realtime publication + RLS rollout = Jerry checklist).
