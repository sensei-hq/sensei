...
- **Scan stage initial dead-time UX.** When the user clicks "Begin scan →", the daemon walks the filesystem before any events fire. For large roots (~/Developer with 90+ projects + thousands of files), this takes 2-3 seconds with NO visible feedback — looks stuck. Fix: (a) emit a `scan_started` activity event from `scan_root()` at the very top, before the filesystem walk; (b) show an immediate spinner / "Scanning filesystem…" status in the scan page on click, cleared when the first SSE event arrives. Captured in user feedback memory (2026-05-28) — silence reads as broken.

- **Queue activity event hardcodes elapsed=0.0.** `crates/senseid/src/tasks/handlers/process.rs:80` emits the queue activity with `0.0` as the elapsed value. The SSE feed therefore shows "+0.00s" for every queue event, even though they fire at different times. Should be `start.elapsed().as_secs_f64()` against a shared scan-start timestamp.

- **Roots stage input UX confusion.** The placeholder text in the Roots input reads like entered text, and the "Add" button doesn't reliably append the root. User-reported 2026-05-28 during smoke test. Investigate the Roots `+page.svelte` — likely a placeholder vs value confusion and possibly a missing click handler or wrong selector for the add action.
...
- **Scan stage — static UI, root cause identified (2026-05-27).** Two distinct daemon ↔ client contract drifts:

  1. *Client stats look for daemon strings that no longer exist.* `app/src/lib/scan-state.svelte.ts`'s `discovered` getter filters discover-level activities by `message.includes('found')`; `processed` filters process-level by `message.includes('graph extracted')`. The unit-test fixtures in `scan-state.spec.svelte.ts` use messages like `"~/code/lumen · found root"` and `"app · 842/842 processed · graph extracted"` — the original daemon contract. The current daemon (`crates/senseid/src/tasks/handlers/scan.rs:27-56`) emits `"<path> · git folder"`, `"<path> · non-git sibling"`, `"<path> · standalone folder"` — none contain "found", so `discovered` is always 0 even when events arrive.

  2. *Daemon emits no progress events after the initial queue.* Only `scan.rs` (discover/info) and `process.rs` (one `Queue` + `folder_add` + `project_add` per folder) emit anything. The indexer pipeline queued at `process.rs:136+` runs to completion but emits no `folder_update` calls with incremented `files_completed`, no `Process`-level activity events, and no `project_update` flipping status from `indexing → active`. The progress bar can never animate because nothing tells the client a file was processed.

  Recommended fix order: (a) emit `folder_update` events from the file-indexing tasks (or a batching consumer) with running `files_completed` counts and a final `status=indexed` per folder; (b) emit `project_update` with `status=active` when all folders in a project are done; (c) decide whether to update `scan-state.svelte.ts` to count `discover` events by level alone (drop the substring filter) or to align the daemon's discover messages to include "found". The substring-matching approach is brittle either way — preferring counting by `level` and `status` field changes over message scraping.
