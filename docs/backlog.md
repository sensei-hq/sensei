...
| L10 | `hooks.client.ts:55-58` | Open | `pendingUpgrade` only checks `sensei:app-version` is non-null. Also revealed `DatabaseResolver.resolve()` shells to `sensei db:create` — a CLI subcommand that does not exist (the working `create + ensure_extensions + dbd deploy` flow was deleted in `7a73eb70` along with `_legacy/database.rs`). | (a) Narrow version-compare in reroute. (b) Restore the DB-create/deploy logic from legacy, decompose into `DatabaseResolver` (create + extensions) + `SchemaDeployResolver` (dbd deploy, shared with `UpgradeResolver`) + new `BrewUpdateResolver` + `UpgradeResolver`. Replace stub `run_upgrade_steps` to call UpgradeResolver. |
