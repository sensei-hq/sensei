...
| K3 | Homebrew — `sensei-dev.rb` formula | Missing `service do` block | `homebrew/Formula/sensei.rb` declares a `service do` block (run `senseid`, keep_alive, log paths) so `brew services start/stop sensei` works. `homebrew/Formula/sensei-dev.rb` does NOT — so even after `make install-dev` (which does run `brew install --HEAD sensei-hq/tap/sensei-dev`), `brew services start sensei-dev` fails because no `homebrew.mxcl.sensei-dev.plist` is generated. The `DaemonStartResolver` works because it spawns `senseid start` directly via the on-PATH binary, but operator-style `brew services` management is broken for dev. Add the same `service do` block to `sensei-dev.rb` (point `run` at `opt_bin/"senseid-dev"`, separate log paths under `var/"log/sensei-dev.log"`). Surfaced 2026-05-20 during cold-start E2E design. |
