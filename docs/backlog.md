...
|---|------|----------|-------|-----|
| L1 | `+layout.svelte:77-80` + `src-tauri/src/lib.rs:110-119` | Critical | `dev-navigate` Tauri event listener writes `sessionStorage.setItem('sensei:health', 'ready')` unconditionally. View-menu items (`Health`, `Observatory`, `Setup`, `Upgrade`) emit this event with no `#[cfg(debug_assertions)]` or `dev` feature gate, so the bypass ships in production. Any user clicking a View menu item sets `ready` and bypasses every subsequent gate. | Gate both the Rust menu registration AND the JS listener behind a dev-only cfg (e.g. `cfg(feature = "dev")` on Rust side, `import.meta.env.DEV` check on JS side). |
| L2 | `hooks.client.ts:60` | Critical | Upgrade gate `if (pendingUpgrade && !HEALTH_EXEMPT.has(path) && path !== '/')` excludes the observatory route from the upgrade redirect. If `sensei:app-version` is set in localStorage and the user lands on `/`, the upgrade step is silently skipped. | Remove the `&& path !== '/'` clause. The post-upgrade flag-clearing on the upgrade page already prevents the loop concern the author appears to have been guarding against. |
| L3 | `+layout.ts:14-19` + `appstate.svelte.ts:106-110` | Critical | Root layout `load()` calls `appState.load()` → `fetch('/api/config')` on every navigation, including `/health`. `senseiApi.get()` swallows the failure and returns `{}`, so the `/health` page makes a useless daemon round-trip and shows confusing failed `/api/config` requests in devtools even when the gate is working. | Move `appState.load()` out of root layout into `(observatory)` and `(project)` group layouts where it actually matters. |
| L4 | `hooks.client.ts:76` vs `appstate.svelte.ts.setupComplete` | Critical | Two sources of truth for setup-complete: `localStorage['sensei:setup-complete']` (read by reroute) and daemon `config.setup_complete` (read by app state). The localStorage write is unconditional; the daemon write swallows errors. Drift scenarios: setup finishes while daemon is unreachable → localStorage `1`, daemon never received it → next launch passes setup gate but `appState.setupComplete` is false → broken downstream. | Design decision: pick one canonical source. Probable answer is daemon-backed, gated only after daemon health is green. |
