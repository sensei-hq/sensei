...
| [#74](https://github.com/sensei-hq/sensei/issues/74) | Retention / pruning. **PARTIAL — structured-log TTL SHIPPED (`dedbc8f3`):** `tasks/log_pruner.rs` daemon task prunes `public.logs` older than `logs.retention_days` (default 30) every `logs.prune_interval_secs` (default daily); first tick on startup, retention re-read each tick; live (no >30d rows yet → correct no-op). **REMAINING — activity-data GC:** periodic prune of `assistant_events`/`turns`/`sessions`/`transcript_turns` older than a TTL, only after analysis has derived insights (keep `detected_patterns`/recommendations/teachings/memories). GC counterpart to the #65 analyzer + #73 backfill. |
