...
| [#76](https://github.com/sensei-hq/sensei/issues/76) | Table-driven gateway config from seed — daemon loads routers/models/**chains** from `gateway.*` (seeded via `import/staging/*.jsonl`) instead of hardcoded `baseline_production_config()`; default chains embedded→ollama→cloud (router-gated), embeddings 384-dim embedded→ollama; all tasks call via named chain/purpose; `EMBED` in all builds (CI native toolchain). Embedded chat already live via baseline (`679e9de1`); this moves it to the seed-driven path. |
