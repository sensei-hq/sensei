...
Each agent integration is implemented as a concrete `AgentAdapter` that extends `BaseAgentAdapter`. The reference implementation is `ClaudeAdapter`. Additional adapters cover opencode, Zed, Kilo, Kiro, Codex, Cursor, Copilot, Aider, and a `GenericAdapter` for agents with no structured skills concept. Each adapter is responsible for installing skills to the agent's expected location and format.
...
Skills are generated by a local model from the combined Layer 1 (`project_profile`) and Layer 2 (`project_config`) data. They are not templated — each skill is authored from the actual project context. Generated skills are stored in `.sensei/skills/` and installed by the appropriate `AgentAdapter` to the agent-specific location. Skills are regenerated whenever the profile or config changes, using the same trigger as incremental indexing.
...
`sensei.agents_config` stores per-repo agent preferences. When installing context files, Sensei checks which agents are configured for the repo and dispatches to the appropriate adapters. Agents that have no native skills or rules concept are handled by `GenericAdapter`, which writes skill references into `AGENTS.md` instead.

```gherkin
Feature: Agent Config and Capability Detection

  Scenario: Configured agents are detected and dispatched
    Given a repo where agents_config records Claude Code and Cursor as configured agents
    When Sensei runs the skills installation step
    Then ClaudeAdapter installs skills to .claude/skills/
    And CursorAdapter installs skills to .cursor/rules/
    And no other adapters are invoked

  Scenario: Unconfigured agents are not installed
    Given a repo where only Claude Code is configured in agents_config
    When Sensei runs the skills installation step
    Then CursorAdapter does not run
    And no .cursor/rules/ directory is created
  ...
  Scenario: Developer adds a new agent to agents_config
    Given a repo previously configured for Claude Code only
    When the developer adds opencode to agents_config
    And runs sensei install-skills
    Then OpenCodeAdapter runs and installs skills to .opencode/skills/
    And ClaudeAdapter re-runs to ensure its skills are current
```

### Token Usage Tracking Per Agent

Every task session records the agent name and version in `task_sessions`. Every turn records the model ID and provider in `task_turns`, handling mid-session model switches. This data enables cross-agent quality comparisons and per-model cost analytics in the dashboard.
...
Feature: Token Usage Tracking Per Agent
  ...
  Scenario: Session records agent name and version
    ...
    When the session is created in sensei.task_sessions
    Then agent_name and agent_version are recorded in the session row
    And these fields are set from the agent's self-reported identity or config

  Scenario: Each turn records model ID and provider
    Given an active task session
    When the agent makes a turn using claude-sonnet-4-5 from Anthropic
    Then a row is written to sensei.task_turns with model_id=claude-sonnet-4-5 and model_provider=anthropic
    And the token counts for that turn are recorded against that model

  Scenario: Mid-session model switch is captured per turn
    Given a session that started with claude-opus-4 and switched to claude-sonnet-4-5 on turn 5
    When token usage is aggregated for the session
    Then turns 1-4 are attributed to claude-opus-4
    And turns 5+ are attributed to claude-sonnet-4-5
  ...
  Scenario: Cross-agent quality comparison is available in analytics
    ...
    Then token usage per task type is broken down by agent_name and model_id
