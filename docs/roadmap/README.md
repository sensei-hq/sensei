...
| [05-coordinator-adapters](./05-coordinator-adapters.md) | Coordinator vs. model abstraction; Claude Code as default; adapter interface and registry |
...
|---|---|---|
| 0 — Simplify | Replace Supabase with SQLite, remove Docker dependency | Not started |
...
- **Coordinator-agnostic** — Claude Code is the default, not the only option. opencode,
  GitHub Copilot, Kiro CLI, OpenAI Codex, and others are supported via `CoordinatorAdapter`.
  See `05-coordinator-adapters.md`.
...
- Symbol indexing (L0–L3 resolution levels)
- MCP server (protocol-standard, works with any MCP-capable coordinator)
