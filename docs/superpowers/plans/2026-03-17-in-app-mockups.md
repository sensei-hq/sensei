# In-App Mockups Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `skills/building-app-mockups/SKILL.md` — a skill that builds UI design alternatives as real pages inside the existing app instead of standalone HTML mockups.

**Architecture:** Pure skill file. No new code. The skill guides an agent to create `/mockups/a`, `/mockups/b` pages using the project's actual framework and component library, iterate in the running app, and then promote the winning design to its final location.
...
### Task 1: Create `skills/building-app-mockups/SKILL.md`

**Files:**
- Create: `skills/building-app-mockups/SKILL.md`
...
```bash
cat skills/building-app-mockups/SKILL.md | head -10
```
Expected: frontmatter with `name: building-app-mockups` and a `description:` field.

- [ ] **Step 3: Self-check the description against the weakness rubric**

The description must:
- Open with `Use when <specific situation>` ✓ — "Use when designing new UI pages..."
- Name at least one proactive trigger ✓ — "Also use before any significant layout change..."
- State what the agent gains ✓ — "so design decisions happen in the real app, not in throwaway HTML"
- Be specific enough not to overlap with another skill ✓ — references specific frameworks and the /mockups/ route pattern
...
```bash
git add skills/building-app-mockups/SKILL.md
git commit -m "feat(skills): add building-app-mockups skill"
```
...
- [ ] **Verify skill appears in the skills directory**

```bash
ls skills/ | grep building-app-mockups
```
Expected: `building-app-mockups` listed.

- [ ] **Verify frontmatter is well-formed**

```bash
grep "^description:" skills/building-app-mockups/SKILL.md
```
