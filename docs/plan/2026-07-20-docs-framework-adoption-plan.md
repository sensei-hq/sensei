...
- [ ] **Step 1: Run the project scaffold.**
  ...
  Expected: a `[created]/[exists]` report; new `docs/vision/`, `docs/personas/`, `docs/roadmap/`, `docs/design/`, `docs/features/` (+ `docs/features/README.md`), `docs/decisions.md`; existing folders reported `[exists]`.
  Verify: `ls -d docs/vision docs/personas docs/roadmap docs/design docs/features` all exist.
...
- [ ] **Step 3: Rewrite `docs/README.md`** as the map. It MUST contain, in this order:
  ...
  2. A **stages × audiences table** with exactly these rows (Purpose + Primary audience + note), matching the design's Section 1:
     `vision/` (why; PO+all), `personas/` (who we serve; PO+designer), `journeys/` (end-to-end flows; PO+designer+dev), `roadmap/` (phases+status; PO), `design/` (design system + cross-cutting UX; designer+dev), `mockups/` (system-wide mockup bundle; designer+dev), **`features/<name>/` (the complete per-feature spec — the source of truth; PO validates + agent grounds + dev builds)**, `architecture/` (technical how, per surface; dev+agent), `spec/` (legacy per-screen build specs — **transitional**, folding into `features/*/design.md` or `architecture/` as touched), `plan/*` (dated **transient** build plans).
  3. A **reading paths** subsection: **PO** → `vision/` → `features/<name>/{brief,design}` → `journeys/`; **Designer** → `personas/` → `journeys/` → `mockups/` + `design/` → `features/<name>/mockup-ref.md`; **Dev/agent** → `features/<name>/` → `architecture/` → `plan/*`.
...
### Task 2: Move `vision.md` + `objectives.md` → `vision/`; fix inbound links

**Files:**
- Move: `docs/requirements/vision.md` → `docs/vision/vision.md`; `docs/requirements/objectives.md` → `docs/vision/objectives.md`
...
- Modify/create: `docs/vision/README.md`

- [ ] **Step 1: Move the files with git.**
  ```bash
  git mv docs/requirements/vision.md docs/vision/vision.md
  git mv docs/requirements/objectives.md docs/vision/objectives.md
  ```
...
- [ ] **Step 2: Find every inbound link** (both `requirements/vision` and `requirements/objectives`):
  ...
  Expected references in the files listed above (the design doc `2026-07-20-docs-framework-adoption-design.md` legitimately *describes* the move — leave its "current → target" mentions alone).

- [ ] **Step 3: Rewrite the links.** In each referencing file, replace `requirements/vision.md` → the correct relative path to `docs/vision/vision.md` and `requirements/objectives.md` → `docs/vision/objectives.md`. Mind relative depth: from `docs/plan/*` it's `../vision/vision.md`; from `docs/spec/screen/*` and `docs/spec/pipeline/*` it's `../../vision/vision.md`. Edit each explicitly (do not blanket-sed across differing depths).

- [ ] **Step 4: Write `docs/vision/README.md`** — a short index: purpose ("why sensei exists; the north-star and objectives; audience: product owner + everyone"), and links to `vision.md` (north-star FTR + the core loop + themes) and `objectives.md` (measurable "met when" per segment + Dōjō). If scaffold already created a `vision/README.md` stub, replace its body with this.

- [ ] **Step 5: Verify.** `grep -rn "requirements/vision\|requirements/objectives" docs/ | grep -v "2026-07-20-docs-framework-adoption-design"` → **empty** (no stale links). `ls docs/vision/vision.md docs/vision/objectives.md` → both exist. `ls docs/requirements/vision.md 2>&1` → "No such file".

- [ ] **Step 6: Commit.**
  ```bash
  git add docs/vision docs/plan/README.md docs/plan/operating-model.md docs/plan/decisions.md docs/spec/pipeline/insights.md docs/spec/screen/dojo-developer-console.md docs/spec/screen/relay-security.md docs/spec/screen/relay-respond.md docs/spec/screen/relay-task-detail.md
  git commit -m "docs: move vision + objectives into vision/; fix inbound links"
  ```
...
- [ ] **Step 3: Write `docs/personas/README.md`** — a short stub: purpose + audience ("who sensei serves and their goals; audience: product owner + designer") and a one-line note "to be filled — the current audience segments are described in `../vision/objectives.md` (per-segment) meanwhile." (Do not invent personas.)
...
- [ ] **Step 2: Retire `requirements/README.md` to a pointer** — since `vision.md`/`objectives.md` moved to `vision/` and `front-door.md` moved to `features/front-door/`, replace the body with: purpose is superseded → see the top-level map (`../README.md`); vision/objectives now in `../vision/`; feature specs in `../features/`. (Keep the folder as a redirect rather than deleting, per the incremental policy.)

- [ ] **Step 3: Add a "Migration policy" section to `docs/README.md`** (short): the canonical structure is adopted; migration is **feature-by-feature as each is touched**; `requirements/` is fully absorbed (vision/objectives → `vision/`, front-door → `features/front-door/`) and left as redirects; `spec/` folds into `features/*/design.md` or `architecture/` as each screen is next worked; dated `plan/*` are transient. Nothing is deleted until its content is migrated; history is in git.
...
- [ ] **Structure:** `ls -d docs/{vision,personas,journeys,roadmap,design,mockups,features,architecture}` all exist; `docs/features/front-door/` has `brief.md design.md plan.md tests/acceptance.md decisions.md mockup-ref.md`.
...
- **Link-depth care (T2):** relative paths differ by folder depth (`../vision/` from `plan/`, `../../vision/` from `spec/screen/`) — edit per file, no blanket sed.
