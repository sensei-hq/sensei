...
- **G4b — `context_pack` built** (`88ac3d00`). New MCP tool: top-8 hybrid hits + their on-disk code snippets (clamped, 40-line cap) in one call — concept-level retrieval with ready code, not just locations. **Live: verified returning symbols + 9–23-line snippets.** Grep floor = the lexical-fusion half (symbol-name ILIKE); raw file-content grep deferred (use ripgrep).
...
### G9 — Capture + index-reliability residuals *(objective F2, foundation)* — 🔴 WORSE THAN THOUGHT: incremental indexing is BROKEN (diagnosed 2026-07-15)
**Newly found + confirmed:** the watcher doesn't just enqueue *too few* post-processing tasks — the ones it enqueues **silently no-op**. `process_batch` passes the watch-root **NAME** (`file_name`, e.g. "sensei"/"Developer") as `folder_path`, but the handlers resolve via `get_repo_by_path` = `WHERE abs_path=$1` → a name never matches → `folder_id=None` → all writes are `Some(folder_id)`-guarded no-ops. Empirical: **26,123 `process_file` + 6,175 `resolve_edges` + 2,118 `delete_file`** task_executions with bare-name `folder_path`. So the code graph updates **only on full `ScanRoot` scans** (boot + scheduler), never live on edit — which also starves the G4 hybrid search of fresh edits. Watch roots are MIXED (repos AND parent dirs like `~/Developer`), so a per-file owning-repo resolution is required (`PgStore::find_folder_for_path` exists). Blocker: `RootWatcher` has no `pg` handle at enqueue time. See memory `project_incremental_watcher_broken`.
**Approach (revised):** give the watcher a PgStore handle; in `process_batch` resolve each changed file → owning repo (longest-prefix `find_folder_for_path`) → group by repo abs_path → enqueue `ProcessFile`/`ResolveEdges`/**`EmbedNodes`** with the repo abs_path; TDD with an injected resolver; verify live (edit a file → node re-embeds). THEN the original residuals: cursor persistence, `.git/HEAD` watch, overflow→reconcile, residue sweep. WS D.
