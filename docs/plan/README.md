...
**Residuals (still open, WS D):** FSEvents cursor persistence, `.git/HEAD` watch (branch-switch reconcile — partially present via `is_branch_switch`), overflow→force reconcile, the ~54 twinless residue-node sweep; `BuildConnections`/`DetectCommunities` deliberately left periodic (per-edit clustering is wasteful).
