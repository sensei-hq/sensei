...
- **Tier-2 consolidation ✅ WIRED 2026-07-15.** The full generator already existed but was **manual-only** (`POST /api/knowledge/rules/consolidate`) — so `consolidated_rulesets` stayed 0. Extracted the pipeline into a shared orchestrator `analysis::rule_consolidation::consolidate_global_rules` (resolve global rules → `structure_ruleset` → **source-hash skip guard** → gemma4 `reasoning`-chain merge → insert `proposed`), called by BOTH the HTTP handler and a new **`TaskKind::ConsolidateGovernance`** enqueued each analyzer tick alongside the other global passes (cheap no-op when the ruleset is unchanged). **Live: with 0 global rules it correctly skips; seeding 2 rules → gemma4 merged them into grouped-by-enforcement markdown, `consolidated_rulesets` 0→1 (`global` v1 proposed), probe data cleaned up.** *Nuance:* `consolidated_rulesets`=0 was **two** gaps — never-scheduled (fixed) AND no global-rules corpus (a G6-adjacent data gap; there are no user/general-scope rule memories yet). Full suite 1493 pass / clippy clean.
- **Orphan tables ✅ DROPPED 2026-07-15** (user sign-off). The 6 dead husks — `inference.insights` + `insight_batches` + `hyperedges` + `hyperedge_members`, `gateway.inference_assignments`, `staging.hook_events` — plus the dead `sensei.hyperedge_kind` enum, all verified empty + not-in-DDL + 0 writers/readers/dependents, `DROP`ped from `sensei` + `sensei_test` (aligns the live DBs to the design; the daemon boot-deploy is `create if not exists`, so they never came back). *(`inference.insights` was the abandoned "collective-intelligence sharing references" materialised-insight design — never got a writer; the real insight model is the live aggregator, see [`pipeline/insights`](../spec/pipeline/insights.md).)*
- *Remaining G7:* impact-writer decision, testability/benchmarks.
...
Worker + Supabase) and the federation backend now lives in the dojo Worker's `/v1`
(the `dojo-mind` Rust service was removed once its surface was ported) — so the real work
is **the remaining console `/v1` routes + the join flow** (Phase 4), not an infra decision.
...
- **Infra is decided** — the Dōjō cloud runs on a **Cloudflare Worker + cloud Supabase** (`dojo/`), auth via kavach; the old "external-blocked on a SaaS-infra decision" framing is **stale**. The federation backend now lives **in the dojo Worker's `/v1`** (triage/approve/distribute, identity, policies, engagements, incidents, compliance); the old `dojo-mind` Rust service (`sensei-dojo` binary) was **removed** once its surface was ported. senseid's federation (rules + artifacts) targets the Worker over the `dojo_protocol` wire. The gap is the human **console**.
- **Console backend — port `/v1` to the Worker** *(decision 2026-07-16: Path A)* — the old `dojo-mind` `/v1/t/{tenant}/…` logic was ported route-by-route into the Worker (`engagements/+server.ts` was the reference), rather than deploying `sensei-dojo` + proxying (Path B, rejected — no deploy artifact). Order was: triage → members/identities/policies → incidents/engagements+bind → audit/compliance → health. The Rust crate is now removed; keep the ported TS triage scoring / k-anonymity logic correct.
