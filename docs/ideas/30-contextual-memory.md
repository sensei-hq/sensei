...
| # | Decision |
|---|----------|
| 1 | **Preferences merge into memory.** Preferences are learned memories with category=code_style, naming, etc. `inference.preferences` table will be eliminated. |
| 2 | **Conflicting memories require human resolution.** Both "use mocks for speed" and "don't mock the database" can be valid in different contexts. When conflict is detected (opposite memories in same scope), flag for user resolution. Elaboration may resolve it — "use mocks for unit tests, real DB for integration tests" becomes a refined memory that supersedes both. |
| 3 | **Assistant can create memories.** Via `report_learning()` MCP tool. User can validate, enhance, or dismiss. Auto-created memories start at strength=0.5 (below correction-derived memories at 1.0) until user confirms. |
| 4 | **Bloat prevention: merge + hard cap.** Memories with overlapping scope and similar content should be merged (consolidate examples, combine reasoning). MOE panel can assist with consolidation — "these 5 memories about auth are really 2 core concepts with examples." Hard cap per scope level (e.g., 50 active memories per project, 20 global). When cap is reached, weakest memories are archived. |
| 5 | **Stack-scoped memories are on-demand.** Learning happens at language/framework level scope. Assistant calls `get_memories(scope=stack:rust+axum)` to fetch tech-stack memories. Not auto-loaded — fetched when context demands it. |
| 6 | **MOE panel sees all memories.** Access to full memory enables consolidation, conflict detection, and better reasoning about recommendations. High-strength memories weighted more in panel prompts. |
| 7 | **Auto-generate "because" where possible.** If the LLM knows the structure (correction → consequence), it generates the reasoning. The correction text provides the raw material; MOE panel enriches with impact analysis. User can always edit. |
| 8 | **Context management via sensei context pack tool.** Current 1M context windows are large but sessions lose critical information (rules, memories) as context fills with code and file scans. Sensei should provide a context management tool that: (a) snapshots where we are + what we're doing + what's pending, (b) clears accumulated noise, (c) reloads system memory + project memory + snapshot memory. The assistant or user triggers this when context gets bloated. Token budget is less about hard limits and more about smart context rotation. |
| 9 | **No git diff in continuity memories.** Git diffs are bloated and don't provide insight. Instead, the inferred summary ("completed steps 1-3, pending step 4, auth/refresh.ts has uncommitted changes") is more useful. Git diff can be used as INPUT to infer the summary, but the memory stores the human-readable inference, not the raw diff. |
| 10 | **Separate table(s) in inference schema.** Memory is NOT in the nodes table — different purpose, different lifecycle. But memories reference nodes via edges (good_example, bad_example, pattern, evidence links). The graph connection is through edges, not by being nodes themselves. Open: what triggers a memory retrieval — a query/question system, scope matching, or embedding similarity? Likely a combination. |

## Open questions (remaining)

| # | Question |
|---|----------|
| 1 | Memory table design: single table with scope columns + jsonb references, or multiple tables (global_memories, project_memories, etc.)? Single table with scope enum seems simpler. |
| 2 | How does memory consolidation work mechanically? MOE panel proposes a merged memory → user approves → original memories archived, merged memory takes their combined strength? |
| 3 | Should memories have embeddings for similarity-based retrieval? "Working on auth" → fetch memories similar to "auth" by embedding distance? Adds a vector column but enables fuzzy matching beyond exact scope. |
| 4 | Context pack tool UX: should the assistant auto-detect when context is getting bloated and suggest a context rotation? Or only on explicit user request (`/sensei:refocus`)? |
| 5 | How do memory references (edges to nodes) survive when code changes? If `auth_adapter.rs:14` is refactored, the node ID stays but content changes. Do we re-validate references on index? |
