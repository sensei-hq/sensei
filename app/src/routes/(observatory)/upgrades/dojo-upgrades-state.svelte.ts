...
import type { DojoUpgrade, DojoUpgradeScope } from '$lib/types.js';

// The type pill + attribution line are shared with the upstream Share-review
// gate, so they live in `$lib/dojo-artifacts` and are re-exported here to keep
// this module's public surface (and the `+page.svelte` / spec imports) stable.
// `ChipClasses` is also imported for local use as `stateChip`'s return type.
import { typePill, attributionSummary, type ChipClasses, type TypePill } from '$lib/dojo-artifacts.js';
export { typePill, attributionSummary };
export type { ChipClasses, TypePill };
