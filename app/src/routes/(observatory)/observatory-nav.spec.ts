...
describe('buildNavItems', () => {
  ...
  it('hides Review group, separator and Preferences in Focus', () => {
    ...
    expect(entries.some((e) => 'type' in e && e.type === 'separator')).toBe(false);
  });
  ...
  it('renders a separator before Preferences in the full rail', () => {
    const entries = buildNavItems({ focus: false });
    const sepIdx = entries.findIndex((e) => 'type' in e && e.type === 'separator');
    const prefIdx = entries.findIndex((e) => 'href' in e && e.href === '/settings');
  ...
  it('labels /settings "Preferences"', () => {
    expect(byHref(buildNavItems({ focus: false }), '/settings')?.text).toBe('Preferences');
  });
  ...
  it('uses the project count as the Projects badge', () => {
    expect(byHref(buildNavItems({ focus: false, projectCount: 12 }), '/projects')?.badge).toBe(12);
  });
