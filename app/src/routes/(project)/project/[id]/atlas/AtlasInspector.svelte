...
<div class="py-5 px-4 h-full overflow-auto" data-component="atlas-inspector">
  {#if community}
    ...
    <div class="grid grid-cols-[1fr_auto] gap-y-2 mt-4">
      <span class="text-sm text-ink-soft">Members</span>
      <span class="font-mono text-sm text-ink">{community.nodeCount}</span>
      <span class="text-sm text-ink-soft">Share</span>
      <span class="font-mono text-sm text-ink">{community.sharePct}%</span>
    </div>

    <p class="text-sm text-ink-soft leading-relaxed mt-4 mb-0">
  ...
  {:else if symbol}
    ...
    <div class="grid grid-cols-[1fr_auto] gap-y-2 mb-4">
      <span class="text-sm text-ink-soft">Connections</span>
      <span class="font-mono text-sm text-ink">{symbol.degree}</span>
    </div>
  ...
  {:else}
    ...
    <div class="grid grid-cols-[1fr_auto] gap-y-2 mb-4">
      <span class="text-sm text-ink-soft">Nodes</span>
      <span class="font-mono text-sm text-ink">{overview.nodes}</span>
      <span class="text-sm text-ink-soft">Relations</span>
      <span class="font-mono text-sm text-ink">{overview.relations}</span>
      <span class="text-sm text-ink-soft">Communities</span>
      <span class="font-mono text-sm text-ink">{overview.communities}</span>
    </div>
    {#if capped}
