<script lang="ts">
	...
	// Overview — the maintainer dashboard (mockup DojoOverview). Rendered for a
	// MEMBER (the console index branches on hasMembership). The pending-triage
	// count + top-of-queue preview are live (queue from list_triage); the
	// remaining metric cards, confidentiality note, recent activity and the
	// published-adoption table are the design's static demo content until those
	// endpoints land in a later chunk. Presentational — it takes queue/org/error.
	let {
		queue,
		org,
		error
	}: { queue: TriageRow[]; org: DojoOrg | undefined; error: string | null } = $props();

	const m = metrics;
	const pending = $derived(queue.length);
	const preview = $derived(queue.slice(0, 4));
...
<div class="bg-paper flex h-full w-full flex-col overflow-hidden">
	<ConsoleHead
		kanji="全"
		eyebrow="{org?.name ?? 'Your org'} · Dōjō"
	...
	>
		{#snippet right()}
			<div class="mono text-ink-mute text-right text-xs" style="line-height: 1.7">
				<div>{m.approvedWeek + pending} artifacts · {org?.members ?? 0} members</div>
