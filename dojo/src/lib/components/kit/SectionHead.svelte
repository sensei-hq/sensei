<script lang="ts">
	...
	// · optional count · optional right slot. Eyebrow uses the tracked-uppercase
	// micro-label treatment; title is the display face at the section size.
	let {
		kanji,
		eyebrow,
		title,
...
<div class="border-paper-edge flex items-baseline gap-3 border-b" style="padding-bottom: 12px">
	{#if kanji}<KanjiToken char={kanji} size="lg" toneClass="text-ink-mute" />{/if}
