...
import { navGroupsFor } from '$lib/dojo2-nav';
import { toKitDojos, toKitOrg } from '$lib/dojo2-chrome';
...
// in dojo2-nav.spec.ts + dojo2-chrome.spec.ts; here we render the kit chrome the
// (dojo2) layout drives (nav groups per context + role rank; the switcher lists
...
describe('dojo2 shell — nav groups render per context + role rank', () => {
	afterEach(cleanup);
...
describe('dojo2 shell — org switcher lists memberships', () => {
	afterEach(cleanup);
