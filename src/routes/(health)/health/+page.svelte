<script lang="ts">
  ...
  function statusColor(s: GateStatus): string {
    if (s === 'ready') return 'oklch(var(--color-success-z5) / 1)';
    if (s === 'missing' || s === 'blocked') return 'oklch(var(--color-primary-z5) / 1)';
    if (s === 'checking' || s === 'installing' || s === 'starting') return 'oklch(var(--color-surface-z7) / 1)';
    return 'oklch(var(--color-surface-z5) / 1)';
  }

  function pillBg(s: GateStatus): string {
    if (s === 'ready') return 'oklch(var(--color-success-z5) / 0.10)';
    if (s === 'missing' || s === 'blocked') return 'oklch(var(--color-primary-z5) / 0.08)';
    if (s === 'checking' || s === 'installing' || s === 'starting') return 'oklch(var(--color-surface-z2) / 1)';
    return 'transparent';
  }
</script>
    ...
    <div class="max-w-[760px] w-full mx-auto flex flex-col gap-10 pb-10">
      ...
      <div>
        <div class="flex items-center gap-2.5 mb-3.5">
          <span class="kanji text-[22px] text-primary-z5">支</span>
          <span class="text-[11px] tracking-[0.14em] uppercase text-surface-z6">bootstrap · checking the foundation</span>
        </div>
        <h1 class="display text-[38px] font-light leading-[1.12] mb-3.5 tracking-[-0.015em]">
          {#if bs.allReady}
            The foundation <span class="text-success-z5">holds.</span>
          {:else if bs.firstBlockedIdx >= 0}
            A few pieces are <span class="text-primary-z5">missing.</span>
          {:else}
            Checking the foundation…
          {/if}
        </h1>
        <p class="text-sm text-surface-z6 leading-[1.7] max-w-[540px]">
          {#if bs.allReady}
      ...
      <div class="flex items-center gap-3">
        <div class="text-[10px] tracking-[0.14em] uppercase text-surface-z5 tabular-nums whitespace-nowrap">
        ...
        <div class="flex-1 flex gap-[3px]">
          {#each bs.gates as gate}
            <span class="flex-1 h-[2px] rounded-[1px] transition-colors duration-300" style="background: {statusColor(gate.status)}; opacity: {gate.status === 'waiting' ? 0.5 : 1};"></span>
          {/each}
        </div>
      </div>
    </div>
    ...
    <div class="max-w-[760px] w-full mx-auto">
      ...
      <div class="flex flex-col border-t border-surface-z2">
        {#each bs.visibleGates as gate, i (gate.id)}
          ...
          <div class="gate-row border-b border-surface-z2 py-4 transition-opacity duration-300" class:pending={isPending}>
            <!-- Main row -->
            <div class="grid grid-cols-[32px_1fr_auto] gap-4 items-center">
              <div class="kanji text-[22px] text-center" style="color: {statusColor(gate.status)};">{gate.n}</div>
              <div>
                <div class="flex items-baseline gap-2.5">
                  <span class="display text-[17px] font-normal">{gate.name}</span>
                  <span class="text-xs text-surface-z5">· {gate.detail}</span>
                </div>
                <div class="mono text-[11px] text-surface-z5 mt-1">{gate.check}</div>
              </div>
              <div class="inline-flex items-center gap-1.5 px-[10px] py-1 rounded tabular-nums text-[11px] tracking-[0.08em] uppercase" style="color: {statusColor(gate.status)}; background: {pillBg(gate.status)};">
                {#if isBusy}<span class="spinner"></span>{/if}
                {#if isReady}<span class="text-[10px]">✓</span>{/if}
                {#if isBlocked}<span class="text-xs">·</span>{/if}
                {gate.status}
              </div>
            </div>
            ...
            {#if gate.sub && (isBusy || isBlocked || isReady)}
              <div class="sub-checks mt-3 ml-12 flex flex-col gap-1 pl-[14px]">
                {#each gate.sub as sub}
                  {@const subStatus = isReady ? 'ready' : isBusy ? 'checking' : 'missing'}
                  <div class="flex items-center gap-2.5">
                    <span class="w-1.5 h-1.5 rounded-full inline-block" style="background: {statusColor(subStatus)};"></span>
                    <span class="text-xs text-surface-z7">{sub.name}</span>
                    <span class="mono text-[11px] text-surface-z5">{sub.check}</span>
                  </div>
                {/each}
              </div>
            {/if}
            ...
            {#if showRemedy && gate.remedy !== 'prereq'}
              <div class="mt-4 ml-12 px-5 py-[18px] bg-surface-z2 border border-surface-z2 rounded-md">
                {#if gate.remedy === 'install'}
                  <div class="display text-[15px] mb-1">{bs.platformInfo.pkgmgr_remedy.title}</div>
                  <p class="text-[13px] text-surface-z6 leading-[1.6] mb-3.5">{bs.platformInfo.package_manager} is the base that installs everything else.</p>
                  <div class="bg-surface-z1 border border-surface-z2 rounded-md px-3 py-[10px] mb-3">
                    <code class="font-mono text-xs text-surface-z9 break-all">{bs.platformInfo.pkgmgr_remedy.command}</code>
                  </div>
                  <div class="flex gap-2.5 items-center">
                    {#if bs.platformInfo.pkgmgr_remedy.url}
                      <a href={bs.platformInfo.pkgmgr_remedy.url} target="_blank" rel="noreferrer" class="btn-outline btn-sm">Learn more <span class="text-surface-z6">↗</span></a>
                    {/if}
                    <button class="btn-solid btn-sm" onclick={() => retry(gate.id)}>I've installed it — retry</button>
                  </div>

                {:else if gate.remedy === 'db'}
                  <div class="display text-[15px] mb-1">Could not create the sensei database</div>
                  <p class="text-[13px] text-surface-z6 leading-[1.6] mb-3.5">Postgres is running but sensei couldn't create its database automatically.</p>
                  <div class="bg-surface-z1 border border-surface-z2 rounded-md px-3 py-[10px] mb-3">
                    <code class="font-mono text-xs text-surface-z9 break-all">createdb sensei && psql sensei -c 'CREATE EXTENSION IF NOT EXISTS vector;'</code>
                  </div>
                  <div class="flex gap-2.5 items-center">
                ...
                {:else if gate.remedy === 'daemon'}
                  <div class="display text-[15px] mb-1">Daemon failed to start</div>
                  <p class="text-[13px] text-surface-z6 leading-[1.6] mb-3.5">The database is reachable but the daemon did not come up.</p>
                  <div class="flex gap-2.5 items-center">
                    <button class="btn-solid btn-sm" onclick={() => retry(gate.id)}>Retry</button>
                  </div>
                {/if}
      ...
      {#if bs.needsPrereqInstall}
        <div class="mt-6 px-6 py-5 bg-surface-z2 border border-surface-z2 rounded-md">
          <div class="display text-[15px] mb-1">{bs.platformInfo.prereq_remedy.title}</div>
          <div class="flex gap-1.5 flex-wrap mt-2 mb-3">
            {#each bs.missingPrereqGates as gate}
              <span class="text-[11px] tracking-[0.06em] uppercase text-primary-z5 px-2 py-[3px] rounded-[3px]" style="background: oklch(var(--color-primary-z5) / 0.08);">{gate.name}</span>
            {/each}
          </div>
          <p class="text-[13px] text-surface-z6 leading-[1.6] mb-3.5">
          ...
          {:else}
            <div class="bg-surface-z1 border border-surface-z2 rounded-md px-3 py-[10px] mb-3">
              <code class="font-mono text-xs text-surface-z9 break-all">{bs.platformInfo.prereq_remedy.command}</code>
            </div>
            <div class="flex gap-2.5 items-center">
      ...
      <!-- Footer -->
      <div class="flex justify-between items-center gap-4 pt-[22px] border-t border-surface-z2">
        <div class="text-[11px] text-surface-z5 leading-[1.6]">
