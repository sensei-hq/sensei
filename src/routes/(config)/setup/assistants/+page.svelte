...
<div>
  <p class="text-sm text-surface-z6 leading-[1.6] m-0 mb-6">
  ...
  <div class="grid grid-cols-2 gap-3">
    {#each assistants as fam (fam.id)}
      ...
      <button
        class="card flex items-center gap-4 px-6 py-5 rounded-lg cursor-pointer text-left bg-surface-z1 border border-surface-z3 transition-all duration-[140ms] min-w-0"
      ...
      >
        <div class="flex-1 min-w-0">
          <div class="flex items-baseline justify-between gap-2 mb-1.5">
            <span class="text-[15px] font-semibold">{fam.name}</span>
            {#if anyInstalled}
              <span class="text-[11px] text-surface-z5 whitespace-nowrap">{installedCount} detected</span>
            {:else}
              <span class="text-[11px] text-surface-z5 italic whitespace-nowrap">not found</span>
            {/if}
          </div>
          <div class="flex flex-wrap gap-[5px]">
            {#each caps(fam.id) as cap}
              <span class="chip text-[11px] font-mono text-surface-z7 px-2 py-[2px] bg-surface-z3 rounded-[3px] whitespace-nowrap">{cap}</span>
            {/each}
          </div>
        </div>
        <div class="checkbox w-[22px] h-[22px] rounded-[4px] border-2 border-surface-z3 flex items-center justify-center shrink-0 bg-surface-z1 transition-all duration-[140ms]" class:checked={fam.selected}>
          {#if fam.selected}<span class="text-surface-z1 text-xs font-bold">&#10003;</span>{/if}
        </div>
      </button>
    {/each}
  </div>
  ...
  {#if assistants.length === 0}
    <p class="text-[13px] text-surface-z6 italic">No AI coding assistants detected. Make sure the daemon is running.</p>
  {/if}
</div>
