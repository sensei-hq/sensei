...
<div class="max-w-[960px]">
  {#if !started}
    <div class="flex flex-col items-center text-center gap-6 bg-surface-z2 rounded-lg px-10 py-12">
      <div class="kanji text-[80px] text-primary-z5 opacity-25 leading-none">探</div>
      <p class="text-[15px] text-surface-z7 max-w-[440px] leading-[1.6] m-0">
  ...
  {:else}
    <!-- Stats bar -->
    <div class="grid grid-cols-4 gap-4 mb-6 pb-6 border-b border-surface-z2">
      {#each [
        { value: rootCount, label: 'ROOTS' },
        { value: activities.discovered, label: 'DISCOVERED' },
        { value: activities.queued, label: 'QUEUED' },
        { value: activities.processed, label: 'PROCESSED' },
      ] as stat}
        <div class="text-center">
          <div class="display text-[28px] leading-[1.2]">{stat.value}</div>
          <div class="text-[10px] tracking-[0.12em] text-surface-z6 mt-1">{stat.label}</div>
        </div>
      {/each}
    </div>
    ...
    <div class="grid grid-cols-[1fr_340px] gap-6">
      ...
      <div class="flex flex-col gap-4">
        {#each projects.items as proj (proj.id)}
          ...
          <div class="bg-surface-z2 rounded-lg p-5 border border-surface-z3">
            <div class="flex justify-between items-start mb-3">
              <div>
                <div class="text-[15px] font-semibold">{proj.name}</div>
                <div class="text-xs text-surface-z6 mt-0.5">{path} · {folderCount} folders · {readyCount} ready</div>
              </div>
              <span class="text-[10px] tracking-[0.1em] text-surface-z6 font-semibold" class:text-success-z5={proj.status === 'active'}>{proj.status.toUpperCase()}</span>
            </div>

            {#if totalFiles > 0}
              <div class="text-[13px] text-surface-z7 font-mono mb-3">{completedFiles.toLocaleString()} / {totalFiles.toLocaleString()}</div>
            {/if}

            <div class="flex flex-col gap-2">
              {#each proj.folders as f (f.id)}
                <div class="flex items-center gap-3">
                  <span class="text-[13px] font-medium min-w-[120px]">{f.name}</span>
                  {#if f.stack.length > 0}
                    <span class="text-[11px] text-surface-z6 min-w-[80px]">{f.stack.join(', ')}</span>
                  {:else if f.filesTotal > 0}
                    <span class="text-[11px] text-surface-z6 min-w-[80px]">{f.filesTotal}f</span>
                  {/if}
                  {#if f.filesTotal > 0}
                    <div class="flex-1 h-1 bg-surface-z3 rounded-[2px] overflow-hidden">
                      <div class="h-full bg-warning-z5 rounded-[2px] transition-[width] duration-300" style="width: {(f.filesCompleted / f.filesTotal) * 100}%"></div>
                    </div>
                  {/if}
                </div>
              {/each}
        ...
        {#if projects.items.length === 0 && started}
          <div class="text-[13px] text-surface-z5 p-6 text-center">Discovering projects...</div>
        {/if}
      </div>

      <!-- Right: Activity feed -->
      <div class="bg-surface-z2 rounded-lg p-4 border border-surface-z3 max-h-[600px] flex flex-col">
        <div class="flex justify-between pb-3 border-b border-surface-z2 mb-3 text-[11px] text-surface-z6">
          <span class="mono">SSE · /EVENTS</span>
          <span class="mono text-surface-z7">{activities.totalElapsed.toFixed(1)}s</span>
        </div>
        <div class="flex-1 overflow-y-auto flex flex-col gap-[2px]">
          {#each activities.recent as evt (evt.id)}
            <div class="flex gap-3 text-[11px] py-[2px] items-baseline">
              <span class="mono text-surface-z5 min-w-[52px] text-right">+{evt.elapsed.toFixed(2)}s</span>
              <span class="font-medium min-w-[56px]" style="color: {LEVEL_COLORS[evt.level] ?? 'oklch(var(--color-surface-z6) / 1)'}">{evt.level}</span>
              <span class="text-surface-z7 flex-1">{evt.message}</span>
            </div>
          {/each}
        </div>
      </div>
