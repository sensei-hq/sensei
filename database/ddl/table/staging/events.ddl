...
create unique index if not exists events_id_ukey
    on events(id);

comment on table events is
'Intermediate import buffer for bulk-loading rows into sensei.events.
Fields match sensei.events with staging-specific tracking columns.';
...
comment on column events.folder_id is 'UUID of the folder associated with this event; resolved against sensei.folders on import.';
