...
const ALWAYS_IGNORE = ["**/.git/**", "**/.sensei/**", "CLAUDE.md"];
...
export async function reindexRepo(
  ...
  const docIndexPath = join(repoPath, ".sensei/doc-index.json");
  ...
  await Promise.all([
    writeFile(join(repoPath, ".sensei/stack.md"), formatStack(stack)),
    writeFile(join(repoPath, ".sensei/shortcuts.md"), formatShortcuts(shortcuts)),
    writeFile(join(repoPath, ".sensei/symbol-map.json"), JSON.stringify(symbolMap, null, 2)),
    writeFile(join(repoPath, ".sensei/doc-index.json"), JSON.stringify(newDocIndex, null, 2)),
    writeFile(join(repoPath, ".sensei/traceability.json"), JSON.stringify(traceability, null, 2)),
    ensurePatternsMd(repoPath),
  ]);

  if (!existsSync(join(repoPath, ".sensei/llmspec.yaml"))) {
    await writeFile(join(repoPath, ".sensei/llmspec.yaml"), generateLlmSpecTemplate(repoPath, stack, shortcuts));
  }
...
async function buildTraceability(
  ...
  const llmspecPath = join(repoPath, ".sensei/llmspec.yaml");
...
async function ensurePatternsMd(repoPath: string): Promise<void> {
  const path = join(repoPath, ".sensei/patterns.md");
  if (!existsSync(path)) {
...
async function generateLlmsTxt(repoPath: string): Promise<void> {
  ...
  const name = repoPath.split("/").at(-1) ?? "project";
  await writeFile(join(repoPath, ".sensei/llms.txt"),
    `# ${name}\n\n${readme.split("\n").slice(0, 20).join("\n")}\n\n> Generated by sensei\n`);
}
