...
//! (resolve `key → metric_id` via the active registry, write a daily project-scope
//! row to `sensei.project_metrics` via [`PgStore::upsert_project_metric`]) for ONE
//! project. The single row is `grain = daily`, `folder_id` NULL (project scope),
//! `computed_on = today` — a SNAPSHOT (see below), matching the other snapshot
//! computers (`churn`'s `rework_density`, `duplication`'s `duplication_ratio`).
//!
...
//! measure (a 0/0 would be a fabricated zero, not a measured one).
//!
...
//! a real `denominator >= 1` with 0 memories writes a real `0.0`. Strict project
//! scoping (patterns via `project_id`, corrections via `project_ids` membership,
//! memories via `project_id`) — another project's items never leak in.
...
mod tests {
    ...
    use crate::tasks::test_support::{
        cleanup_metrics_fixture, make_ctx, purge_corrections, seed_correction, seed_memory,
        seed_metrics_project_folder, seed_pattern_with_instances,
    };
