...
pub async fn scan_doc_drift(ctx: &TaskContext, task: &Task) -> Result<u32, String> {
    ...
    let (scanned, new_broken, resolved) =
        (field("scannedDocs"), field("newBroken"), field("resolved"));
    tracing::info!(
...
mod tests {
    ...
    async fn flags_broken_only_after_a_real_symbol_is_removed_then_resolves() {
        ...
        std::fs::write(
            ...
            format!("The `{sym}` helper is the entry point."),
        )
        .unwrap();
        let abs = dir.path().to_string_lossy().to_string();

        let pid = pg
            .create_project(&format!("_test:drift-{}", uuid::Uuid::new_v4()), None, None)
            .await
            .unwrap();
        let root = pg
            .add_watch_root(
                &format!("/_test/drift-root-{}", uuid::Uuid::new_v4()),
                "t",
                &serde_json::json!([]),
            )
            .await
        ...
        .unwrap();
        assert_eq!(
            scan_doc_drift(&ctx, &task).await.unwrap(),
            0,
            "a resolvable mention is not drift"
        );
        ...
        sqlx_core::query::query(
            "DELETE FROM sensei.nodes WHERE folder_id = $1 AND kind = 'function' AND name = $2",
        )
        .bind(fid)
        .bind(&sym)
        .execute(pg.pool())
        ...
        assert_eq!(
            scan_doc_drift(&ctx, &task).await.unwrap(),
            1,
            "removed-symbol mention ⇒ one broken row"
        );
        ...
        let pool = pg.pool();
        sqlx_core::query::query("DELETE FROM inference.drift_items WHERE folder_id = $1")
            .bind(fid)
            .execute(pool)
            .await
            .ok();
