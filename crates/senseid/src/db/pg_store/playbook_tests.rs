...
async fn playbook_rules_load_and_run_roundtrip() {
    ...
    assert_eq!(
        row,
        ("greenfield".into(), "feature".into(), "high".into(), "spec_driven".into(), true)
    );
...
async fn session_confirmed_run_gate() {
    ...
    pg.execute_raw(&format!("delete from sensei.playbook_run where session_id = '{sid}'"))
        .await
        .ok();
}
...
async fn apply_learn_plan_reweights_and_upserts() {
    ...
    use crate::playbook::{Intent, LearnPlan, LearnedRule, Lifecycle, Risk};
    let plan = LearnPlan {
        reweights: vec![(debug.id.unwrap(), debug.base_priority + 5)],
        proposals: vec![LearnedRule {
            lifecycle: Lifecycle::Stable,
            intent: Intent::Feature,
            risk: Risk::Low,
            playbook: "mockup_first".into(),
            priority: 200,
            rationale: "t".into(),
        }],
    };
    ...
    assert_eq!(
        pg.list_playbook_rule_proposals()
            .await
            .unwrap()
            .iter()
            .filter(|p| p["playbook"] == "mockup_first")
            .count(),
        1
    );
}
...
async fn accept_flips_proposal_enabled() {
    ...
    let id = props.iter().find(|p| p["playbook"] == "spec_driven").unwrap()["id"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    assert!(
        ...
        "accepting a real learned proposal returns true"
    );
    assert!(
        pg.list_playbook_rules().await.unwrap().iter().any(|r| r.id == Some(id.parse().unwrap()))
    );
    // A nonexistent id flips NOTHING → returns false (never a fabricated success).
    assert!(
        !pg.accept_playbook_rule(&uuid::Uuid::new_v4()).await.unwrap(),
...
async fn find_duplicates_scoped_surfaces_same_folder_pairs() {
    ...
    assert!(
        dups.iter().any(|d| {
            ...
            names == (Some("_dupfn_a"), Some("_dupfn_b"))
                || names == (Some("_dupfn_b"), Some("_dupfn_a"))
        }),
...
async fn patterns_for_symbol_matches_by_file_and_is_honest_empty() {
    ...
    assert!(
        hit.iter().any(|p| p["name"] == "route-handler"),
...
async fn playbook_combo_trust_is_project_scoped() {
    ...
    let (na, fa) =
        pg.playbook_combo_trust("stable", "bug", "low", "debug_flow", &proj_a).await.unwrap();
    ...
    let (nb, fb) =
        pg.playbook_combo_trust("stable", "bug", "low", "debug_flow", &proj_b).await.unwrap();
    assert_eq!(nb, 1);
    assert!((fb - 1.0).abs() < 1e-9);
    // empty combo in A → (0, 0.0)
    let (n0, f0) =
        pg.playbook_combo_trust("greenfield", "ux", "high", "vibe", &proj_a).await.unwrap();
    assert_eq!(n0, 0);
    assert_eq!(f0, 0.0);
    pg.execute_raw("delete from sensei.playbook_run where feature = 'trust-test'").await.ok();
}
...
async fn model_stats_groups_by_classified_by() {
    ...
    if let Some(r) = rows.first() {
        assert!(r.get("classified_by").is_some() && r.get("ftr_rate").is_some());
    }
}
