...
impl PgStore {
    ...
    pub async fn list_active_memories_global(
        &self,
        limit: i64,
    ) -> Result<Vec<serde_json::Value>, String> {
        let rows: Vec<(
        ...
        Ok(rows
            .into_iter()
            .map(|(id, title, scope, impact, strength, modified)| {
    ...
    pub async fn add_memory_example(
        &self,
        memory_id: &uuid::Uuid,
        node_id: &str,
        is_good: bool,
    ...
    pub async fn list_memory_examples(
        &self,
        memory_id: &uuid::Uuid,
    ) -> Result<Vec<serde_json::Value>, String> {
    ...
    pub async fn get_memory_children(
        &self,
        parent_id: &uuid::Uuid,
    ...
    pub async fn get_memory_parent(
        &self,
        child_id: &uuid::Uuid,
    ...
    pub async fn memory_id_by_source(
        &self,
        source_id: &uuid::Uuid,
    ) -> Result<Option<uuid::Uuid>, String> {
        let row: Option<(uuid::Uuid,)> = sqlx_core::query_as::query_as(
    ...
    pub async fn list_promotion_candidates(&self) -> Result<Vec<serde_json::Value>, String> {
        ...
        Ok(rows
            .into_iter()
            .map(|(id, title, content, ns, enforcement)| {
    ...
    pub async fn memory_telemetry_7d(
        &self,
        memory_id: uuid::Uuid,
    ...
    pub async fn get_memory_detail(&self, id: uuid::Uuid) -> Result<serde_json::Value, String> {
        ...
        let examples: Vec<(Option<String>, bool, Option<String>)> = sqlx_core::query_as::query_as(
    ...
    pub async fn record_outcomes_batch(
        ...
        for r in rows {
            ...
            if s == "archived" || s == "rejected" {
                skipped.push(
                    serde_json::json!({"memory_id": r.memory_id, "reason": format!("status_{s}")}),
                );
                continue;
            }
            sqlx_core::query::query(
