...
impl Provisioner {
    ...
    pub async fn ensure_model(&self, id: &str) -> Result<PathBuf, String> {
        ...
        self.download_verify(&blob_url, &dest, &expected_hex).await?;
...
mod tests {
    ...
    async fn ensure_model_pulls_and_registers_from_registry() {
        ...
        let path = prov.ensure_model("all-minilm").await.expect("ensure_model");
        ...
        let again = prov.ensure_model("all-minilm").await.expect("idempotent");
