...
pub trait ToolDiscovery: Send + Sync {
    ...
    fn discover_mcp(&self, home: &Path, project_roots: &[(uuid::Uuid, PathBuf)]) -> Vec<DiscoveredMcp>;
}
...
fn scan(home: &Path, roots: &[(uuid::Uuid, PathBuf)], fam: AcpFamily) -> Vec<DiscoveredMcp> {
    let mut out: Vec<DiscoveredMcp> = mcp_discovery::discover_user_scope(home)
        .into_iter().filter(|d| d.acp_family == fam).collect();
    for (pid, root) in roots {
        out.extend(mcp_discovery::discover_project_scope(root, *pid)
            .into_iter().filter(|d| d.acp_family == fam));
    }
    out
}

impl ToolDiscovery for ClaudeCodeDiscovery {
    fn family(&self) -> &'static str { "claude" }
...
pub fn plugin_name_from_cache_path(path: &Path) -> Option<String> {
    let comps: Vec<String> = path.components()
...
pub fn discover_claude_plugin_mcps(home: &Path) -> Vec<DiscoveredMcp> {
    ...
    if let Some(doc) = mcp_discovery::read_jsonc(&claude_json) {
        out.extend(
            mcp_discovery::parse_mcp_section(&doc, "mcpServers", AcpFamily::Claude, None, &claude_json)
                .into_iter().filter(|d| !d.command.is_empty()),
        );
    }
    ...
    for entry in walkdir::WalkDir::new(&cache).max_depth(6).into_iter().filter_map(|e| e.ok()) {
        if entry.file_name() != std::ffi::OsStr::new(".mcp.json") { continue; }
        let path = entry.path();
        ...
        for d in parse_any_server_map(&doc, AcpFamily::Claude, path) {
            if d.command.is_empty() { continue; }
    ...
    let sensei_cfg = home
        .join(".claude/plugins/marketplaces/sensei-marketplace/plugins/sensei-mcp/config.json");
...
pub async fn run_capture(pg: &PgStore) -> Result<CaptureCounts, String> {
    ...
    for adapter in all_discoverers() {
        for e in adapter.discover_mcp(&home, &roots) {
            if let Err(err) = pg.upsert_mcp_server(
                e.acp_family.as_str(), &e.mcp_key, e.project_id,
                &e.config_source.to_string_lossy(), &e.command, &e.args, &e.env,
            ).await {
    ...
    for name in &builtins {
        let _ = pg.upsert_assistant_tool("claude", "builtin", "builtin", name, name, None, None).await;
    }
    ...
    for s in &servers {
        let Some(id) = s["id"].as_str().and_then(|x| uuid::Uuid::parse_str(x).ok()) else { continue };
        ...
        let args: Vec<String> = s["args"].as_array()
            .map(|a| a.iter().filter_map(|v| v.as_str().map(String::from)).collect())
            .unwrap_or_default();
        let env: HashMap<String, String> = s["env"].as_object()
        ...
        let cwd = s["config_source"].as_str().map(PathBuf::from)
        ...
        match crate::tasks::mcp_probe::probe_tools(&command, &args, &env, cwd.as_deref()).await {
            crate::tasks::mcp_probe::ProbeOutcome::Ok(manifest) => {
                ...
                let _ = pg.upsert_mcp_tool_manifest(
                    &id, &tools_val, manifest.tools.len() as i32,
                    manifest.protocol_version.as_deref(), manifest.server_name.as_deref(),
                    manifest.server_version.as_deref(), None,
                ).await;
                let _ = pg.set_mcp_server_connection_state(&id, "connected").await;

                let bare: HashSet<String> = manifest.tools.iter()
                    .filter_map(|t| t["name"].as_str().map(String::from)).collect();
                let source_key = bridge_source_key(&bare, &usage_prefixes).unwrap_or_else(|| mcp_key.clone());
                for t in &manifest.tools {
                    if let Some(tn) = t["name"].as_str() {
                        let inv = mcp_invoked_name(&source_key, tn);
                        let _ = pg.upsert_assistant_tool(
                            &family, "mcp", &source_key, tn, &inv, t["description"].as_str(), Some(id),
                        ).await;
                    }
                }
            }
            crate::tasks::mcp_probe::ProbeOutcome::Error(msg) => {
                ...
                let _ = pg.upsert_mcp_tool_manifest(&id, &empty, 0, None, None, None, Some(&msg)).await;
...
mod tests {
    ...
    fn set(xs: &[&str]) -> HashSet<String> { xs.iter().map(|s| s.to_string()).collect() }

    #[test]
    fn bridge_picks_the_best_overlapping_prefix() {
        let probed = set(&["browser_click", "browser_navigate", "browser_evaluate"]);
        let mut usage = HashMap::new();
        usage.insert("plugin_playwright_playwright".to_string(),
                     set(&["browser_click", "browser_navigate"]));           // overlap 2
        usage.insert("svelte".to_string(), set(&["svelte-autofixer"]));       // overlap 0
        assert_eq!(bridge_source_key(&probed, &usage).as_deref(),
    ...
    fn pretty_source_name_labels() {
        ...
        assert_eq!(
            pretty_source_name("mcp", "plugin_playwright_playwright"),
            "playwright"
        );
    ...
    fn parse_any_server_map_handles_wrapper_and_direct() {
        ...
        let wrapped = json!({ "mcpServers": { "semgrep": { "command": "semgrep", "args": ["mcp"] } } });
        ...
        let direct = json!({ "playwright": { "command": "npx", "args": ["@playwright/mcp@latest"] } });
    ...
    fn plugin_name_parsed_from_cache_path() {
        let p = Path::new("/Users/j/.claude/plugins/cache/claude-plugins-official/semgrep/0.5.3/.mcp.json");
