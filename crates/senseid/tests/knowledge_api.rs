...
async fn full_lifecycle_proposal_accept_outcome() {
    ...
    let Some(pid) = std::env::var("SENSEI_TEST_PROJECT_ID").ok() else {
        return;
    };
    let c = Client::new();
    ...
    // 1. Propose
    let propose = c
        .post(format!("{url}/api/knowledge/proposals"))
        .json(&json!({
            "project_id":    pid,
            "scope":         "project",
            "type":          "convention",
    ...
    let mid =
        propose.json::<serde_json::Value>().await.unwrap()["id"].as_str().unwrap().to_string();

    // 2. It appears in status=proposed
    let listing = c
        .get(format!("{url}/api/knowledge/memories?status=proposed&project_id={pid}"))
        .send()
        .await
        .unwrap()
        ...
        .await
        .unwrap();
    let titles: Vec<&str> = listing["memories"]
        .as_array()
        .unwrap()
        .iter()
    ...
    // 3. Accept
    let accept = c
        .post(format!("{url}/api/knowledge/proposals/{mid}/accept"))
        .json(&json!({}))
        .send()
        .await
        .unwrap();
    ...
    let ctx = c
        .get(format!("{url}/api/knowledge/context?project_id={pid}"))
        .send()
        .await
        .unwrap()
        .json::<serde_json::Value>()
        .await
        .unwrap();
    let ctx_titles: Vec<&str> =
        ctx["memories"].as_array().unwrap().iter().map(|m| m["title"].as_str().unwrap()).collect();
    assert!(ctx_titles.contains(&"lifecycle-test"));

    // 5. Record an outcome
    let outcome = c
        .post(format!("{url}/api/knowledge/outcomes"))
    ...
    let detail = c
        .get(format!("{url}/api/knowledge/memories/{mid}"))
        .send()
        .await
        .unwrap()
        .json::<serde_json::Value>()
