...
fn mock_payload(status: HealthStatus, with_remedy: bool) -> HealthPayload {
    let mk =
        |id: &str, label: &str, note: Option<&str>, s: ComponentStatus, verb: &str| Component {
            id: id.to_string(),
...
fn health_event_component_patch_serializes_correctly() {
    let ev = HealthEvent::Component {
        ...
        patch: ComponentPatch { status: Some(ComponentStatus::Installing), ..Default::default() },
    };
    let json: serde_json::Value =
        serde_json::from_str(&serde_json::to_string(&ev).unwrap()).unwrap();
