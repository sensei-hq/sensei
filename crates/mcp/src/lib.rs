...
pub fn daemon_request_for(
    ...
    match tool {
        ...
        "match_pattern" => {
            ...
            Some(
                DaemonRequest::get(format!("/api/patterns/{repo_id}/match"))
                    .with_query("description", desc),
            )
        }
        "get_pattern_for" => {
        ...
        "get_project_conventions" => {
            Some(DaemonRequest::get(format!("/api/patterns/{repo_id}/conventions")))
        }
        ...
        "accept_proposal" => {
            let id = args["id"].as_str().unwrap_or("");
            Some(DaemonRequest::post_json(
                format!("/api/knowledge/proposals/{id}/accept"),
                json!({}),
            ))
        }
        "reject_proposal" => {
...
pub fn build_log_event_body(args: &Value, cwd: &str) -> Value {
    ...
    body["assistant_family"] =
        json!(args["family"].as_str().filter(|s| !s.is_empty()).unwrap_or("claude"));
...
pub fn build_daemon_params(args: &Value, repo_id: &str) -> Value {
    let query =
        args["query"].as_str().or(args["name"].as_str()).or(args["pattern"].as_str()).unwrap_or("");
...
pub fn context_project_param<'a>(
    explicit_pid: &'a str,
    repo_id: &'a str,
...
pub fn rules_query_param<'a>(repo_id: &'a str, cwd: &'a str) -> (&'static str, &'a str) {
    if !repo_id.is_empty() { ("project", repo_id) } else { ("folder", cwd) }
}
...
pub fn resolve_default_project(
    ...
    match (cwd, pin_name) {
        ...
        (Some(c), _) => {
            ProjectResolution::Resolved { name: c.to_string(), source: "cwd", note: None }
        }
        (None, Some(p)) => {
            ProjectResolution::Resolved { name: p.to_string(), source: "pin", note: None }
        }
        ...
        if let Some(folders) = p["folders"].as_array() {
            for folder in folders {
                if let Some(abs_path) = folder["abs_path"].as_str()
                    && cwd.starts_with(abs_path)
                    && abs_path.len() > best_len
                {
        ...
        let mut under: Vec<String> = projects
            ...
            .filter(|p| {
                p["folders"].as_array().is_some_and(|fs| {
                    fs.iter()
                        .any(|f| f["abs_path"].as_str().is_some_and(|a| a.starts_with(&prefix)))
                })
            })
...
mod tests {
    ...
    fn resolve_from_cwd_auto_picks_single_repo_under_parent_else_ambiguous() {
        ...
        let one = vec![
            json!({"id":"a","name":"only","folders":[{"abs_path":"/wrap/only","name":"only"}]}),
        ];
        ...
        assert_eq!(
            resolve_from_cwd_in(&many, "/wrap"),
            "",
            "ambiguous parent → no guess (pass project=/pin)"
        );
    }
    ...
    fn lib_docs_tools_declare_their_documented_params() {
        ...
        assert_eq!(
            sld["inputSchema"]["required"],
            json!(["query"]),
            "search_lib_docs requires query"
        );
    }
    ...
    fn gateway_and_noop_tools_are_handled_inline() {
        for t in ["infer", "embed", "gateway_status", "consensus", "generate_image", "log_event"] {
            assert_eq!(
                daemon_request_for(t, &json!({}), "/cwd", Some("sensei")),
                None,
    ...
    fn context_request_sends_project_name_as_project_query() {
        ...
        let req =
            daemon_request_for("get_layered_context", &json!({}), "/cwd", Some("sensei")).unwrap();
    ...
    fn log_event_body_maps_capture_fields_for_any_assistant() {
        ...
        let d = build_log_event_body(
            &json!({ "type": "phase_transition", "cwd": "/repo", "success": "false" }),
            "/mcp/cwd",
        );
    ...
    fn run_checkers_omits_cwd_folder_when_project_is_given() {
        ...
        let fb =
            daemon_request_for("run_checkers", &json!({}), "/mcp/cwd", Some("sensei")).unwrap();
    ...
    fn commands_request_targets_project_path_with_optional_category() {
        ...
        let filtered = daemon_request_for(
            "get_commands",
            &json!({ "category": "test" }),
            "/cwd",
            Some("sensei"),
    ...
    fn start_run_posts_goal_with_resolved_project_and_optional_plan() {
        ...
        let noproj =
            daemon_request_for("start_run", &json!({ "goal": "g" }), "/cwd", None).unwrap();
        assert!(
            noproj.body.unwrap().get("project").is_none(),
            "no project anywhere → omit the key"
        );
    }
    ...
    fn duplicates_and_conventions_target_pattern_endpoints() {
        ...
        let c = daemon_request_for("get_project_conventions", &json!({}), "/cwd", Some("sensei"))
            .unwrap();
    ...
    fn match_pattern_and_pattern_for_carry_their_args() {
        let m = daemon_request_for(
            "match_pattern",
            &json!({ "description": "add SQL parsing" }),
            "/cwd",
            Some("sensei"),
        )
        .unwrap();
        ...
        let pf = daemon_request_for(
            "get_pattern_for",
            &json!({ "symbol": "SqlAdapter" }),
            "/cwd",
            Some("sensei"),
        )
        .unwrap();
    ...
    fn workflow_state_get_and_put_target_state_endpoint() {
        let g =
            daemon_request_for("get_workflow_state", &json!({}), "/cwd", Some("sensei")).unwrap();
        ...
        let pinned_get = daemon_request_for(
            "get_workflow_state",
            &json!({ "project": "torii" }),
            "/cwd",
            Some("sensei"),
    ...
    fn search_falls_through_to_the_mcp_proxy() {
        let req = daemon_request_for("search", &json!({ "query": "foo" }), "/cwd", Some("sensei"))
            .unwrap();
        assert_eq!(req.method, HttpMethod::Post);
    ...
    fn get_project_summary_proxies_with_repo_id() {
        let req =
            daemon_request_for("get_project_summary", &json!({}), "/cwd", Some("sensei")).unwrap();
        assert_eq!(req.path, "/api/mcp/call");
        let body = req.body.unwrap();
    ...
    fn proposal_lifecycle_requests_hit_id_scoped_endpoints() {
        let acc =
            daemon_request_for("accept_proposal", &json!({ "id": "abc" }), "/cwd", None).unwrap();
        assert_eq!(acc.path, "/api/knowledge/proposals/abc/accept");
        assert_eq!(acc.body, Some(json!({})));
        let rej = daemon_request_for(
            "reject_proposal",
            &json!({ "id": "abc", "reason": "dup" }),
            "/cwd",
            None,
        )
        .unwrap();
        ...
        let prom = daemon_request_for(
            "promote_memory",
            &json!({ "id": "xyz", "gov_scope": "team" }),
            "/cwd",
            None,
        )
        .unwrap();
    ...
    fn unknown_tool_defaults_to_the_mcp_proxy() {
        ...
        let req =
            daemon_request_for("totally_made_up", &json!({}), "/cwd", Some("sensei")).unwrap();
        assert_eq!(req.path, "/api/mcp/call");
    ...
    fn find_projects_uses_explicit_under_over_cwd() {
        let req =
            daemon_request_for("find_projects", &json!({ "under": "/some/dir" }), "/my/cwd", None)
                .unwrap();
        assert_eq!(req.path, "/api/projects");
    ...
    fn pause_run_posts_until_and_defaults_project_to_cwd() {
        let req = daemon_request_for(
            "pause_run",
            &json!({ "until": "2026-07-25T11:30:00Z", "reason": "usage limit" }),
            "/cwd",
            Some("sensei"),
        )
        .unwrap();
    ...
    fn set_stance_posts_dials_and_defaults_under_to_cwd() {
        ...
        let req =
            daemon_request_for("set_stance", &json!({ "autonomy": "run_freely" }), "/my/cwd", None)
                .unwrap();
        assert_eq!(req.method, HttpMethod::Post);
        ...
        assert!(
            ...
            "omitted dials are not sent (daemon keeps stored default)"
        );
    ...
    fn resolve_active_project_returns_id_and_name() {
        ...
        assert_eq!(
            resolve_active_project_in(&sample(), "11111111-1111-1111-1111-111111111111")
                .unwrap()
                .name,
            "sensei"
        );
    ...
    fn default_project_explicit_then_cwd_beats_pin_then_pin_fallback_then_unresolved() {
        ...
        assert_eq!(
            resolved(resolve_default_project(Some("explicit"), Some(&pin), "cwdname")).0,
            "explicit"
        );
        ...
        assert!(
            note.as_deref().unwrap().contains("pinned project is 'pinned'"),
            "stale-pin override is surfaced"
        );

        // cwd + pin AGREE → cwd, no note (nothing to warn about).
        let agree = ActiveProject { id: "id".into(), name: "sensei".into() };
        assert_eq!(
            resolved(resolve_default_project(None, Some(&agree), "sensei")),
            ("sensei".into(), "cwd", None)
        );

        // pin is the fallback ONLY when the cwd resolves nothing (e.g. a container dir).
        assert_eq!(
            resolved(resolve_default_project(None, Some(&pin), "")),
            ("pinned".into(), "pin", None)
        );

        // cwd used when there's no pin at all.
        assert_eq!(
            resolved(resolve_default_project(None, None, "cwdname")),
            ("cwdname".into(), "cwd", None)
        );
        ...
        assert_eq!(
            resolved(resolve_default_project(Some(""), Some(&empty), "cwdname")),
            ("cwdname".into(), "cwd", None)
        );
    }
}
